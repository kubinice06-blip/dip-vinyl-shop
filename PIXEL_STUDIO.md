...
- **色盤**（選單「色盤」或色盤上方的 ⋯）：
  - **產生明暗階**——給一個基底色與階數，直接生一條可用的明暗階：暗部往藍紫偏、彩度拉高，
    亮部往黃偏、彩度降低（色相偏移、彩度變化、明度幅度都可調，暗部也可以改成暖色調給燭光火光用）。
    產生的顏色**連號**接在色盤最後面，並直接選成色階，接著就能用「上色」墨水與色階漸層。
  - **排序**（明暗／色相／彩度／用量／反轉）——畫面上的像素跟著重新對應，顏色不會變。
    `DOC.pal` 的順序也會一起重建，所以存檔再開回來，色階的連續性還在。
  - **合併相近色**——門檻 0–40，先報出「N 色 → M 色」再套用；同一群留下的是**畫面上用得最多**的那一個，
    不是平均色（平均色會生出一個原圖裡根本沒有的顏色）。
  - **刪掉沒用到的顏色**、**換色**（整張圖的 A 像素換成 B，色盤本身不動）。
  - **載入／匯出色盤**：`.gpl`（GIMP／Aseprite／Krita）、`.hex`（Lospec）、`.pal`（JASC-PAL）；
    認不出格式時會把整份文字裡所有六位 hex 撿出來。載入可以「接在後面」或「取代整個色盤」
    （取代時像素會對應到新色盤裡最接近的顏色）。演算法在 `dip-pixel-palette.js`
    （純資料，`node scripts/pixel-palette-test.mjs` 可單獨測）。
  - **拖曳色盤格換位置**——色階必須是連號的一段，能自己排色盤才是「上色」墨水好不好用的關鍵。
- **動畫標籤**（格 → 動畫標籤…）：「第幾格到第幾格是哪一段動作」，存進物件的 `animTags`，
  給遊戲端挑段落播；狀態列會顯示目前這一格屬於哪個標籤。
- **開啟**：可以打字過濾、照最近改過／名稱／尺寸排序，上面一排是最近開過的捷徑。
- **匯入**：圖片（見 2.1）、`.aseprite`／`.ase`（見 2.2）、雪碧圖切成動畫格（見 2.3）。
- **匯出**：PNG ×1–16（此格或每格）、PNG 雪碧圖（所有格排成一張、每列幾格可調）、
  **GIF 動畫**（索引色本來就是 GIF 的原生格式，色盤直接當色表，不會再失真一次；
  倍率上限 8、透明用第 0 格、每格時間吃 `durations` 或 fps）、物件 JSON。
...
### 2.2 匯入 `.aseprite`／`.ase`（檔案 → 匯入 .aseprite）

直接讀 Aseprite 的原生檔，建成一個**新的素材庫物件**（目前開著的圖像不動）。
解析在 `dip-pixel-aseprite.js`（純資料，`node scripts/pixel-aseprite-test.mjs` 可單獨測）。

| 帶得進來 | 帶不進來 |
|---|---|
| 8 bit 索引色／16 bit 灰階／32 bit RGBA 三種色彩深度 | 圖層不透明度與混合模式（我們沒有 alpha，一律當成不透明的一般圖層） |
| 多圖層、多格、cel 的 x／y 偏移 | tilemap 圖層與 tileset |
| cel 三種型態：raw、linked（連結格）、zlib 壓縮 | 切片（slices）與使用者資料 |
| 新舊兩種色盤區塊、每格時間、動畫標籤 | 參考圖層 |

帶不進來的東西**不會默默吃掉**：匯入視窗會把每一條警告列出來，並抄一份進物件的備註。
RGBA 檔的半透明像素一律當成不透明（alpha 低於一半才算透明）；顏色超過色盤上限時，
少用的顏色會對應到最接近的保留色，不會變透明。

> **注意**：解析器是照 Aseprite 的檔案格式規格寫的，測試用的 `.aseprite` 也是照規格自己組出來的
> （雲端機器抓不到真的樣本）。第一次丟真檔案進去請確認一下結果，有出入就把警告訊息貼給 Claude。

### 2.3 雪碧圖切成動畫格（匯入圖片 → 切成動畫格…）

把一張橫排／格狀的雪碧圖切成動畫。走的是 2.1 的匯入分析管線，算完之後再照欄列切開，
由左到右、由上到下排成格。欄列會自動猜（找能整除、而且每格接近正方形的組合），
每一格都有預覽，可以設每格毫秒數，也可以自動裁掉全空的格。切出來是一個新物件。
...
- 圖層只有顯示與鎖定。
- **還沒做**：獨立的動畫預覽視窗（要看動畫目前是按 Enter 在畫布上播）、
  `.aseprite` 的**匯出**（只做了匯入）、切片（slices）、參考圖層。
- `.aseprite` 解析器是照規格寫的、用自己組的檔案測的，沒拿真的 Aseprite 輸出驗證過。
- GIF 每一格都是整張重畫（沒有做只寫差異區域的最佳化），所以格數多的時候檔案會偏大；
  換來的是每一格都獨立、透明不會疊錯。
- 點陣字只有英數與標點兩套內建字型，沒有外部字型匯入，中文一律自己畫。
- 上色墨水要先在色盤上 Shift 選一段色階才有作用；色階是色盤上**連續的一段編號**，
  所以色盤的排列順序要自己顧好——用「色盤 → 產生明暗階」生出來的本來就是連號的，
  手動湊的就自己拖曳排好，或用「色盤 → 排序」。
