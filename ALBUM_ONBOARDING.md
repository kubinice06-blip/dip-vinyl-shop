...
### 0.8 深掘線與非英語圈的冷門軸：錨點制（2026-09-02 店主核可校準）

`/album-rating` 的冷門軸是 **Last.fm listeners 的絕對分級**（由 525 對歷史配對反推，
邊界乾淨：<3,000＝5、<20,000＝4、<100,000＝3、<500,000＝2、其餘 1）。
這套分級對英語圈正典運作正常，但在兩種情況下會系統性失真，機制都是**「聽眾數量的是
流通，不是冷門」**：

1. **考古廠牌的再發會製造聽眾。** Numero／Light in the Attic 把一張私壓盤放上串流，
   它就有了幾萬個 listeners、掉到 2–3 分——而那次再發正是證明它值得收的證據。
   實例：池中 Otis G. Johnson、Junei、Starcrost、Calvin Keys、Rudolph Johnson、
   The Awakening 全部 `obscurity=2`，與正規大廠盤同分；soul 2,279 張只有 3 張 5 分。
2. **Last.fm 的聽眾分佈偏英語圈。** 鳳飛飛《掌聲響起》553 listeners、江蕙《酒後的心聲》303，
   絕對分級都是 5——那是台灣家家戶戶會唱的歌。西里爾、希臘文、阿拉伯文的批次同理。

因此 **`lineType=深掘` 的卡，以及掛名為非拉丁文字的廣度批，冷門軸一律錨點制人工評分**，
與 §0.7 古典同一個做法。定義沿用 §0.7：**冷門＝玩家撞見這份錄音的機率**，
但把「原盤流通」與「再發後流通」兩層分開看：

| 分 | 錨點 |
|---|---|
| **5** | 原盤私壓或極小廠、壓量或通路有據極小（約千張以下、或無商業通路），**且**沒有進入串流的授權再發（黑膠限量復刻不算進入串流） |
| **4** | 原盤如上，但已有一次以上授權考古再發進入串流——「被挖出來了，但仍是收藏家領域」 |
| **3** | 原盤有正規廠牌但當年商業失敗或發行有限；或該語言圈的一線作品在該圈以外幾乎無人知 |
| **2** | 正規廠牌正常流通、有國際發行或該語言圈的主流流通 |
| **1** | 入門名單常客 |

- 非英語圈的卡以**該語言圈**為尺度：鳳飛飛在華語圈是 1，不是 5。
- manifest 的 `ratings.source` 記 `manual:depth-rubric`（非拉丁文字廣度批記 `manual:regional-rubric`）；
  機器值仍記錄於 manifest 供對照，但不得作為分數來源。
- **機器抽查（報警不評分）**：人工 5 但 `_listeners` >20,000、或人工 ≤2 但 `_listeners` <3,000，
  拉清單複核。前者常是再發後的串流流量，後者常是 worker 查到別的實體。
- **pearl 門檻不變**：仍須 `obscurity=5` **且** listeners <300。錨點制可以把一張已有串流再發的
  私壓盤評成 4、把一張沒再發的評成 5，但 pearl 判的是「近乎無人聽過」這個絕對事實，
  再發進串流的碟不會是 pearl——這是設計，不是漏洞。
- 稀有度公式不變。
- **已在池中的深掘型卡**（上述六張與同類）可依此重評；工具 `batch-progress/recalibrate-obscurity.mjs`
  會從本機的 KV 快取列出候選與兩套分級的對照，**不自動寫回**，逐張人工定案。
