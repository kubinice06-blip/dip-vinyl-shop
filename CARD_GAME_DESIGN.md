...
>
> **v3 為何改：** v2 把「宣告屬性」同時當猜拳與傷害數字，導致玩家只挑星星最多的屬性、無視相生相剋。v3 借鏡《三國志X》舌戰/一騎討——**靠讀相剋＋鋪連段充「氣勢槽」，滿了放【論破】爆發**；星數只剩微薄充能，讀招才是勝負手。已用 `battle.html` Beta 實測，數值為 playtest 值、可調。
...
- 傷害幾乎只有兩種來源：**相剋直接壓制**（讀招）與 **論破爆發**（氣勢滿）。星數本身不直接扣血。
...
### 8.5 結算（氣勢 + 傷害）
比對雙方宣告屬性：
- **相剋時**：剋制方 **氣勢 +3**、被剋方 **氣勢 −1**，且被剋方 **HP −3**（壓制傷害，讀招驅動、與星數無關）。
- **無相剋（同屬性或無關）**：純比面板星數，高的一方 **氣勢 +1**（星星只剩微薄加速）；平手不充能、不扣血。
- 結算後 **氣勢槽 ≥ 6 → 自動發動【論破】**：對手 **HP −6**，自身氣勢清零。← 週期性爆發 accent。
- 雙殺（雙方同回合 ≤0）→ 剩血多者勝，無平手。

### 8.6 王牌被動（重綁氣勢經濟）
- **共通【霸權】**（hall/pearl/heresy）：作為被剋方時，**取消該次相剋**（對手不得 +3 氣勢與 −3 壓制，落入純面板比氣勢）。王牌＝難以充能的硬壁壘。
- **殿堂【眾望所歸】**：打出回合，**對手氣勢 −2**（主流壓制，延遲對手論破）。
...
- **P3 對戰 PvE**：依第 8 節 v3 氣勢制實作——氣勢槽＋論破、相生充能、相剋壓制、王牌被動、HP30、疲勞；config 化（`BATTLE_CONFIG`）、打電腦。✅ 可玩 Beta（`battle.html`，內建範例卡池）。
