...
稀有度＝**三項評分加總**（各 1–5，總分 3–15），越高越稀有。順位高→低：

| 總分 | 稀有度 | 顏色 |
|---|---|---|
| —（無數值） | 殿堂 Hall | 🩶 銀 |
| 14–15 | 傳奇 Legendary | 🟠 橘 |
| 12–13 | 史詩 Epic | 🟣 紫 |
| 10–11 | 罕見 Uncommon | 🟢 綠 |
| 8–9 | 稀有 Rare | 🔵 藍 |
| 3–7 | 普通 Common | ⚪ 灰 |

> **殿堂（銀）= 最稀有、沒有數值。** 不是由總分決定，而是「經典模式」抽到的公認殿堂神作（OK Computer 等級）才會是殿堂卡。經典模式觸發機率約 3%（`Math.random() < 0.03`），抽到時結果頁顯示銀色「殿堂級」徽章、不顯示星數；收進卡冊時存 `hall: true`，卡冊以銀框＋銀字呈現、無星數。
> 另：可在 `RARITY_OVERRIDES`（index.html）手動指定某藝人的稀有度（目前 `dollar brand → epic`），優先於總分計算。

> 註：依用戶指定，「罕見綠」順位高於「稀有藍」。
> 例：Boris《Amplifier Worship》5+5+5=15 → 傳奇；Fishmans《男達の別れ》4+5+3=12 → 史詩；ABBA Gold 2+5+1=8 → 稀有。
> 程式：`rarityFromRatings()`（index.html），門檻為 config 可調。

### 門檻調校（重要）
遊戲只抽冷門盤，冷門度常 4–5，總分會偏高 → 高階可能偏多。`card_catalog` 已記錄每張被抽到的卡，**累積一段時間後可拉真實總分分布來微調門檻**，確保五色都出現且「越高級越少」。
