...
## 8. 對戰系統（PvE，規則 v3 · 品味氣勢制 — 2026-06-26）
...
### 8.7 回合流程
1. **抽牌**：各抽 1；牌庫已空 → 觸發疲勞（見 8.8）。
2. **選曲**：暗選 1 卡 ＋ 點選主打屬性，鎖定。
3. **亮牌**：同時公開（王牌即時被動：異端歸零氣勢、殿堂壓氣勢先生效）。
4. **相生**：各算連段充能。
5. **相剋／數值**：算氣勢增減與壓制傷害。
6. **品味碾壓**：氣勢滿者發動（玩家自選、電腦自動），無視對方屬性一擊。
7. **棄牌**：用過的卡入棄牌堆；手牌 >7 棄到 7。進下一回合。

### 8.8 牌庫耗盡（疲勞傷害）
牌庫空、無牌可抽時：該方該回合不補牌並受疲勞傷害，**首次 1 點、之後每次遞增 1**（1/2/3…），不可被減免，可達成 HP≤0 判負。對局拉長後加速收尾。

### 8.9 工程備註
- **先做 PvE**：幾乎純前端、零後端，先驗證好不好玩；PvP（配對/同步/防作弊）確認有趣後再評估。已有可玩 Beta：`battle.html`。
- **AI 難度**建議分級：簡單＝隨機合法出牌；普通＝追連段＋危急時放王牌（Beta 現況）；困難＝會預判對手連段並讀相剋。
...
_最後更新：2026-06-26（對戰系統升級為 v3 品味氣勢制＋可玩 Beta）。這份文件可隨想法調整。_
