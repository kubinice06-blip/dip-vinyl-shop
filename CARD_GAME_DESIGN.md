...
- `classic` 經典度、`accessibility` 硬蕊度（AI 依樂評共識）
- 未來新增數值都放這層
...
> | `heresy` | 異端 | 硬蕊度 | AI 推極難入耳的前衛/噪音怪盤（異端模式，主觀軸不驗數據） | 暗赤血光 `--crimson-shine` | `heresy:true` |
...
- **一般卡種子池 `seed_cards.json`**：5526 張 `[artist, album, classic, obscurity, accessibility]`（2026-07-07 精選版：MusicBrainz 收割的一萬張雜盤太多、近半在 Spotify 無封面，故大砍——手工 3055 張全留＋收割卡經「雜盤關鍵字過濾→每藝人上限（依冷門度 3/2/1 張）→ iTunes API 逐張驗真」＋台灣/華語經典包 80 張全數驗過 Spotify 封面，伍佰、草東、羅大佑、交工樂隊、閃靈、落日飛車、椎名林檎等；再把被誤殺的淘汰卡直連 Spotify 複驗撈回 490 張〔誤殺兩來源：限流風暴期 worker 競態把空值寫進 KV、iTunes term search 誤判如 Radiohead〈Pablo Honey〉只回 boy pablo〕）。五級分布 稀有2750／獨特1571／普通1023／史詩164／傳奇18。真消失的 389 張備份在 repo 外 `~/dip-vinyl-home/seed_dropped_backup.json`。殿堂名單擴至 400 張（heresy/pearl 各 100，共 600）。後台同分頁可批次匯入 `card_catalog`（含 `rarity` 欄；新卡 `updatedAt=1` 沉底，不洗掉玩家抽過的卡；封面顯示時才懶抓回存）。
- **封面預熱（2026-07-06）**：`seed_cards.json` 約 4234 張的封面已用 iTunes 600x600 圖直接 PATCH 進 Firestore `card_catalog`（前台/對戰查封面第一順位讀 catalog、命中就不打 Spotify），另 490 張複驗撈回卡的封面走直連 Spotify 存入。目的：對戰開局隨機發牌不再因 Spotify 限流而整片沒封面。
...
> 欄位沿用既有目錄：`classic`(經典度)、`obscurity`(冷門度)、`accessibility`(硬蕊度，**高＝越難入耳/越前衛**)。
>
> **v3 為何改：** v2 把「宣告屬性」同時當猜拳與傷害數字，導致玩家只挑星星最多的屬性、無視相生相剋。v3 借鏡《三國志X》舌戰/一騎討——**靠讀相剋＋鋪連段充「氣勢槽」，滿了放【品味輾壓】（玩家自選、無視對方屬性的保證一擊）**；星數只剩微薄充能，讀招才是勝負手。已用 `battle.html` Beta 實測，數值為 playtest 值、可調。
...
- **傷害＝星數驅動，讀招決定打不打中、星數決定多痛**：相剋命中扣血 = 勝方那招星數（1–7）；純比星扣血 = 星數差；品味輾壓固定 −6（無視屬性）。→ 讀招（命中）與星數（傷害）兩個維度都要算，既不是「只比星數大小」、也不是「星數無用」。
...
> - **硬蕊 accessibility**＝其實是「入耳<u>難度</u>」＝前衛實驗的硬度，**越前衛／越解構＝星數越高**（非「好聽／洗腦」，勿搞反）。

**相生（自我充能，循環：經典 → 冷門 → 硬蕊 → 經典）**——宣告屬性接續上一回合者觸發；這條鏈是一個聽眾的**進化史**：
...
**相剋（對手壓制，循環：冷門 剋 經典 → 經典 剋 硬蕊 → 硬蕊 剋 冷門）**——每組剋制都有文化上的道理：
...
### 8.5 結算（氣勢 + 傷害）
比對雙方宣告屬性：
- **相剋時**：剋制方 **氣勢 +3**、被剋方 **氣勢 +1**（安慰、不倒扣），且被剋方 **HP − 剋制方那招的星數**（讀招命中，傷害＝口徑；普卡 1–5、王牌單軸 7）。但若被剋方比剋制方 **高 2 星以上**，數值輾壓使相剋失效，改走純比星（例：硬蕊 2 對冷門 4，仍由冷門 4 勝）。
- **無相剋（同屬性或無關）**：純比面板星數，高的一方 **氣勢 +1**、且 **對手 HP −（星數差 × valueDmg，預設 ×1）**；平手雙方 **氣勢 +1**、不扣血。（`valueDmg` 設 0 即回到「純比星只給氣勢、不扣血」。）
- **全 1 弱牌不設數值豁免**：仍能用來觸發經典回血、冷門充氣勢、相生連段，或剋制 1～2 星招式；Roguelike 裡另可拿來割捨或以「丟最弱盤」淘汰。這讓弱牌保有牌組管理價值，但不會因補償過頭變成萬用剋制牌。
- **品味輾壓**：**氣勢槽滿（≥6）時，玩家可自選**對一張卡發動——**無視對方屬性的保證一擊，對手 HP −6**，自身氣勢清零（電腦則在氣勢滿時自動發動）。← 主導權在玩家、週期性爆發。
- 雙殺（雙方同回合 ≤0）→ 剩血多者勝，無平手。

### 8.6 王牌被動（重綁氣勢經濟）
- **共通【霸權】**（hall/pearl/heresy）：作為被剋方時，**取消該次相剋**（對手不得 +3 氣勢與 −3 壓制，落入純面板比氣勢）。王牌＝難以充能的硬壁壘。
- **殿堂【眾望所歸】**：打出回合，**對手氣勢 −2**（主流壓制，延遲對手輾壓）。
...
3. **選曲**：玩家讀提示，暗選 1 卡 ＋ 主打屬性（或氣勢滿時選品味輾壓）。
4. **亮牌**：公開（王牌即時被動：異端歸零氣勢、殿堂壓氣勢先生效）。
5. **相生 → 相剋／數值 → 屬性效果**：依序算氣勢增減與壓制傷害。
6. **品味輾壓**：氣勢滿者發動（玩家自選、電腦自動），無視對方屬性一擊。
7. **棄牌**：手牌 >7 棄到 7。回到 1。
...
const BATTLE_CONFIG = {
  ...
  valueCrush: 2,      // 被剋方高 2 星以上 → 相剋失效，改純比星
  qiValue: 1,         // 數值勝充能
  ...
  ultimate: 6,        // 品味輾壓傷害（無視對方屬性、保證命中）
  attrFx: { classic: 1, obscurity: 1, accessibility: 1 }, // 屬性效果：經典守+HP / 冷門挖+氣勢 / 硬蕊攻-對手氣勢
...
- **P3 對戰 PvE**：依第 8 節 v3 氣勢制實作——氣勢槽＋品味輾壓、相生充能、相剋壓制、王牌被動、HP30、疲勞；config 化（`BATTLE_CONFIG`）、打電腦。✅ 可玩 Beta（`battle.html`，內建範例卡池、AI 已調軟）。
