...
稀有度＝**所有數值加總**，越高越稀有。三個數值各 1–5，總分範圍 **3–15**。

| 總分（3 數值） | 稀有度 | 視覺 |
|---|---|---|
| 13–15 | 傳說 Legendary | 發光描邊 / 動態 |
| 11–12 | 史詩 Epic | 金/紫框 |
| 9–10 | 稀有 Rare | 銀框 |
| 3–8 | 普通 Common | 一般 |

> 例：Fishmans《男達の別れ》冷門4+經典5+入耳3 = 12 → 史詩；ABBA Gold 2+5+1 = 8 → 普通。
> 越稀有 = 又冷門、又經典、又有深度的「深櫃神作」，剛好符合樂迷收藏的成就感。

### 未來加數值要用「比例」而非「絕對總分」（重要）
若直接用總分當門檻，未來加第 4 個數值（最高總分變 20）門檻就全錯了。
所以建議稀有度用**比例分數**：
```
rarityScore = 數值總和 / (數值個數 × 5)   // 永遠是 0~1
```
門檻用比例（加幾個數值都不用改）：
| rarityScore | 稀有度 |
|---|---|
...
- **P0 基礎**：Firebase 匿名登入；結果頁「收進卡冊」；Firestore 寫入。
- **P1 卡冊**：我的卡冊頁、稀有度視覺、重複張數、懶人補值顯示。
- **P2 帳號**：綁定 Google 跨裝置同步。
