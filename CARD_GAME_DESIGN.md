...
- 傷害幾乎只有兩種來源：**相剋直接壓制**（讀招）與 **品味碾壓**（氣勢滿時自選發動）。星數本身不直接扣血。
...
> **星數退位**：v4 後星數只剩「平手微小加成 + 卡的稀有度身分」，不再是選屬性的主因——選屬性是為了讀相剋／續連段／拿屬性效果。

### 8.5 結算（氣勢 + 傷害）
比對雙方宣告屬性：
- **相剋時**：剋制方 **氣勢 +3**、被剋方 **氣勢 +1**（安慰、不倒扣），且被剋方 **HP −3**（壓制傷害，讀招驅動、與星數無關）。
- **無相剋（同屬性或無關）**：純比面板星數，高的一方 **氣勢 +1**、低的 **+0**；平手雙方 **+1**（皆不扣血）。
...
_最後更新：2026-06-26（v4：屬性專屬效果＋半公開讀心虛晃，根治「只挑高星」問題）。這份文件可隨想法調整。_
