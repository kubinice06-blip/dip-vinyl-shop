...
## 第 431 條（主線 2026-09-15，研究層 a 組交件後）：**《Drum Shot》改 1968——「兩個來源逐日相符」有時是同一個來源被數了兩次**

策展層第 404 條以「MB 的 1971-07-10 與店面官方條目逐日相符」把年份定在 1971。
**研究層查出那兩筆是同一份數位母帶**：MB 轄下唯一那筆是 NIPPONOPHONE 的 Digital Media release，
與店面 `1752085022`（℗ 1971 Nippon Columbia／NIPPONOPHONE）**是同一組欄位，軌長一秒不差正是同源的證據**。
日本コロムビア 官方 discography（COCB-54128）明寫「**原盤 1968 年発売／LP JAZZ-16／1967-12-14 錄音**」，九軌逐軌相同。
**卡單改 1968。**

**這是這條線最值得記的一條方法論**：
**「兩個獨立來源一致」要先確認它們真的獨立。** 數位發行的 metadata 會同時餵給資料庫與店面，
**逐日相符、軌長一秒不差，恰恰是同源的特徵，不是交叉驗證。**
真正的獨立來源是：**實體盤面／廠牌自己的目錄頁／同期紙本**。

## 第 432 條（同日）：**《The Dialogue》的 catno 刪掉 ALJ-1047**

四說裡 **ALJ-1047 查無任何實據**，ALJ-1059／ALJ-3359／ALJ-HG1 三個都查得到實體條目。
**卡單 `label` 欄已刪掉 ALJ-1047。** 年份 1977 站得住（四個二手來源一致、零個 1978），
**但仍是「四個二手一致」不是一手證實**——寫進 HANDOFF 給本機。

## 第 433 條（同日）：**《Oliver Nelson In Tokyo》的掛名：原信夫為主體，Oliver Nelson 寫成客座**

三條依據：廠牌把本張放在原信夫的 discography 底下；廠牌頁把 Nelson 的職務寫成編曲・指揮＋薩克斯風、
演奏主體是樂團十七人；MB 的 RG credit 是單一元素。**不得寫成兩人共同領銜**（池中已有 Oliver Nelson 五張）。
另補上原盤編號 NCB-7002 與原盤發行日 1970-12-25，**2013→1970 的改判覆核成立**。

## 第 434 條（同日）：**第 254 條那個坑的累計——同一位在店面的 artistId 數目沒有上限**

原信夫有**第三個** artistId `292640268`（2008 年 King 復刻整段目錄只掛在它底下）；
鈴木宏昌《Rock Joint Cither》掛在第三個 id `1553206089`（名下只有兩張）。
本線至今踩了九次。**藝人目錄那一步，要先把該藝人在店面的全部 artistId 找齊再逐個 dump。**

## 第 435 條（同日）：**同一個編制掛名被拆進兩個 MB 實體，`chk-prop` 看不到**

群組實體 `817512f5` 掛著 c-131 的《ロック・ジョイント琵琶》與本批的《Primrose》，
而本批《Rock Joint Cither》掛的是個人實體 `e2ece9b8`——**同一個「鈴木宏昌トリオ」分屬兩個實體**。
另：《Primrose》的 rgMbid 版本位是 3（UUID v3），**任何檢查 v4 的工具都會標紅，那不是錯誤**。
