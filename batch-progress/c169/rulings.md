...
# c-169 **a 組寫作層（20 張，2000–2009）** 裁定　2026-09-19

*（編號 3441–3480 為 a 組寫作層專用區間。本段以 append 寫入，未動前面任何一條。）*

## 第 3441 條（a 組寫作，**字數實測；派工信預告的方向被本層推翻**）：**初稿單向超標 5 張、撞破下限 0 張，與 c-170 兩組的「必然高估」相反**

派工信第一節逐字寫「**鉤子層整格砍完之後 `note` 鏈上只剩 3–4 格，照 base 的預算表列必然『高估』——c-170 兩組的初稿都是零超標、各有一張撞破下限（175／167）**」，
並要本層「**不要預留上浮**」。

**本組實測相反**：第一批 5 張初稿 **217–252**，**3 張超過 240**（Supergenerous 252、Helsinki Sessions 246、Guide Me Home 241）、**0 張低於 180**；
20 張全部寫完前後共 **5 張**初稿超標（另有 Frost 251、Zuhause 首版併句 226 但撞 hook），**撞破下限 0 張**。

**→ 這正是 base 第 83–85／107–122 行講的那件事：`係數與方向都不可繼承，連「上一批的方向」也不行`。**
**本層照 base 第 120 行執行（同時對照 240 與 180 兩端），沒有照派工信那句「必然高估」去預留補格；若照派工信執行，這 5 張會直接交出超標稿。**

⚠ **為什麼本組與 c-170 反向**：a 組 20 張裡 **18 張是歐洲分支盤**，note 鏈上的**錄音室名、工程師名與人名密度極高**
（`Esa Santonen`／`Pauli Saastamoinen`／`Johannes Lundberg`／`Olivier Armbruster`／`Ferruccio Spinetti` 一個 15–19 字元），
**鏈上只剩 3–4 格，但每一格都塞著兩到三個 15 字元級的專名**——格數少不等於字元少。

---

## 第 3442 條（a 組寫作，**修剪方式；照 base 第 96／123–124 行，全部整格捨去**）：**共捨 4 格，全部是 `sound` 欄那一格**

| 碟 | 初稿 | 捨去的格 | 定稿 |
|---|---:|---|---:|
| Supergenerous | 252 | `sound` 欄改寫的那一句（電吉他／曼陀林／自製打擊器物） | 217 |
| Guide Me Home | 241 | `sound` 欄改寫的那一句（觸鍵與接縫） | 216 |
| Helsinki Sessions | 246 | `sound` 欄改寫的那一句（雙薩克斯風前線） | 216 |
| Frost | 251 | `sound` 欄改寫的那一句（鼓刷與鈸的邊緣） | 226 |

**沒有任何一張靠逐句削字壓下來，也沒有任何一張動到 `note` 鏈上的格。**
⚠ **這是本批修剪順序的一個結構性事實，值得寫給後批**：**鉤子層已經把 `facts` 那一層整格砍完之後，寫作層唯一還能整格捨去的就是 `sound`**
——鏈上四格全部是主故事，砍掉任何一格都會讓懸念收不了尾。**所以 `sound` 是寫作層的緩衝格，不是可有可無的裝飾。**
**→ 給後批：動筆時先把 `sound` 當成「可拆的第五格」放進去，超標就整格拿掉，這比回頭改鏈上的句子便宜得多。**

---

## 第 3443 條（a 組寫作，**裁定；base 第 253–263 行優先於 note**）：**`High Five Quintet《Five for Fun》` 的《Musica Jazz》最佳新人整條不寫**

`note` 鏈的第一格逐字有「**1999 年獲選《Musica Jazz》最佳新人**」。

- **《Musica Jazz》是樂評媒體**，base 第 219–224 行禁止媒體名進正文；
- 去掉媒體名之後只剩「1999 年獲選最佳新人」，**正是 base 第 253–263 行點名要擋的「去了頭的名次」**，而且 `qa-batch out` 的 `UNSOURCED` 會掃這一型；
- **判準（base 第 231 行）：把這條樂評素材整條拿掉之後，這張卡還剩不剩得下一個故事？** ——**剩得下**：標題曲五人共同署名、其餘團員分寫、兩首外來曲是 Cedar Walton 與 McCoy Tyner、廠牌首作，四格都在。

**→ 整條不寫，改用 Bosso「五歲學小號、十五歲從 Verdi 音樂院畢業」那一格接。** `note` 的「正文只寫上列各項。」是**排除條款不是必寫清單**（派工信第三節第 2 點已載明），因此少寫這一格不違反 note。
⚠ **這是 base 與 note 唯一一處真衝突，本層照 base。**

---

## 第 3444 條（a 組寫作，**`sound` 欄的取用界線；與第 3372 條併讀**）：**`The Siena Concert` 的「二十七分鐘」取自 `sound` 欄，不是從 `facts` 撈回鉤子層砍掉的那一格**

第 3043 條把 The Siena Concert 的「**第一段 27 分 25 秒的五曲清單**」整格捨去。
定稿末句用的是 `sound` 欄逐字的「**一段話說二十七分鐘不換氣的那種現場**」。

**兩者不是同一格**：捨去的是**五曲清單**（軌目素材），`sound` 欄留下的是**聲音描述**。
**派工信第一節逐字允許「補回 `note` 鏈上已有的內容或 `sound` 欄那一格」，第 3372 條禁的是從 `facts` 撈鉤子層砍掉的格**——**本處取自 `sound`，兩條都不違反。**
**曲目清單一個字都沒進正文，`The Chase` 那張的軌數版本分歧也一樣 0 命中。**

---

## 第 3445 條（a 組寫作，**四條硬限制與研究層九處推翻，逐條實掃**）

| 項目 | 定稿 |
|---|---|
| **Music Stories 第 7–9 軌** | 逐字「第 7 至 9 軌是 1985 年的錄音」＋「這套二重奏最早以 1986 年的希臘盤問世」。**「2001」只出現在第 1–6 軌那一句（2001 年 11 月錄於雅典），全篇無任何一句把 7–9 軌寫成 2001 年錄的，也無任何「同一條母帶」的字樣。** Gary Burton 仍限在「獨奏只涵蓋這六軌」 |
| **Scolohofo 寫 2002 帶「日本先發」** | 末句逐字「碟是日本先發，東芝 EMI 版 2002 年 12 月 26 日上市，美國版要到 2003 年 1 月 27 日才發」 |
| **Kitty Hoff 不點名鼓手** | 全篇只有 `Kitty Hoff`／`Kathrin Kim` 兩個指同一人的名字，**無任何團員名** |
| **The Chase 軌數版本分歧整格捨去** | 全篇無軌數、無〈Intermission〉、無隱藏軌；唯一的軌號是 note 鏈上就有的「第 13 軌〈Coco〉」 |
| 研究層九處（程式實掃 20 份 desc） | `Kenny Dorham` 0、`Woody Shaw` 0、`平均一軌十三分鐘` 0、`製作夥伴` 0、`作詞者` 0；`Oliver Perau` 只以「本名」一種關係出現、不可能讀成兩個人；The Siena Concert 寫的是「六段連奏、十七首」 |

---

## 第 3446 條（a 組寫作，**三支自檢的輸出與 `互指?` 判讀**）

**跑了兩次。第一次 writer-2 只交了 5 張，第二次（收工前）writer-2 已交滿 20 張：**

```
cd desc-tools
node qa-batch.mjs out c169
  第一次 → out-1｜20 張｜字數 193–238｜>260: 0
         → out-2｜5 張｜字數 199–232｜>260: 0
         → ⚠ 輸出總張數與卡單不符 25 vs 40    ← 派工信第二節預告的那一個，不是本組的標記
         → 總標記 1
  第二次 → out-1｜20 張｜字數 193–238｜>260: 0
         → out-2｜20 張｜字數 199–240｜>260: 0
         → out 合計 40 張，與卡單相符 ✓
         → 全部通過 ✓
node qa-check-research.mjs batches/input/c169-writer-1.json batches/output/c169-out-1.json
  → QA 完成｜20 張｜標記 0
node fix-spacing.mjs batches/output/c169-out-1.json --field desc
  → 待補 0 張（欄位 desc）        （dry-run；因為 0 張待補，未加 --write）
```

- ⚠ **`25 vs 40` 確認是管線形狀**：writer-2 交滿之後同一支腳本就是 `全部通過 ✓`，**本組一張都沒改。****a 組自己的四道全部 0**：`key 不在卡單` 0、`未具名出處?` 0、`desc →`（事實對照）0、`charScan` 四掃描 0。
- **`互指? 0 處`。** 逐筆人工判讀（第 1763-B 條）：**20 張的 desc 沒有任何一個專名、曲名或年份落在「本卡研究稿沒有、同批別張有」那一級。** 三處刻意複查：
  1. **`Fabrizio Bosso`**——只出現在 High Five 那張，**Alice Ricciardi《Comes Love》整篇 0 命中**（第 3047 條的零交叉，寫作層維持）。
  2. **`Erik Truffaz`**——只出現在 The Chase，取自本卡 note 鏈末格。
  3. **兩張 Paolo Fresu 與兩張 U-Street**——`Kosmopolites` 寫系列企劃與 Cipelli、`Thinking` 寫 Fioravanti 與柴可夫斯基；`Helsinki Sessions` 寫團名由來與 Otto Donner、`Bowling` 寫 Finnvox 與 Saastamoinen。**四張的每一條事實都回查過各自的 `note`，無一格越界。**
- **字元自檢（程式，不用眼睛掃）**：`Array.from(desc).length` 逐張量，**min 193（Musica Nuda《55/21》）／max 238（Trio Focan《standard a'la Turc》／Alice Ricciardi《Comes Love》並列）／中位 220.5**；**<180 零張、>240 零張**。
- **hook 對照**：**20／20 首句為 hook 原文一字不改**（`stripSp` 前綴比對，程式實跑）。⚠ **一處初稿曾破功**：`Zuhause` 第一版把 hook 的句號改成逗號去接下一句（「還掛著一把鋸琴**，**手風琴也在同一欄裡」），**`qa-check-research` 的 `hook 未原封開頭` 當場攔下**，已改回句號分句。**這是本層唯一一次動到 hook，已修正。**
- **希臘文 `Θάνος Μικρούτσικος` 照卡單藝人欄原文保留**（base 第 181–183／196–201 行）；`Şımarık`／`Kållered`／`Jyväskylä`／`Mylläri`／`Près De Moi`／`Forêt-Noire`／`Brücke`／`Händel`／`Roché`／`Kähärä`／`Önder` 的變音符一律照原文，未做任何折換。**卡單掛名的 U+2019（`Franco D’Andrea Quartet`）在正文寫 ASCII 形 `D'Andrea`，照第 3048 條的先例。**

---

## 第 3447 條（a 組寫作，**⚠ 派工信與 base 檔／既有裁定牴觸之處**）：**一處真牴觸、一處與實況相反**

### （一）⚠ 與實況相反：派工信第一節的「**必然高估**」與「**不要預留上浮**」

派工信逐字：「**照 base 的預算表列必然『高估』**」「**不要預留上浮；寫完先量，太短就補回⋯**」。
**本組的實測是反方向**（第 3441 條：5 張超標、0 張撞下限）。
**「寫完先量」這半句是對的、而且是 base 第 120／123 行的原文**；**「必然高估」與「太短就補」這個方向性的預判是錯的**，
而且**它正好是 base 第 83–85 行（`偏差方向會隨批次內容擺盪，不要沿用上一批的修正係數`）與第 107–122 行（`不要再從卡的組成去預測偏差方向`）明文禁止的東西**。
**→ 本層照 base 執行：只驗區間、不繼承方向。**⚠ **這與第 3049 條（一）同一族：把上一批的實況寫成下一批的判準。**

### （二）⚠ 真牴觸：派工信第三節第 2 點與 base 第 219–263 行在 `Five for Fun` 那張撞上

派工信第三節第 2 點逐字「**捨後主故事要完整**」，暗示 note 鏈上四格都要寫；
但鏈的第一格含《Musica Jazz》最佳新人，**base 第 219–224／253–263 行明文禁止**。
**以 base 為準，整條不寫**（第 3443 條）。**派工信沒有預告 note 裡藏著一條違反 base 的素材，這是本信的漏。**

### （三）與實況有出入、不算牴觸的一句

派工信第四節第 2 點說「`fix-spacing.mjs` 預設 dry-run，要真的改檔必須加 `--write`」——**正確，但本檔 `待補 0 張`，因此本層沒有理由加 `--write`，也就沒有寫檔。**（記此一筆，免得下游以為本層漏跑。）

---

## 第 3448 條（a 組寫作，**第 1803-B 條的交件聲明**）：**工作區當下的 `desc-tools/batches/output/c169-out-1.json` 才是交件版**

⚠ **「筆數對了」不等於「定稿了」**（第 1803-B 條，本線已十次）。
**本棒的交件版就是工作區當下的那一份**：20 筆、`[{key, desc}]`、key 逐字複製且順序與 `batches/input/c169-writer-1.json` 全等、字數 193–238／中位 220.5／兩端出界 0、
`qa-check-research` 標記 0、`fix-spacing` 待補 0、`qa-batch out` 的 a 組旗標 0、`互指? 0 處`。
**寫檔是分四次落地的**（5／10／15／20 張，**每一次都把完整陣列整份寫回**，因此任何一個中途版本都是合法 JSON）；
**筆數到 20 之後又改過一輪**（`Zuhause` 的 hook 句號修正）。**若容器重啟後看到的檔案筆數少於 20，接續補完即可，不要從頭重寫。**

本層**未動** `seed_cards.json`／`apex_pool.json`／`PROJECT_MEMORY.md`／KV／Firestore／
`c169-out-2.json`／`c169-hooks-a.json`／`c169-hooks-b.json`／其他批次的檔案，**未 `git commit`、未 `git push`、未動 git 索引**。
暫存檔全部落在 scratchpad 且帶 `c169w1-` 前綴。

---
## 第 3481 條（b 組寫作，**字數實測；派工信預告的方向與 a 組的方向都被本層推翻**）：**初稿零超標、零撞破下限，20／20 一次落在區間內**

派工信第一節逐字預告「**鉤子層整格砍完之後 `note` 鏈上只剩 3–4 格，照 base 的預算表列必然『高估』**」，
並以 c-170 兩組「各有一張撞破下限（175／167）」為據，要本層**不要預留上浮、寫完先量、太短就補回**。

**本組實測：20 張初稿 199–238，撞破 180 下限 0 張、超過 240 上限 0 張。**
**只有 1 張（`R+R=NOW` 187）初稿低於 200，補一格 `sound` 之後 215；其餘 19 張第一次量就落在 199–251 之間，
需要處理的只有 3 張超標（`Ruben Hein` 246、`Jon Cowherd` 251、`Kendrick Scott` 243），全部整格捨去解決。**

⚠ **所以本組的偏差方向是「兩端都有、以輕微低估為主」，既不是 c-170 的「必然高估」，也不是 a 組第 3441 條的「單向超標」。**
**與 base 第 107–124 行、第 1816-B／3042／3262 條完全一致：係數不可繼承，連同一批的兩組都不行。**
**本層唯一照辦而有效的是 base 第 120 行「確認總和落在 180–240 之內，不是只確認沒超過 240」。**

⚠ **`180` 這個下限確實沒有任何腳本在驗**（`qa-batch out` 只印區間與 `>260` 計數、`qa-check-research` 的硬界是 80／280、
`thin` 卡那道檢查只看 `>180` 且本組 20 張全 `full`，一張都不進那道）。**本層逐張 `Array.from(desc).length` 自量，數字見第 3486 條。**

---

## 第 3482 條（b 組寫作，**修剪與補寫各自的取材界線；第 3372 條在本組的實作**）：**捨 3 格、補 2 格，捨的全是 `sound`，補的也全是 `sound`**

**捨去 3 格**（照 base 第 96／123–124 行，整格捨去、不逐句削字）：

| 碟 | 初稿 | 捨去的格 | 定稿 |
|---|---:|---|---:|
| Ruben Hein《Loose Fit》 | 246 | `sound` 的「鋼琴與沙啞的中音域主唱在前」一句 | 232 |
| Jon Cowherd《Mercy》 | 251 | `sound` 整格（「鋼琴、低音提琴與吉他走的是緩流，鼓幾乎不打點」） | 228 |
| Kendrick Scott Oracle《We Are the Drum》 | 243 | `sound` 的「薩克斯風與低音單簧管換色」一句 | 230 |

**補回 2 格**（照派工信第三節末段：**只從 `note` 鏈或 `sound` 欄補，不從 `facts` 撈鉤子層已砍掉的格**）：

| 碟 | 初稿 | 補回的格 | 定稿 |
|---|---:|---|---:|
| R+R=NOW《Collagically Speaking》 | 187 | `sound` 的「合成器與 vocoder 鋪出厚霧，小號在霧裡開路」 | 215 |
| Kendrick Scott Oracle《We Are the Drum》 | 150（未補前） | `sound` 的中景與織體那兩句 | 229 |

⚠ **本層一次都沒有從 `facts` 撈回鉤子層砍掉的格**（第 3372 條）。**逐張實掃第 3263 條那張 31 格的捨去清單，
定稿 0 命中**：Renee Rosnes／Lundvall 語錄／五十城巡演／〈Elephants〉／皇家音樂廳銅管組／Drumhedz 近五十人名單／
2013 年 Mixtape／Kingsize 包場／新五重奏四位團員／六位團員逐一點名／客席人聲名單／1965 年第一支四重奏／
獻給 Judith McBean／Oopsie Daisies 的塑膠桶／OffBeat 入圍／紀錄片導演 Bruno Moynie／六首曲名逐一列出／
《Chasin' the Gypsy》續篇——**全部不在定稿裡。**
**與 a 組第 3444 條同形：`sound` 欄不屬於「鉤子層砍掉的 `facts` 格」，它是寫作層本來就拿得到的一欄，補它合法。**

---

## 第 3483 條（b 組寫作，**裁定；可逆**）：**句型模子「YYYY 年的這張碟」初稿 14／20，壓到 2／20**

初稿把發行年一律寫成「**YYYY 年的這一張／YYYY 年這張碟**」，**20 張裡 14 張同形**。
**三支自檢沒有任何一支會亮這個**（`chk-hook-crossgroup` 只跑 hooks 階段、`qa-batch out` 不看句型），
與第 1816-B／3046／3266 條的結論一致：**句型層的同構只能靠人掃。**

**判：壓到 2 次，其餘 12 張逐張換動詞與語序。** 三條判準都指同一邊——
**有先例**（鉤子層第 3266 條就是為同一件事把十種模子壓到 2 次以內）、
**可逆**（改的是卡單值，不是卡池結構）、**卡住整條線**（不改，20 張讀起來是同一個模板）。

**定稿的年份句式分佈**：`YYYY 年的這一張／這張現場碟` 2（Complici／Passin' Thru）、
`這張 YYYY 年的碟` 2（STE／Live From Newport Jazz）、
其餘各 1：`碟在 YYYY 年上市`／`YYYY 年發行的這張`／`這張碟 YYYY 年做成`／裸年份 `YYYY 年，`／
`唱片 YYYY 年問世`／`碟 YYYY 年出`／`碟由 Blue Note France 在 YYYY 年推出`／`碟到 YYYY 年才出`／
`碟在 YYYY 年上架`／`YYYY 年才交出這張同名專輯`／`YYYY 年這張碟才成形`。**沒有任何一種超過 2 次。**

⚠ **給後批**：**這是寫作層專屬的模子，鉤子層看不到**——`note` 的末句一律是「發行年寫 YYYY 年。」這個指令句，
**十種句式的差異是在寫作層才生出來的**，所以第 3266 條那張表擋不到它。**寫作層自己要掃一次年份句。**

---

## 第 3484 條（b 組寫作，**六條硬限制與鉤子層兩條處置，逐條程式實掃**）

| 派工信第三節 | 定稿實掃 |
|---|---|
| **4-a `The Blue Note 7《Mosaic》` 鎖 8 軌形** | `收錄`／`經典錄音`／`原版母帶`／`Special Edition`／`16 軌` **全篇 0 命中**；年份 **2008 命中**（「歐美版要到 2009 年 1 月才上架，日本 2008 年 12 月 26 日就先出了」照鏈上原樣）；**美版 2CD 的第二片一字未提** |
| **4-b `Passin' Thru` 年份 2017、非 Montreux 單場** | **2017 命中**；**Montreux 與 The Lensic 兩地並寫**（「七軌剪自 2016 年夏天的兩場演出：Montreux 一軌，聖塔菲的 The Lensic 六軌」） |
| **4-c `We Are the Drum` 高中寫中文校名** | **`HSPVA` 與 `High School for` 0 命中**；定稿逐字「當地的表演與視覺藝術高中」（第 3263 條那處改寫照抄） |
| **4-d 三處來源打架不寫** | Truffaz：`隱藏`／`靜音`／`5:05`／`11:08` **0 命中**；Charles Lloyd：`Shiva` **0 命中**；James Carter：`印錯`／`11:41`／`08:31`／`7:12`／`6:27` **0 命中** |
| **4-e 兩筆查不到不寫** | Blue Note 7：**全篇沒有錄音地點或錄音日期**（`Bennett`／`錄音` 皆 0 命中）；Jon Cowherd：**全篇沒有錄音日期**（`12 月`／`2012` 0 命中，**連月日都沒寫**） |
| **4-f `Mercy` 不是純 ArtistShare 盤** | 定稿逐字「目錄號同時掛 ArtistShare 與 Blue Note 兩個廠牌」 |
| **5. 序數句只剩 2 次** | **實掃 `第一張|第一次|首作|首張|第一位` 共 4 處**：`Ruben Hein` 的「首張碟」與 `Erik Truffaz` 的「第一次把樂團名放上封面」**都在 hook 原文裡、一字不可改**；`Kenny Barron` 的「第一張領班作」是第 3265 條指定的那一張；`Northern Governors` 的「這個家族第一次合奏」**是鏈上原句、講的是家族首次同台而非領班首作，不同範圍**。**第 3265 條點名要壓到 0 的四張（Mercy／The Thought of You／We Are the Drum／Live From Newport Jazz）實測全部 0。** |
| **6. 兩處「同名不同物」不從 `facts` 補** | **`Dream Weaver` 只出現在 `Passin' Thru` 一張**（`Shift` 那張只寫「多收兩首」，**曲名 0 命中**）；**`Nicholas Payton` 全 20 張 0 命中** |

---

## 第 3485 條（b 組寫作，**三支自檢的輸出與 `互指?` 逐筆判讀**）

```
cd desc-tools && node qa-batch.mjs out c169
  → out-1｜20 張｜字數 193–238｜>260: 0
  → out-2｜20 張｜字數 199–238｜>260: 0
  → out 合計 40 張，與卡單相符 ✓
  → 全部通過 ✓            （總標記 0、互指 0 處）

cd desc-tools && node qa-check-research.mjs batches/input/c169-writer-2.json batches/output/c169-out-2.json
  → QA 完成｜20 張｜標記 0

cd desc-tools && node fix-spacing.mjs batches/output/c169-out-2.json --field desc
  → 待補 0 張（欄位 desc）
```

- **`互指? 0 處`**（第 1763-B 條逐筆人工判讀）。**本組四處刻意檢查過的同批鄰居，全部確認取自本卡自己的素材**：
  1. **`Robert Glasper`** 在 `We Are the Drum` 的首句——**那是 hook 原文**，來自本卡 `facts[0]` 的校友名單，不是從 `Collagically Speaking` 搬來的。
  2. **`Kendrick Scott`** 在 `Our Point of View` 的定稿裡 **0 命中**（六位團員整格未寫，照第 3267 條）。
  3. **兩張 Musica Nuda** 的鏈零重疊：`Complici` 走「外找作者寫新歌」、`Banda larga` 走「十週年＋管弦樂團」，
     **只有「Petra Magoni 加 Ferruccio Spinetti 的二重奏」這個編制說明兩張都有——那是識別這張碟所必需的**（第 3267 條第 5 列已定性）。
  4. **`Erik Truffaz`** 在 `Complici` **0 命中**、**Musica Nuda 在 Truffaz 那張 0 命中**（第 3267 條第 4 列）。
- ⚠ **`fix-spacing` 的 `--write` 本層未使用**：dry-run 逐張回報 **待補 0 張**，**沒有任何一處要補空格，跑 `--write` 會是空操作**。
  **第 1740-B 條（預設 dry-run、要改檔必須加 `--write`）仍然成立，只是本組沒有觸發條件。**
- ⚠ **派工信第二節預告的「40 vs 20」沒有出現**：**本層開跑前 writer-1 已交件**，`c169-out-1.json` 20 張在位，
  `qa-batch out c169` 直接就是 40／40 相符。**那句預告對本組不適用，不是標記被掩蓋。**
- **字元自檢（程式，不用眼睛掃）**：`qa-batch` 的 `charScan('out-2')` 四道（非拉丁亂碼／簡體字／半形逗號貼中文／千分位逗號）**全部 0**。
  本層另掃：`全片`／`全曲` **0**；base 第 168 行禁語 **0**（`qa-check-research` 的 `BANNED` 同時覆蓋）；
  樂評媒體名與樂評人姓名 **0**（`AllMusic`／`滾石`／`Billboard`／`DownBeat`／`衛報`／`The New Yorker`／`OffBeat` 全 0）；
  **`入圍`／`提名` 全 20 張 0 命中**——第 3266 條把五張的「入圍而非得獎」整格捨在鉤子層，寫作層自然拿不到，**這道無需本層處置。**
  ⚠ **日文假名與變音符照原文保留、未做任何折換**：`レコード番長`／`須永辰緒の夜ジャズ`／`〈愛のバラード〉`／`《犬神家の一族》`／
  `須永辰緒`／`大野雄二`／`Malmö`／`Prättälä`／`Beyoncé`／`Benoît`／`Rytmihyrrä`；
  **`Bruce’s Vibe` 與 `You’re Still the One` 的 U+2019 照鉤子層原樣**（第 3269 條）。
- **schema 自檢**：20 筆、`[{key, desc}]` 兩欄、**key 與輸入檔逐字全等且順序相同**（`JSON.stringify` 比對 `true`）、
  重複 key 0、**首句＝hook 原文 20／20**（`qa-check-research` 的 `hook 未原封開頭` 硬檢查亦 0）。

---

## 第 3486 條（b 組寫作，**第 3481 條要求的逐張字數；派工信第六節指定要寫進回報的那組數字**）

**min 199（`The Blue Note 7《Mosaic》`）／max 238（`Otis Brown III《The Thought of You》`）／中位數 223。**
**撞破 180 下限 0 張、超過 240 上限 0 張。**

分佈：**`<180` 0 張／`180–199` 1 張／`200–219` 6 張／`220–240` 13 張／`>240` 0 張。**

⚠ **「撞下限」這個詞在本組要先定義才有意義**：若照派工信引 c-170 的用法（實際低於 180）是 **0 張**；
若放寬成「距下限 5 字以內（≤185）」也是 **0 張**；**最接近下限的一張是 199，離下限還有 19 字。**
**→ 本組與 c-170 那兩張（175／167）不是同一個形狀，補一格 `sound` 就夠的只有 `R+R=NOW` 一張。**

---

## 第 3487 條（b 組寫作，**⚠ 派工信與 base 檔／既有裁定牴觸之處**）

派工信第一節逐字要求「本信若與它牴觸，以它為準，並在交件回報裡指出本信哪一句寫錯了」。
**本棒查到一處真牴觸、三處與實況不符。**

### （一）⚠ 真牴觸：第三節第 2 點的「正文只寫上列各項」被派工信自己解成排除條款，但**同一節第 1 點又要本層在太短時「補回 `note` 鏈上已有的內容或 `sound` 欄那一格」**

`note` 末句逐字是「**正文只寫上列各項。**」——**字面上 `sound` 欄不在「上列各項」裡**，
照字面執行，`R+R=NOW` 那張 187 字就補不回來、只能停在下限邊緣。

**→ 以派工信第三節第 2 點的定性為準**（逐字：「『正文只寫上列各項』是排除條款，不是必寫清單」），
**並與 a 組第 3444 條併讀**（`sound` 欄可取，取的不是鉤子層砍掉的 `facts` 格）。
**本層據此補了 2 格 `sound`**（第 3482 條），**兩格都不是 `facts`。**
⚠ **這與第 1795-B／3049／3270 條同一族**：**把 `note` 裡寫給鉤子層的指令句當成寫作層的獨立條件，兩句就會互相打架。**

### （二）與實況不符、不算牴觸的三句

1. ⚠ **第一節逐字「鉤子層整格砍完之後 `note` 鏈上只剩 3–4 格，照 base 的預算表列必然『高估』⋯不要預留上浮」
   ——本組實測方向相反的有 3 張**（`Ruben Hein` 246、`Jon Cowherd` 251、`Kendrick Scott` 243 初稿全超標）。
   **本組 `note` 的鏈實測是 3–5 格不是 3–4 格**（`STE` 與 `Shift` 各 5 格、`Mosaic` 4 格），
   **鏈越長越會超標，與 c-170 的形狀不同。** 派工信引 c-170「兩組初稿都是零超標、各有一張撞破下限」作為預告，
   **在本組只對了 1／20 張**（第 3481／3486 條）。**這正是 base 第 117 行「不要再從卡的組成去預測偏差方向」要擋的東西。**
2. **第二節逐字「`qa-batch out c169` 會比對卡單總數，writer-1 未交件時會報『40 vs 20』」——本組沒有觸發**，
   開跑時 `c169-out-1.json` 已在位，首跑就是 40／40 相符（第 3485 條）。**這句是對的預防說明，只是條件沒發生。**
3. **第四節第 2 點逐字要求跑 `fix-spacing ... --write` 才會改檔——本組 dry-run 即 `待補 0 張`，`--write` 無事可做**，
   故未執行。**第 1740-B 條未被違反，只是沒有觸發條件**（第 3485 條）。

---

## 第 3488 條（b 組寫作，**第 1803-B 條的交件聲明**）：**工作區當下的 `desc-tools/batches/output/c169-out-2.json` 才是交件版**

⚠ **「筆數對了」不等於「定稿了」**（第 1803-B 條，本線已十次）。
**本棒的交件版就是工作區當下的那一份**：20 筆、字數 199–238／中位 223／超標 0／撞破下限 0、
`qa-batch out c169` 全部通過（總標記 0、`互指? 0 處`、40／40 與卡單相符）、
`qa-check-research` 標記 0、`fix-spacing` 待補 0。

**寫檔是分四次落地的**（5／10／15／20 張，**每一次都把完整陣列整份寫回**，因此任何一個中途版本都是合法 JSON）；
**筆數到 20 之後又整份改過三輪**（超標三格捨去、下限一格補回、第 3483 條的年份句式改寫 12 張、三處行文微調），
**任何一個 20 筆的中途版本都不是定稿。**
**若容器重啟後看到的檔案筆數少於 20，接續補完即可，不要從頭重寫。**

本層**未動** `seed_cards.json`／`apex_pool.json`／`PROJECT_MEMORY.md`／KV／Firestore／
`c169-out-1.json`／`c169-hooks-a.json`／`c169-hooks-b.json`／其他批次的檔案，**未 `git commit`、未 `git push`、未動 git 索引**。
暫存檔全部落在 scratchpad 且帶 `c169w2-` 前綴。

---
## 第 3489 條（b 組寫作，**⚠ 第 1803-B 條在本批當場應驗一次，而且是實據不是推論**）：**主線把本層的中途版本 commit 進去了**

**收工前跑 `git status --short` 時發現 `desc-tools/batches/output/c169-out-2.json` 顯示的是 `M`（已修改）而不是 `??`（未追蹤）
——代表它已經在 HEAD 裡。** 回查：**`30a425c`「checkpoint: c171 鉤子 b／c169 寫作 2 中途存檔」把本層寫到一半的檔案收進了提交。**

**那個 commit 裡的版本是 20 筆、字數 199–238、三支自檢也會全過——形狀上完全看不出它不是定稿。**
**但它是第 3483 條年份句式改寫之後、最後三處行文微調之前的版本**，與工作區當下版本差 3 張
（`Rytmihyrrä / Rytmyra` 把句末那句孤立的「2011 年發行。」併回句首、
`The Thought of You` 的「曲名獻給的那個人」收成「曲名獻給的人」、
`We Are the Drum` 的「2015 年交出的十一軌裡」改成「碟 2015 年出，十一軌裡」）。

⚠ **這正是第 1803-B 條講的那個失效模式的實例**：**「筆數對了、自檢全過」三個條件同時成立，版本卻仍然不是定稿。**
**中途存檔的 commit 本身沒有錯**（容器會不定時重啟，存檔是對的），
**錯的是把它當成交件版**。**→ 給主線：請以工作區當下的 `c169-out-2.json` 重新提交一次，不要沿用 `30a425c` 裡的那一份。**

**本層未動 git 索引、未 commit、未 push**（第 3488 條），上面的差異是 `git diff` 讀出來的，不是本層造成的。
