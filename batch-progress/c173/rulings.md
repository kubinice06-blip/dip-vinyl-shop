...
# 寫作層裁定（c-173，11 張，a 組 6 ＋ b 組 5）

批次 c-173｜寫作層｜2026-09-21｜編號區間 **4031–4060**（本節只 append，3945 以前一行未動）
產出：`desc-tools/batches/output/c173-out-1.json`（6 筆）與 `c173-out-2.json`（5 筆）。

## 4031　先倒回去量舊批，確認同尺才動筆

派工信給的倒量參考逐格重現（`Array.from(desc).length`）：

| 檔案 | 派工信 | 本層量到 |
|---|---|---|
| `c172-out-1.json`（3 張） | 234／239／232 | **234／239／232** ✔ |
| `c171-out-1.json`（10 張） | min 199／max 238／中位 218 | **min 199／max 238／中位 218** ✔ |

確認同尺後才量自己的稿。量測腳本在 scratchpad 的 `c173w/c173w-measure.mjs`（逐句斷行印字元數）。

## 4032　**逐張字數全表（定稿）**：11 張 **217–236**，**無一張低於 180、無一張高於 240**

| 組 | 卡 | 初稿 | 伸縮格怎麼動 | **定稿** |
|---|---|---:|---|---:|
| a | The Original Big Four | 261 | **整格砍掉 sound**（−37） | **224** |
| a | Modern Jazz Screen Mood | 270 | **整格砍掉 sound**（−52），再依 4035 改寫發行句 | **217** |
| a | Modern Jazz Show Case | 252 | **整格砍掉 sound**（−33） | **219** |
| a | 真夜中のラテン | 251 | **整格砍掉 sound**（−31），再依 4035 改寫兩處 | **220** |
| a | Harlem Nocturne | 258 | **整格砍掉 sound**（−34） | **224** |
| a | Charlie Mariano & Sadao Watanabe | 302 | **整格砍掉 sound**（−57）後仍 245，再緊縮兩個連接語 | **236** |
| b | Jazz & Bossa | 262 | **整格砍掉 sound**（−27），再依 4035 改寫錄音句 | **231** |
| b | Iberian Waltz | 250 | **整格砍掉 sound**（−14） | **236** |
| b | Operation Sam Taylor | 232 | **sound 保留**（一句，23 字），依 4035 改寫錄音句 | **229** |
| b | This Is Jazz-Rock | 226 | **sound 保留**（一句，15 字），依 4035 改寫發行句 | **229** |
| b | Megalopolis | 223 | **sound 保留**（一句，28 字） | **225** |

**min 217／max 236／中位 225。8 張整格砍掉 sound、3 張保留 sound。**

## 4033　⚠ **本批是單向高估，幅度 11–62 字元**——但這條不可繼承

初稿 11 張**全部超過 240**（251–302），**無一張掉到 180 以下**。手算預算表與實測的落差：
最小 11（Operation Sam Taylor 232 vs 估 203）、最大 62（Charlie Mariano & Sadao Watanabe 302 vs 估 226）。

成因與 `writer-base.md` 講的完全一致，本批的三個吃字大戶逐一記下來：

1. **廠牌＋目錄號的複合專名**：`Victor World Group SMJ-7446` 28、`Catalyst Records 的 Jazz From Japan 系列` 33、
   `Columbia ZS-1015` 16＋`ZL-1160` 8、`Kiyoshi Yamaya & His All Stars` 29。**單張塞兩個就是一整格。**
2. **片假名的廠牌名並列**：真夜中のラテン 那句「キングレコード、ビクターレコード、ポリドール・レコード 與日本コロムビア」
   **一句 41 字元**，是全批最貴的一句（但它是 note 指派的主故事，不能捨）。
3. **日本人名後面的半形空格**：本線行文慣例是日文專名後補一個空格（`鈴木宏昌 彈電鋼琴`，c-133 起的既有寫法），
   11 張合計約 60 個這種空格，**等於白送 60 字元**。⚠ **這一項在手算時完全看不見。**

⚠ **照 w2-118 那條，本層不把「日本爵士批次會高估」寫成可繼承的係數**——
c-173 只是又一次證明「**唯一可靠的是算完要同時對照 240 與 180 兩端**」。下一批仍要兩端都量。

## 4034　⚠ **首句照抄 hook 與第二句不得複述的對撞：11 張逐張怎麼解**

**有代稱的 6 張**（第二句＝替代稱具名）：

| 卡 | hook 的代稱 | 第二句具名成 |
|---|---|---|
| The Original Big Four | 「那四個人」「貝斯手」「他」 | 小野満 ＋ 團名ビッグ・フォア |
| Modern Jazz Show Case | 「兩個不同的人」 | 瀬川昌久（立體聲）／野口久光（單聲道） |
| Charlie Mariano & Sadao Watanabe | 「兩支中音薩克斯風」「同一組人」 | Charlie Mariano 與 渡辺貞夫 |
| Iberian Waltz | 「一個波士頓來的中音薩克斯風手」 | Charlie Mariano ＋ 曲名〈Stone Garden of Ryoanji〉 |
| Operation Sam Taylor | 「左聲道一支次中音，右聲道一支次中音」 | 左＝松本英彦／右＝宮沢昭 |
| Megalopolis | 「鼓手」「鋼琴手」 | 日野元彦／市川秀男（順帶帶出領銜的松本浩） |

**通篇沒有代稱的 5 張**（依第 1847-B 條，第二句職責一般化為「補上 hook 沒有指明的那個位置」）：

| 卡 | hook 沒有指明的是 | 第二句補了什麼 |
|---|---|---|
| Modern Jazz Screen Mood | 那十五個樂手是誰、這是誰家的碟 | 廠牌日本コロムビア ＋ 1960 年 8 月 ＋ SL-1001 ＋ 四位管樂席具名 |
| 真夜中のラテン | 「名單到第四個」的那四個名字 | 横内章次／八城一夫／宮沢昭／福原彰 與各自的樂器 |
| Harlem Nocturne | 吹尺八的人是誰、碟是誰出的 | 村岡実 ＋ 1967 年 11 月 1 日 ＋ 日本コロムビア JPS-5132 |
| Jazz & Bossa | 這是哪一張碟 | 目錄號 JAZZ-1＝Takt 爵士系列第一號 ＋ 発売元タクト電機 |
| This Is Jazz-Rock | 是誰把那兩首改成這樣 | 編曲指揮 前田憲男 ＋ 1968 年 9 月 10 日 ＋ 日本コロムビア YS-10036-J |

**11 張逐張回頭讀過頭兩句**（第二句一律不複述 hook 的句意，只做具名或補位）。
⚠ 這一項**沒有任何腳本會抓**，上表即人工檢查結果本身。

## 4035　跨張中文 4-gram 自查：**初稿撞了 5 條句型，改寫後只剩專名與樂器名**

做法：把 11 張 `desc` 只留漢字（U+3400–U+9FFF，等於把年份、目錄號、拉丁專名全部剝掉），
逐張取相異 4-gram，統計出現在幾張卡上。腳本在 scratchpad 的 `c173w/c173w-ngram.mjs`。

**初稿的 3 張以上命中有 9 條**，其中 **5 條是句型同構、非專名**：
`發行編號`（4 張）、`由日本發`／`日本發行`（3 張）、`在年月日`（3 張）、`錄音在年`／`音在年月`（3 張）、`的次中音`（3 張）。

**逐條改寫**（每條只動到剩 2 張以下）：

| 撞到的句型 | 原本幾張 | 改寫的卡與改法 |
|---|---:|---|
| `發行編號` | 4 | 真夜中のラテン 與 Megalopolis 改成「出在⋯，目錄號⋯」 |
| `由日本發`／`日本發行` | 3 | Modern Jazz Screen Mood 改成「碟由日本コロムビア 在⋯推出，編號⋯」 |
| `錄音在年` | 3 | Jazz & Bossa 刪掉句首「錄音在」，Operation Sam Taylor 改成「⋯日錄於⋯」 |
| `在年月日` | 3 | 同上（Operation Sam Taylor 一併解決） |
| `的次中音` | 3 | 真夜中のラテン 的樂器列改成平行結構「分別是木吉他、鋼琴、次中音與小號」 |
| （另補）`年月日由日本發行編號十二` 12 字連續同構 | 2 | This Is Jazz-Rock 改成「唱片⋯日發行，廠牌是日本コロムビア，編號⋯」 |

**定稿結果：出現在 3 張以上的中文 4-gram 只剩 2 條——`渡辺貞夫`（專名）與 `薩克斯風`（樂器名）**，
兩條都是派工信第六點允許的類別。出現在 2 張的有 11 條，逐條看過全部是專名、樂器名或三四字的短搭配
（`了錄音室`／`進錄音室`／`是宮沢昭`／`宮沢昭與`／`碟年出在`／`在年月日`／`編號十二`／
`中音薩克`／`音薩克斯`／`這張碟的`／`的次中音`），**無句型同構**。
⚠ **鉤子層自查後剩下的那 2 次重複（`中音薩克`／`音薩克斯`／`薩克斯風`）本層沒有再疊上去**——
定稿的 `薩克斯風` 3 張裡有 2 張是 hook 原文帶進來的，本層自己只在 Operation Sam Taylor 的 sound 句用了一次。

## 4036　本批四個特殊風險的執行結果（逐項）

1. **〈Harlem Nocturne〉橫跨三張**：
   - 《Operation Sam Taylor》**寫了**（`〈Harlem Nocturne〉` 出現一次，是 Sam Taylor 的成名曲）；
   - 《村岡実 Harlem Nocturne》**整首不提**，改寫「八支尺八 26 到 74 公分」與「十二軌對半分給兩組伴奏」；
   - 《真夜中のラテン》**整首讓出**，一個字都沒有。
   - ⚠ **Earle Hagen 與 1939 三張全部沒有寫**（程式逐張 `/Earle Hagen/` 與 `/1939/` 掃過，0 命中）——
     照第 3934 條整格捨去，本層沒有因為在別張的 facts 看過就撿回來。
2. **連續兩天雙錄音**：《Charlie Mariano & Sadao Watanabe》寫 6/27 ＋ Victor Studio ＋ SMJ-7446，
   接「隔天那一場換了錄音室也換了廠牌，曲數與走向都不同」，**未點對方盤名與廠牌**；
   《Iberian Waltz》**只寫 6 月 28 日**，**沒有「一天錄完」**（程式掃過：`一天錄完` 全批只命中
   《Charlie Mariano & Sadao Watanabe》一張，那是它自己 `note` 的逐字指派，正確）。
3. **五位樂評一人一張，零混用**：久保田二郎→《The Original Big Four》、油井正一→《Modern Jazz Screen Mood》、
   瀬川昌久＋野口久光→《Modern Jazz Show Case》（同卡，那正是該卡骨架）、岩浪洋三→《Megalopolis》。
   **其餘 7 張一個樂評名都沒有。**
4. **同姓不同人／同名不同團**：村岡実 那張以「尺八」開場並貫穿全文，樂器自帶區隔；
   《This Is Jazz-Rock》逐字寫「稲垣次郎 的ソウル・メディア 要到隔年才成立」。
5. **5 張無串流、2 張無封面**：11 張**沒有任何一句寫成「可以去聽」那種話**（逐句確認，另程式掃 `可以去聽` 0 命中）。

## 4037　⚠ **與上游條件牴觸一處：`writer-base.md` 禁樂評姓名 vs `note` 指派了五位樂評**

`writer-base.md`「來源平台與樂評姓名不進正文」那節逐字寫「**不得把樂評媒體名⋯或樂評人姓名寫進 desc**」，
**但本批有 4 張的 `note` 正面指派了樂評姓名**（久保田二郎／油井正一／瀬川昌久＋野口久光／岩浪洋三），
派工信第四節第 3 點也逐字要求「五位樂評一人一張，零混用」。

**本層的處置：照 `note` 寫，四張都寫了。** 判準三條：

1. **這四筆是唱片本身的工作人員掛名**（監修欄、內頁署名、原盤封套解說），
   與該節要擋的「拿樂評當評價背書、把 `src` 寫給讀者看」是兩回事——
   本批沒有任何一張引用他們說了什麼，**零評語、零分數、零星等**（程式掃過）。
2. **《Modern Jazz Show Case》整張卡的骨架就是「兩個壓次掛兩個不同的內頁作者」**（第 3933／3940 條），
   拿掉姓名等於整張卡沒有故事——這正是該節「例外二」的判準（**樂評整條拿掉之後還剩不剩得下一個故事**）。
3. 規則層級上，`note` 的正面指派句是鉤子層已經結算過預算的施工圖，
   而該節禁的是**寫作層自己去搬研究稿的 `src`**。本層沒有自己新增任何一位。

**同一處另記一筆**：《The Original Big Four》寫了媒體名 **《スイングジャーナル》**。
理由同「例外一」——**那本雜誌的人氣投票直接造出了這支樂團**，它是主故事的一部分，不是評價背書。

⚠ **這一條留給主線裁定要不要回寫 base**：若要禁，受影響的是 4 張卡與第 3940 條的整個分配表；
本層判斷是 base 的措辭需要補一句「**唱片本身的掛名（監修／內頁／封套解說）不受此禁**」，
因為日本 1950–60 年代盤的內頁與監修欄**本來就是由樂評擔任**，一律禁等於這條線每張卡少一格。

## 4038　其他上游條件的遵守情形（逐項自述）

- **只寫 `note` 裡列出的東西**：11 張逐張比對過，**沒有補任何常識、年份、曲風或評價**。
  `qa-check-research` 的零編造比對（曲名／西文專名／年份三項都要在事實表 blob 裡）**兩組各 0 標記**，即機器旁證。
- **note 的警示不進正文**：`不得`／`禁寫`／`整格不寫`／`uncertain`／`查無` 逐字掃過正文，**0 命中**；
  **也沒有寫任何否定句**（《This Is Jazz-Rock》照 `note` 的正面版本寫「十二軌裡有九首認得出原曲出處」，
  沒有寫成「全翻唱」也沒有寫成「不是全翻唱」）。
- **不得把鉤子層砍掉的格子撿回來**：第 3936 條那 11 格逐條對照過，**一格都沒有復活**
  （久保田二郎 的總編輯經歷、瀬川昌久 退休年、東京尺八三重奏團、逐軌錄音日、五人編制逐項、
  曲單全列、〈God Has Mercy〉、〈Serenade to Dimly-Lit Street〉、Earle Hagen 那格等，全部未寫）。
- **《Charlie Mariano & Sadao Watanabe》是同名盤**（第 3852 條第 6 點）：正文**沒有使用「某某與某某的某張碟」句型**，
  以「那兩支中音是⋯」承接 hook，盤名沒有被寫成描述句。
- **反向禁令第二類**：富樫雅彦 1970 年受傷、ジョージ川口 的「ほら吹き」虛言（驅逐艦／芝公園／Buddy Rich）、
  四位樂手的辭世——**11 張全部未寫**。
- **禁語表**（傑作／必聽／里程碑／獨樹一格／融合多種元素／具有代表性／層次豐富／全片／全曲／你／我）
  逐張掃過 **0 命中**；半形逗號貼中日文 **0 處**；千分位 **0 處**；簡體字 **0 處**
  （本層**未新增**任何字串到 `desc-tools/jp-proper-names.json`——正文沒有用到 `国`／`会` 同形字，
  `稲葉国光` 本層未寫進正文）。
- **年月日與名次用阿拉伯數字**：全部照辦（1959／1960 年 8 月／1967 年 11 月 1 日／1968 年 9 月 10 日…）；
  **概略數量與年齡維持中文數字**（二十七歲、二十三歲、二十四歲、十一首、十二軌、七首）。

## 4039　三支腳本的逐字結果

```
$ node qa-batch.mjs out c173
out-1｜6 張｜字數 217–236｜>260: 0
out-2｜5 張｜字數 225–236｜>260: 0
out 合計 11 張，與卡單相符 ✓
...
```
$ node qa-check-research.mjs batches/input/c173-writer-1.json batches/output/c173-out-1.json
QA 完成｜ 6 張｜標記 0

$ node qa-check-research.mjs batches/input/c173-writer-2.json batches/output/c173-out-2.json
QA 完成｜ 5 張｜標記 0
```

```
$ node fix-spacing.mjs batches/output/c173-out-1.json --field desc
待補 0 張（欄位 desc）

$ node fix-spacing.mjs batches/output/c173-out-2.json --field desc
待補 0 張（欄位 desc）
```

⚠ **`fix-spacing` 兩檔都是 0，因此`--write` 一次都沒有跑**（照派工信：先 dry-run、確認無誤才加 `--write`；
既然要改的是 0 張，跑 `--write` 沒有意義）。
⚠ **`qa-batch out` 的 `互指?` 一行完全沒有出現**——11 張正文都沒有點到同批別張的盤名
（《Iberian Waltz》與《Charlie Mariano & Sadao Watanabe》互不點名，照第 3935 條）。

## 4040　⚠ **三支腳本驗不到、只有人工能抓的四項**（本層逐張自驗的清單）

第 1834-B／1847-B 條的同一個道理，本批具體是這四項：

1. **180 下限**：`qa-batch out` 只檢查 >260、`qa-check-research` 只檢查 <80 或 >280，
   **180 以下無人會抓**。本層逐張量在第 4032 條，**最小 217，距下限 37 字元**。
2. **第二句不複述 hook**：無腳本。逐張人工檢查表在第 4034 條。
3. **跨張中文 4-gram**：`chk-hook-crossgroup` 只看 hook／note 且看不到「同句型 3 次以上」，
   本層自己寫腳本掃 `desc`，結果在第 4035 條。
4. **note 的警示有沒有被寫成正文**：`qa-check-research` 的零編造比對是**反方向**的
   （它擋的是編造，不擋「把禁寫指示照抄進正文」，因為那些字串本來就在 blob 裡、必然通過）。
   本層另掃禁令字串，見第 4038 條。

## 4041　交付與邊界（自述）

- **只動了三個檔**：`desc-tools/batches/output/c173-out-1.json`（新增，6 筆）、
  `desc-tools/batches/output/c173-out-2.json`（新增，5 筆）、本檔（**append**，3945 以前一行未動）。
- **欄位只有 `key` 與 `desc` 兩個**（程式逐筆 assert 過）；**key 逐字複製自 `batches/input/c173-writer-N.json`、順序與張數不變**
  （程式比對 `[x.key for x in input] == [x.key for x in out]`，兩組皆 True）；
  **首句與 hook 逐字相符 11/11**（程式 `desc.startsWith(hook)` 逐筆驗過，未做任何標點更動）。
- **未碰**：`seed_cards.json`、`apex_pool.json`、`PROJECT_MEMORY.md`、`previews.json`、`caa.json`、
  `desc-tools/jp-proper-names.json`（**本層一個字串都沒加，因為沒有誤報**）、
  `desc-tools/batches/cards/*`、`batches/research/*`、`batches/hooks/*`（皆唯讀或未讀）、
  其他批次的任何檔案、KV、Firestore、三支 QA 腳本（只執行，未修改）。
- **未執行任何 git 指令**（不 `add`／不 `commit`／不 `push`／未動索引）。
- 中間檔全部在 scratchpad 的 `c173w/`，檔名帶 `c173w-` 前綴：
  `c173w-measure.mjs`（逐句字元量測）、`c173w-ngram.mjs`（跨張 4-gram）、
  `c173w-draft1.json`／`c173w-draft2.json`／`c173w-draft-b.json`（初稿與中間稿）。
- 續跑保護：a 組定稿後先寫檔才處理 b 組，兩檔各自獨立可讀；本層未遇容器重啟。

## 4042　**編號區間結算**：本節用到 **4031–4042**（共 12 條），**4043–4060 未使用**，留給後續層。
