...
## 附錄二（2026-09-19，雲端線實測）：⚠ 三條查證路徑的更正

**這一節由 c-168／c-169／c-170／c-171 四批的實測結果加註，適用於雲端工作階段。**

1. ⚠ ⚠ **`allmusic.com` 與 `allaboutjazz.com` 在雲端一律 HTTP 403（Cloudflare）。**
   c-168 試四次、c-169 a 試四次，**一條事實都沒取到**。**不要再把這兩站排進查證路徑**；
   要它們的樂評內容，改從英文維基的引用段落取。
2. ⚠ **`itunes.apple.com/search` 在雲端會回 HTTP 403**（c-171 a 整批的三種店面查法都做不了）。
   **`/lookup` 通，但需要先有 collectionId。** **查不到不可寫成「不在架上」。**
3. **`api.discogs.com/releases/<id>` 的整筆逐軌 credits 要列為固定動作。**
   **`search` 摘要的 `extraartists` 只回前四筆**；c-169 a 實測 **13／20 張的主故事只存在於整筆的
   `extraartists` 或逐軌 `written-by`**，該批九處推翻策展層有六處出在這兩欄。
   ⚠ **`Discogs` 的 `tracklist` 會夾標題列**（`position` 為空），**數軌數前要先濾掉**。

### 來源結構隨年代翻轉（四批實測）

| 路徑 | 1985–1999（c-168） | 2000–2009 歐洲分支（c-169 a） | 2023–2026（c-166／c-167） |
|---|---|---|---|
| Discogs 整筆 | **18／18 絕對主力** | **20／20** | 第六條路徑 |
| Billboard／Cash Box OCR | **11／18** | — | — |
| `bluenote.com` wp-json／專頁 | ⚠ **1／18** | ⚠ **2／20**（**只有美國本部那兩張有專頁**） | **39／45、13／14 的主力** |
| 各國語維基 | 6／18 | 13／20（en 7＋de/fi/it/fr/tr 6） | 只用於獎項覆核 |
| 藝人官網 | — | 2／20（**其中一張是決定性的**） | 0／2（JS 渲染） |

⚠ **`bluenote.com` 有沒有專頁，分界不是年代，是碟的出身地**：美國本部的有、各國分支的沒有。
⚠ **查八〇、九〇年代的廠牌史，關鍵字要用製作人／A&R 的人名，不是藝人名**
（c-168 唯一有用的那篇是製作人 Namekata 的悼念文，一篇餵飽四條線）。

### 店面與封面覆蓋率的預期，要照年代與出身地分開講
...
| c-169 | 2000–2019 歐洲分支為主 | **38/40** | ⚠ **25/40** |
| c-168 | 1985–1999 | ⚠ **11/18** | ⚠ **9/18** |

**→ 串流只有 1985–1999 段明顯偏低；封面則是「非美國本部」與「1985–1999」兩段都偏低。**
⚠ **探測層判 unavailable 的，五批合計 31 筆裡有 20 筆（65%）是假的**——**一律要人工回撈，不可當結論。**
