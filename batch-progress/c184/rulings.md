...
# b 組寫作層（6011–6025）｜2026-09-25

> ⚠ **本節 append 在檔末，編號區間 6011–6025（派工信配的就是這一段；另一位寫作代理用 5996–6010）。**
> **寫入前跑過 `git show HEAD:batch-progress/c184/rulings.md`（`^## 60(1[1-9]|2[0-9])` 命中 0）與 `ls`（檔在、468554 位元組，`^## 60(1[1-9]|2[0-9])` 命中 0），兩者都確認 6011 以後是空的。既有任何一行都沒有動。**

## 6011　開工檢查與倒量：**派工信的檔名與 13 個數字都對，尺一致才動筆**

- **輸入 `c184-writer-2.json` 12 筆**，`key` 逐字同序複製、12/12 全中，`status` 12 筆全 `full`（**本組 0 張 thin**，與派工信第一節相符）。
- 派工信第三節逐字警告「檔名是上一批 c-183 的檔」，實測兩檔存在、筆數 5 與 8。**13 個數字逐格重現，一個都沒有差**：
  `c183-out-1.json` ＝ **218／230／227／237／236**、`c183-out-2.json` ＝ **238／226／232／233／235／239／227／234**（`Array.from(desc).length`）。
- ⚠ **開工時 `c184-out-1.json` 已經存在（a 組已交件）**，所以**本組是「後交件的那一組」**（`writer-base.md` 第 6 點）：**跨兩檔合掃是義務**，而「2 張殘片當 3 張」那條（第 5 點／第 1940-B 條）是先交件方的責任。

## 6012　逐張字數全表（**兩端都驗，不預設方向**）

| # | 掛名 —《盤名》 | 字數 |
|---|---|---:|
| b1 | 青木望 —《1999 A.D》 | **215** |
| b2 | 土取利行・高木元輝 —《Origination》 | **233** |
| b3 | 渡辺貞夫 —《At Pit Inn》 | **233** |
| b4 | 渡辺貞夫 —《Swiss Air》 | **221** |
| b5 | 笠井紀美子 with Cedar Walton Trio —《Kimiko Is Here》 | **225** |
| b6 | 大野俊三 —《Bubbles》 | **232** |
| b7 | 大野俊三 —《Something's Coming》 | **215** |
| b8 | 増尾好秋 —《111 Sullivan Street》 | **232** |
| b9 | 日野皓正 —《Speak To Loneliness》 | **219** |
| b10 | 坂田明トリオ —《Counter Clockwise Trip》 | **225** |
| b11 | 山下洋輔トリオ —《Up-To-Date》 | **226** |
| b12 | 土取利行・坂本龍一 —《Disappointment-Hateruma》 | **208** |

**12 張全部落在 180–240；min 208、max 233、中位 225.5。分佈：208–219 共 4 張、220–229 共 4 張、230–233 共 4 張。**
⚠ **初稿超標 2 張（b5 252、b6 255），一律「整格捨去」壓回**（見 6016），**其餘 10 張初稿就在區間內、0 張掉破 180**
——**與 c-184 鉤子層「全批超標」的方向相反，再次證實 `writer-base.md`「任何係數都不要繼承」。**

## 6013　⚠ ⚠ 第二句：**12 張的 hook 全是代稱或不具名位置開頭，第二句一律去具名，0 張複述**

派工信第二節那條對撞（首句照抄 vs 不得複述）在本組 12 張全部命中，逐張的具名對象：

| # | hook 的代稱／缺口 | 第二句具名的是 |
|---|---|---|
| b1 | 「同一個名字」 | **青木望** |
| b2 | 「兩個人」「兩排樂器」 | **土取利行 與 高木元輝 ＋ 各自的樂器格** |
| b3 | 「新宿一家爵士屋」「四個人」 | **新宿ピット・イン ＋ 掛名欄的一人 ＋ 三位美國樂手** |
| b4 | 「全是日本人的四重奏」 | **作曲欄與編曲欄掛的 渡辺貞夫** |
| b5 | 「那位鋼琴手」「別家公司」 | **Cedar Walton ＋ RCA Records** |
| b6 | 「同一個曲題」「這張碟」 | **那個曲題＝盤名，兩版的長度各是多少** |
| b7 | 「一個字母」 | **那個字母是 M，中的是盤名與首軌曲題** |
| b8 | 「一個門牌號」 | **曼哈頓 SoHo 的 Sullivan Street 111 號** |
| b9 | **無代稱** → 補 hook 沒指明的位置 | **日野皓正 ＋ 錄音日與廠牌與目錄號** |
| b10 | 「一家新廠牌」 | **Frasco ＋ 目錄號尾數的序位** |
| b11 | **無代稱** → 補 hook 沒指明的位置 | **場合（1975-04-28 東京厚生年金会館 的整場實況）＋ 兩枚組** |
| b12 | **無代稱**（「素材清單」是物不是人） | **兩位演奏者（土取利行／坂本龍一）** |

⚠ **這一條沒有腳本會抓**（`qa-check-research` 只驗事實對照）：**12 張各自寫完都回頭讀了一次頭兩句**，**0 張是把 hook 再說一次。**

## 6014　⚠ ⚠ 三欄配表：**繼承鉤子層第 5840 條 b 組那十二列，並自配「日」後一字與「日期前面那一個漢字」**

`writer-base.md` 第 3 點（直接繼承鉤子層那張表）照辦；第三欄（黏合位）是本層自配，**與 a 組第 5999 條那十二個（題／節／會／走／尾／行／它／人／上／花／樂／張）刻意 0 重疊**：

| # | 日期＋動詞／語序 | 「日」後面那一個字 | **黏合位**（日期前一個漢字） | 目錄號引入法 |
|---|---|---|---|---|
| b1 | `年在⋯錄下十一軌，碟就在同年出自` | （無日） | **曲** | 號碼 |
| b2 | `日壓的黑膠` | 日**壓** | **管** | 系列第四號 |
| b3 | `唱片隔年才交給`（日期全在 hook，正文不重寫） | （無日） | — | 號 |
| b4 | `日那場現場同年成了⋯的黑膠` | 日**那** | **辦** | 號 |
| b5 | `錄音落在⋯日，地點⋯，年才輪到⋯發它` | 日**，地點** | **是** | 號 |
| b6 | `年⋯的五軌盤⋯錄音卻早了半年，在年月的紐約完成` | （無日） | **秒**（＋領頭 `這是`） | 號 |
| b7 | `日在紐約錄完，同年進了⋯的目錄` | 日**在** | **了** | 號 |
| b8 | `兩日錄成八軌，碟同年掛⋯問世` | （`兩日`）**錄** | **號** | 號 |
| b9 | `日錄於東京，碟同年在⋯那條線上發出來` | 日**錄** | **正**（人名領句，非句首日期） | 號 |
| b10 | `日到⋯日進⋯的 Union Studio，碟同年發出` | 日**到** | **字** | 號（＋`尾數`） |
| b11 | `日在⋯錄下的一整場實況，唱片同年月由⋯壓成兩枚組` | 日**在** | **鐘**（＋領頭 `那是`） | 號 |
| b12 | `年的黑膠，四軌，字標⋯` | （無日） | — | 系列第七號 |

- ⚠ ⚠ **「日」後面那一個字 7 種、無日的 4 張各走不同領頭名詞、黏合位 10 個互異**，**且避開 c-183 出事的 `與` 與 `場`（第 1964-B 條）**。
- ⚠ **b7 與 b11 的「日在」看似同形，靠後一個字分開**（`日在紐` 對 `日在東`），**合掃確認 0 命中。**
- ⚠ **本層把 b9／b10 的鉤子層同形（兩張都是 `日錄於`）主動拆開**：**b10 改成 `日到⋯日進`**——**鉤子層的 note 允許 2 張，但寫作層是 desc，兩張同形會被下游當成樣板。**
- ⚠ **廠牌與目錄號一律不連寫**：12 張之間最少隔一個全形逗號加一個引入詞，`fix-spacing` 0 報、`qa-batch` 的 `互指?`／`專名?` 0 行。

## 6015　⚠ ⚠ ⚠ **三組「同一位掛名各兩張」的「他」引入法：六張六種，並與 a 組第 6000 條登記的三種 0 重疊**

| 藝人 | 卡 | **引入法** | 實作要點 |
|---|---|---|---|
| **渡辺貞夫** | **b3《At Pit Inn》** | **碟的掛名欄** | 第二句先具名爵士屋，再由「掛名欄只寫他一人」把名字帶進來，樂器接在後面當附屬 |
| | **b4《Swiss Air》** | **作曲欄與編曲欄的軌數歸屬** | 第二句不寫人也不寫樂器，直接寫「作曲欄掛他兩軌、編曲欄掛他三軌」 |
| **大野俊三** | **b6《Bubbles》** | **名字領句 ＋ 赴美的邀約** | 名字放在最後一格的句首，接 1974 年 Art Blakey 的邀約 |
| | **b7《Something's Coming》** | **以伴奏關係的介詞片語** | 「替 X 伴奏的五個人裡只有一位日本人」，名字夾在介詞片語裡 |
| **土取利行** | **b2《Origination》** | **兩人並列具名** | 「兩個人是 A 與 B」，直接把 hook 的「兩個人」點開 |
| | **b12《Disappointment-Hateruma》** | **樂器分工兩格對舉** | 「打擊交給 A，鋼琴與人聲交給 B」 |

⚠ ⚠ **a 組第 6000 條已登記他們那三張用掉「名字直接領句＋生年與出生地」「樂器（中音薩克斯手）」「樂團的主人／早年僱主身分」，並把年齡／綽號／廠牌關係／親屬／攝影集留給 b 組。**
**本層實地執行時發現那份清單在本組不可用**：**b3／b4 的 note 裡沒有年齡、沒有綽號、沒有廠牌關係、沒有親屬、沒有攝影集**
（鉤子層第 5847 條已把 b3 的 `ナベサダ` 與六本攝影集與妹妹弟弟整格捨去，b4 的生平前史照第 5843 條一個字不寫）
——**照派工信「note 沒有的事實就是沒有」，本層改用 note 裡實際有的兩格（掛名欄／作曲與編曲欄）當引入法，兩種都不在 a 組那三種之內，五張五種仍然成立。**
⚠ ⚠ **本層初稿一度把 b3 寫成「⋯的是 渡辺貞夫」，合掃當場報出 `的是渡辺` ×3（b3 ＋ a3 ＋ a9）**——**與 a 組第 6001 條報的 `是渡辺貞` ×3 是同一個機制的第二次發作。**
**本層只改得動自己那張，改成「掛名欄只寫 渡辺貞夫 一人」之後那一條歸零。**
⚠ **登記給 c-185 之後**：**這條同構的根因是「具名」這個動作本身只有兩三種中文句式**（`⋯的是 X`／`X 是⋯`／`⋯掛 X`）
——**同一位掛名跨三張以上時，引入法必須像日期＋動詞那樣在動筆前配成表，而且配的人要看得到全批**（鉤子層或主線），**不能留給兩組寫作層各自躲。**

## 6016　「整格捨去」逐筆（**本層砍掉的東西，只有兩張需要砍**）

| 卡 | 初稿 | 捨去的格 | 定稿 |
|---|---:|---|---:|
| **b5 Kimiko Is Here** | 252 | **整句聲音描述**（歌在前、鋼琴填空、貝斯與鼓壓在中庸偏慢那一格） | **225** |
| **b6 Bubbles** | 255 | **作者欄那一格**（「五軌的作者只有兩個名字，領銜者四軌、鼓手一軌」）——**同時把 大野俊三 的具名移到赴美那一格，引入法因此從「作者欄佔軌」換成「名字領句」** | **232** |

- ⚠ **兩張都是整格捨去，0 張逐句削字**（`writer-base.md` 鐵則）。**b6 砍的是 note g3 的後半**：
  **前半（盤名曲兩版的長度）是 hook 靠著的那一格，照第 5848 條不能碰**——**砍的是同一格裡 hook 沒有靠的那一半。**
- ⚠ **反向的補寫 0 張**：**12 張初稿沒有一張掉破 180**，所以「補主故事後續的第二個事實 → 一筆成績 → 聲音描述」那條補寫順序本批沒有用到。
- **`sound` 欄只在 3 張用到當伸縮格**（b1／b3／b4 各一句），**9 張完全沒有動用**——**鉤子層刪掉的格子一個都沒有撿回來寫。**

## 6017　⚠ ⚠ 跨張 n-gram 自查：**漢字與片假名兩輪都跑、跨兩檔合掃，三輪收斂；定稿 3 張以上只剩專名**

腳本 scratchpad `c184w2/c184w2-ngram.mjs`（沿用派工信指定的 `c174w1/c174w1-ngram.mjs` 骨架，**照派工信第四節把片假名 U+30A0–30FF 另跑一輪**，門檻同時印 3 張與 2 張）。

| 輪 | 做法 | 漢字 3 張以上 | 漢字 2 張 |
|---|---|---:|---:|
| 1 | 12 張自組 ＋ 跨 `c184-out-1.json` 合掃 | **2 條**（`渡辺貞夫` ×6 專名、**`的是渡辺` ×3 真同構**） | 52 |
| 2 | 拆掉 9 處（見下） | **1 條**（純專名） | 23 |
| 3 | 再拆 7 處 | **1 條**（純專名） | **14** |

**第二、三輪拆掉的 16 處（前者本組內、後者跨 a 組）**：
- **本組內（兩張都是我的，可完全消除）**：`五軌裡四／軌裡四軌`（b2 hook 對 b4 正文 → b4 改 `五軌有四軌長過九分鐘`）、`一個名字`（b1 hook 對 b7 → b7 改 `一個人寫完了四軌`）、
  `人聲那一`／`位演奏者`／`是土取利`（b1・b9 對 b12 → b12 改 `打擊交給⋯，鋼琴與人聲交給⋯，盤上就這兩個人`）、`錄完同年`（b1 對 b7 → b1 改 `錄下十一軌，碟就在同年`）、
  `盤面另有／面另有一`（b3 對 b10 → b10 改 `盤上另印了一句致謝`）、`一軌管樂`（b2 對 b10 → b10 改 `管樂三軌，鼓與貝斯各一軌`）、`月日錄於`（b9 對 b10 → b10 改 `日到⋯日進`）。
- **跨 a 組（只改我這邊）**：`的是渡辺／斯的是渡／是渡辺貞／吹中音薩／薩克斯的／克斯的是`（b3 對 a3 → 改掛名欄引入）、`辺貞夫的`（b3 對 a9）、
  `的一號錄／一號錄音／號錄音室`（b1 對 a4 → b1 改 `自家的第一錄音室`）、`年月日的`（b5 對 a2 → b5 改 `日，地點⋯`）、`軌的曲全`（b7 對 a9）、`三軌的曲`（b9 對 a9 → b9 改 `三軌都由他作曲`）、
  `年月與日`（b8 對 a12 → b8 改 `27、28 兩日`）、`八軌同年／軌同年掛`（b8 對 a10 → b8 改 `碟同年掛⋯問世`）、`與合成器／合成器與`（b12 對 a5 與 a4 → b12 改成頓號列舉 `合成器、人聲`）、
  `實況同年／年月日東／月日東京`（b11 對 a9 → b11 改 `日在⋯錄下的一整場實況，唱片同年 7 月⋯`）、`一首曲子`（b11 對 a8 → b11 改 `同一曲的兩個版本`）、`那家廠牌`（b10 對 a7 → b10 改 `新廠牌的名字是`）、`自己寫的`（b3 對 a6 → b3 改 `出自當晚那位貝斯手`）。
...
卡數 24
=== 漢字：4-gram 出現在 3 張以上： 1 條 ===
渡辺貞夫  ×6  [At Pit Inn / Swiss Air / Sadao Watanabe / Kenya Ya Africa / フレンズ / Mbali Africa]
=== 漢字（2 張，先交件方當 3 張處理）：4-gram 出現在 2 張以上： 14 條 ===
=== 片假名：4-gram 出現在 3 張以上： 0 條 ===
=== 片假名（2 張）：4-gram 出現在 2 張以上： 8 條 ===
```

- ⚠ ⚠ **3 張以上唯一那一條是純專名 `渡辺貞夫`（6 張）**，照 `hook-base.md`／`writer-base.md`「專名與樂器名不算同構」保留。
- **漢字 2 張的 14 條逐條看過**：**專名 8 條**（`渡辺貞夫`／`土取利行`／`大野俊三`／`鈴木良雄`／`佐藤允彦`／`山本邦山`／`郵便貯金`／`便貯金的`）、
  **樂器名 5 條**（`中音薩克`／`音薩克斯`／`薩克斯風`／`的薩克斯`／`薩克斯手`）、**剩 1 條見 6018。**
- **片假名 2 張的 8 條全部是兩個專名自己的殘片**：`クラウン`／`ラウンレ`／`ウンレコ`／`ンレコー`／`レコード`（b1 與 b11 共用 `クラウンレコード`）、
  `ピット・`／`ット・イ`／`ト・イン`（b3 與 b5 共用 `新宿ピット・イン`）——**與鉤子層第 5841 條的殘片組成逐格相同。**
- ⚠ **本層定稿時 a 組的 12 張已定稿**，但照 `writer-base.md` 第 6 點，**若 a 組之後又動過，合掃結果會再變——請主線在兩組都定稿後統一再跑一次當收件檢查。**

## 6018　⚠ ⚠ **本層改不動的一條：`的音樂節` ×2，兩端一個是我的 hook 原文、一個是 a 組的正文**

**`的音樂節` ×2 ＝ b4《Swiss Air》hook 的 `在瑞士的音樂節上吹馬賽草原。` 對 a7《Inspiration & Power 14》的正文。**
- **寫作層無權改 hook**（`writer-base.md` 第 7 點），而**這一條在我這張上只存在於 hook**：**b4 的正文寫的是 `那個音樂節自 1967 年起⋯`（`個音樂節`），不是 `的音樂節`。**
- **所以第 7 點那條「改的是你自己那張的正文」在這一條上無處可改**——**本層維持，並登記給主線**：**它只有 2 張、另一端是 a 組正文，若要動就是動 a7 的正文。**
- ⚠ **記一個新形狀給 c-185 之後**：**hook 之間鉤子層已經掃過（第 5842 條，門檻 2 張 0 條），但「hook 對別張的正文」那一組沒有人掃**
  ——**它落在鉤子層（只看 hook）與寫作層（改不動 hook）中間的縫裡。建議鉤子層那一輪把 hook 對全批 note 也掃一次。**

## 6019　⚠ ⚠ 三組雙張的軸不互踏：**逐張核過，0 處越界**

- **`渡辺貞夫`**：**b3 只寫 1974 年平安夜、新宿ピット・イン、三位美國樂手、當晚貝斯手那一軌、`Gerald Oshita` 那一格**（a 組讓出的，**只有本組寫得到，已寫足**）；
  **b4 只寫 Montreux、1967 年起每年 7 月、兩軌作曲／三軌編曲、兩個東非地名曲題、前一年那張雙張現場盤**。
  ⚠ **兩張都 0 字生平前史**（第 5843 條／骨架表），**也都沒有點名 a 組那三張的盤名。**
- **`大野俊三`**：**b6 的正文有 `1976`（發行）與 `1975 年 7 月`（紐約錄音）兩個年份**；**b7 的正文 `1976` 0 命中**（它的「隔年」只在 hook 裡、不帶年份）。
  ⚠ ⚠ **兩張寫完互相對過一次，年份沒有寫反。** **他的三件獎項 0 字**（照研究層的反向禁令與派工信第五節第 1 點）。
- **`土取利行`**：**b2 走 ALM ＋ 半夏社 並列的廠牌欄、系列第四號、間章 的解說、兩人各掛幾軌**；**b12 走五百張壓量、素材清單、兩面標題、坂本龍一 的學籍、竹田賢一 的製作**。
  ⚠ **兩張都沒有寫聯名字串本身，所以 `・`(U+30FB) 那條分隔符裁定（第 5770 條）在正文上 0 風險**——**`key` 從輸入檔逐字複製，一個字元都沒有改。**

## 6020　⚠ ⚠ `live` 那兩張與「不寫成錄音室盤」：**逐字核過**

- **b11《Up-To-Date》**（策展層第 5756 條 `false` → `true`）：**正文逐字寫 `1975 年 4 月 28 日在東京厚生年金会館 錄下的一整場實況`**，**另帶副題把ライブ 與錄音日印在盤上、二十筆 credits 裡一整組演唱會工作人員兩項證據**。**`錄音室` 三個字在這張 0 命中。**
- ⚠ ⚠ **`Chiasma` 在本組 12 張 0 命中**（主線第 1969-B 條／鉤子層第 5845 條）：**b11 的 note 四格本來就沒有曲題，本層也沒有自己補**——**0 字寫成「隔年同名錄音室盤的先行版」。**
- **`錄音室` 這三個字在本組只出現在 b1**（クラウンレコード 自家的第一錄音室，**那張真的在錄音室錄**）；**b2／b10／b12 三張非現場的碟各寫正面的載體或地點交代**（`日壓的黑膠`／`進⋯的 Union Studio`／`四軌的黑膠`），**0 句否定句。**

## 6021　⚠ 派工信第五節逐點回覆（第 3、4、5、7、8 點）

- **第 3 點（`Swiss Air` 的作曲欄）**：**只寫 note 有的「兩軌掛作曲、三軌掛編曲」**，**0 字補作曲者、0 字寫 `Traditional`、0 字寫二刷把五軌記成他的曲。**
- **第 4 點（`111 Sullivan Street` 的編制）**：**逐字寫成 `逐名的編制要到 1979 年同廠的一次重壓才登上盤面`**——**0 字說原壓就印了六人，也沒有寫「原壓一位都沒列」那種否定句**（note 本來就是正面表述，本層照抄）。
- **第 5 點（`Kimiko Is Here`）**：**軸就是盤面那行授權註記（鋼琴手是 RCA Records 借出的）**；**Oliver Nelson 0 命中、她的生涯線 0 字**。
- **第 7 點（版本數）**：**12 張 0 張寫「再發幾次」**——所以那 5 張沒建檔的 Apple 數位版在正文上沒有落地機會。**`版本` 這兩個字只出現在 b11 的「同一曲的兩個版本」（指軌的兩個版本，不是壓次）。**
- **第 8 點（無串流 4 張／無封面 4 張）**：**12 張 0 句聆聽入口、0 句「在架上」、0 字封面**（`封面`／`串流`／`聆聽`／`架上` 四個字串全批 0 命中）。**回撈層的結論（九張全維持、救回 0 張）逐張落地。**

## 6022　⚠ 人名：**照各自那張的 `facts` 抄，0 處互相校正**

- **`本田竹曠`／`本田竹広` 兩形（主線第 1968-B 條／鉤子層第 5850 條）**：**本組 b4 的 note 四格沒有他，本層也沒有補**——**`本田竹` 這三個字在本組 12 張 0 命中，兩形的衝突在本層不需要處理。**
- **鉤子層列的 uncertain 五處（`岡山和義`／`村上光男`／`河原奈美`／`相見明`／`鳥居信影`）本層一處都沒有寫**：**b1 的正文逐字只寫「人聲那一格請的是底特律的合唱組 The Marion Gaines Singers」，0 個漢字伴奏名。**
- **`新宿ピット・イン`（b3／b5）與 `東京厚生年金会館`（b11）照 facts 的字串逐字抄，0 處改字形。**
- **拉丁專名密度**：**b3 五個（Cedar Walton／Sam Jones／Billy Higgins／Gerald Oshita／CBS/Sony）是本組最高**，**超過 `writer-base.md` 的「3 個以內」建議，但那五個是 note 四格的主體、砍掉就沒有故事**；**實測字數 233、未超標**，所以本層維持並在此登記。

## 6023　`desc-tools/jp-proper-names.json`：**本批 append 0 個，既有的一個都沒有動**

`qa-batch` 的 `SIMP` 與 `GARBAGE` 兩道對本組 12 張 0 報。
⚠ **本組用到、靠既有白名單過關的有兩個**：**`東京厚生年金会館`（b11，`会` 在 SIMP 字表裡）與 `半夏社`**——**兩個字串都已經在檔裡，一個字都不用加**（與鉤子層第 5858 條同向）。
...
$ node qa-batch.mjs out c184
out-1｜12 張｜字數 219–239｜>260: 0
out-2｜12 張｜字數 208–233｜>260: 0
out 合計 24 張，與卡單相符 ✓
（qa-check-research 與 fix-spacing 需另跑，共 2 檔各一次）
全部通過 ✓

$ node qa-check-research.mjs batches/input/c184-writer-2.json batches/output/c184-out-2.json
QA 完成｜ 12 張｜標記 0

$ node fix-spacing.mjs batches/output/c184-out-2.json --field desc
待補 0 張（欄位 desc）
```

- ⚠ **a 組已交件，所以 `qa-batch` 沒有報缺檔**（派工信第七節預期的那種情形沒有發生）。
- ⚠ **`fix-spacing` 0 建議**，所以派工信第七節那條「廠牌名與目錄號連寫要回頭改草稿重建、不要 `--write` 打補丁」本批沒有觸發；**`--write` 一次都沒有跑。**
- **本層另外自寫三道（兩支腳本都驗不到）**：**字數兩端**（見 6012）、**首句是否逐字等於 hook**（12/12 通過，`c184w2-build.mjs` 內建檢查）、
  **禁語／否定句／半形逗號貼中文／非拉丁字系**（`傑作`・`必聽`・`里程碑`・`獨樹一格`・`融合多種元素`・`具有代表性`・`層次豐富`・`全片`・`全曲` 0 命中；`不是`・`並非`・`沒有`・`並不`・`未曾`・`從未`・`查無`・`卡池`・`標錯` 0 命中）。
- ⚠ **`名人堂`／`票選`／`百大`／`一份` 四個樣式 0 命中**；**`樂評` 兩處都是掛名而不是評價**（b2 的 間章 是寫解說的、b12 的 竹田賢一 是掛製作的），**照主線第 1876-B 條的判準（做了什麼／說了什麼）成立。**

## 6025　續跑保護、邊界自述，與給主線的三件

**續跑保護（派工信第六節）**：**在第 3、6、9、12 筆各把整份寫回 `desc-tools/batches/output/c184-out-2.json`，共四次整份寫回**；
**判斷檔案在不在一律用 `ls` 與直接讀檔，全程沒有跑過 `git status`。**
臨時檔全部放 scratchpad `c184w2/`，**兩支都帶批次與組別前綴**：`c184w2-build.mjs`（**資料即腳本**：cells 陣列 ＋ 一行產出，重跑即逐字重現定稿）、`c184w2-ngram.mjs`（漢字與片假名兩輪、3 張與 2 張兩個門檻）。

**邊界自述**：
- **動了一個新檔**：`desc-tools/batches/output/c184-out-2.json`（12 筆）。
- **動了一個既有檔**：`batch-progress/c184/rulings.md`（**append 本節 6011–6025，5716–6010 任何一行都沒有改**）。
- **`desc-tools/jp-proper-names.json` 一個字都沒有動**（append 0，見 6023）。
- **沒有碰**：`seed_cards.json`、`apex_pool.json`、`PROJECT_MEMORY.md`、`previews.json`、`caa.json`、卡單、研究稿、鉤子稿、
  **`c184-out-1.json`（只讀，用於合掃與交叉核對，0 位元組寫入）**、其他批次、KV、Firestore。
- **沒有跑過任何 git 寫入指令**：**沒有 `git commit`／`git push`／`git add`，沒有動索引**；只跑過 `git show HEAD:batch-progress/c184/rulings.md` 與 `git diff`（唯讀）。

**給主線的三件（全部已就地決定並且可逆，本層只請覆核）**：
1. ⚠ ⚠ **a 組第 6000 條留給 b 組的五種引入法（年齡／綽號／廠牌關係／親屬／攝影集）在本組全部不可用**——**那五樣的材料在 b3／b4 的 note 裡都被鉤子層整格捨去了。**
   **本層改用掛名欄與作曲編曲欄兩種，五張五種仍成立**（見 6015）。**請主線確認這個替換，並考慮把「引入法配表」挪到鉤子層做**（那一層看得到全批的 note）。
2. ⚠ ⚠ **`的音樂節` ×2 的兩端是 b4 的 hook 原文與 a7 的正文，本層改不動**（見 6018）——**請主線在收件合掃時決定要不要動 a7。**
   **順帶建議：鉤子層那一輪除了 hook 對 hook，也把 hook 對全批 note 掃一次**，這個縫目前沒有人掃。
3. ⚠ **b3《At Pit Inn》的拉丁專名有五個，超過 `writer-base.md` 的「3 個以內」建議**（見 6022 末段）——**五個都是 note 四格的主體，實測 233 字未超標，本層維持；若主線要壓，建議砍的是三位美國樂手的逐名而不是 `Gerald Oshita` 那一格**（那一格是 a 組讓出的，只有這張寫得到）。
