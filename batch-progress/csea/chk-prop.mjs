// c-51 提案檢查。用法：node batch-progress/csea/chk-prop.mjs [組別…]
...
const groups = process.argv.slice(2).length ? process.argv.slice(2) : ['id', 'th', 'ph'];
const all = [];
let bad = 0;
for (const g of groups) {
  const f = path.join(ROOT, `batch-progress/csea/prop-${g}.json`);
