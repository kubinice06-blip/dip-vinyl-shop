...
// 用法：node batch-progress/csea/consolidate.mjs [--write]
//
...
// c-SEA 與前幾批不同：這一區大量正典本來就是廠牌考古合輯，策展層依 §5.6 明確
// 標了 releaseType: "Compilation" 並附上例外理由與證據網址。那些**不該**被
// 「非 Album 一律退回」擋掉——它們是刻意收的，不是配錯碟。
// 只有「策展層說是正規盤、MB 卻說是合輯或現場」這種**不一致**才要退回。
const declaredComp = new Set(props.filter(p => p.releaseType === 'Compilation').map(p => `${p.artist}|${p.album}`));
const bad = out.filter(c => !ACCEPT_SECONDARY[`${c.artist}|${c.album}`]
  && !declaredComp.has(`${c.artist}|${c.album}`)
