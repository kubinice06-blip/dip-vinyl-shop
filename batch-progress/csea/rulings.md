...
## 17. Titiek Puspa《Doa Ibu》：年份維持卡單值，正文寫區間

cseab 的寫作層指出這張的年份三說在 `rulings.md` 裡沒有裁定，它依 hook note
寫成「1960 年代中期」的區間。**這個處置正確，補記為正式裁定。**

**裁定：卡片 `year` 維持卡單的 1964，正文寫「1960 年代中期」不釘單一年份。**
理由與第 2 條（三張越南卡）相同——研究層記下的分歧沒有一方有兩個獨立來源，
卡片欄位需要一個值所以取卡單值，但正文不必假裝那個精確度存在。

**留給本機**：若日後釘定單一年份，這張正文要同步。

## 18. 三處 note 指定因字元預算被整格捨去，判定合理

cseab 寫作層照規矩回報了三處：Djanger Bali 的「爵士與各地音樂結合的最早範例」
定位、Krakatau 的 150 大第 60 名、《Sound of Siam Vol. 2》的編纂者具名。

**裁定：三處都同意捨去。** 前兩者是「定位」與「第二個榜單名次」，
該卡各自已有更硬的內容（五位樂手＋錄音地＋曲目／六人編制＋〈Gemilang〉銷量）；
第三處的兩位編纂者已在同套第一輯那張具名，重複具名換不回 Isan 與 molam
那條指定軸線。

**但 Krakatau 那張要提醒本機**：捨去榜單之後，該卡唯一的商業成績是
**只有單一來源的 80 萬張**，而且原句指的是〈Gemilang〉這首歌不是整張專輯
（寫作層照原句處理成單曲銷量，正確）。**本機若不採信那個數字，這張就沒有
商業成績可寫**——那不是缺失，是這批很多卡的常態。

## 19. `status: "thin"` 撐兩個意思的問題，在這批第二次浮現

cseab 寫作層獨立指出：Sanisah Huri《Senada Hati》的 thin **是真的事實稀薄**，
與 c-51 那七張「查無商業成績但事實表很厚」的 thin 不是同一回事，
**但輸入檔的 `status` 欄只有一個 `thin` 值，寫作層無從自行分辨**。

這與 `batch-progress/c51/rulings.md` 第 2 條記的是同一個規格缺陷，
**由兩批不同的代理各自獨立指出**。本機拆欄位時，這兩批各有乾淨的樣本：
- **事實少**：Sanisah Huri、Waldjinah、Chaweewan、Onuma、เพลิน พรหมแดน、
  Juan de la Cruz Band《Up in Arms》
- **無商業成績但事實厚**：c-51 的 Hugh Mundell、Fania All-Stars 等七張

## 20. 兩筆研究稿的內部矛盾，寫作層都正確迴避，但一筆會污染上架資料

**《Saigon Supersound》——這筆要修，因為它會流進 manifest。**
`researchNotes` 的 §5.6 歷史定位理由寫「目前唯一**經授權**整理、修復並國際
流通的形態」，但同一段下文明確裁定「版權處理查無公開來源，寫作層請勿自行
補述取得正式授權」。**同一段話自己打自己。**

寫作層照後者寫、正文完全不提授權，處置正確。**但那句「經授權」如果被拿去填
`exceptionReason`，就會把一條查無來源的斷言寫進上架資料**——而 §5.6 的
`exceptionReason` 正是驗證器會讀、會跟著卡片上線的欄位。

**裁定：本機填 `exceptionReason` 時改寫成「唯一經整理、修復並國際流通的形態」**，
把「經授權」三個字拿掉。這是這批唯一一筆「正文乾淨、但研究稿的措辭會污染
上架欄位」的情況，值得記住——**研究稿的 notes 不只餵寫作層，也可能被抄進
manifest，所以 notes 裡的斷言要用同一套舉證標準看待。**

**《Siamese Soul》的曲數**：研究稿記 Bandcamp 第 14、15 軌同曲同時長、實際
可能是 14 首，但 `facts` 只寫「收 15 首」而未標存疑，下游因此無從得知。
正文照 facts 寫 15 首。**本機若採 14 首，這張要回頭改。**

## 21. 三處 note 指定被整格捨去（cseab-2），判定合理

Theppabutr 的班底三人（三個專名合計 51 字元，寫進去必須砍掉「錄音室設在
Maha Sarakham」或「200 組以上藝人」，而前者是 hook 收尾、後者是人物軸核心）、
《Molam》的 lam phun／lam sing 兩形式（改保留樂器構造與音階——那是本批唯一的
形式學指派）、Grace Nono 的三首曲目只寫一首（三首連同三個地名族名約 85 字元，
寫滿會擠掉 Tao Music／Tao Foundation 那一格）。

**三處都同意。** 共同的型態是：**note 指派的「列舉型」內容最先被犧牲**，
因為列舉的字元成本隨項目數線性成長，而敘事型的內容砍掉一項就會斷。
這是給下一批派工詞的可用結論——**列舉型指派要標明「列幾項就夠」**，
不要只寫「寫出班底」。

## 22. Joey Ayala 那張的年份，寫作層自己修掉了

初版正文寫「1980 年代中期做的量很少」，`qa-check-research` 標成「編造年份
?1980」——正文出現的四位數年份必須見於 facts，而 1980 這個數字不在。

我正要把它改成中文數字寫法（與 c-51 的 MC5「1950 年代」同型處置），
**但代理在我兩次檢查之間自己改掉了，而且改得比我的方案好**：它不是把數字
換個寫法蒙混過去，而是**整句拿掉那個沒有來源的年代宣稱**，改寫成「卡帶只做了
少量，續作也是同樣的獨立路數」。年代的事全部交給「1992 年重發」那句去定位。

記在這裡是因為我的修補腳本**加了「樣式不符就拋錯停手」的保護才沒有蓋掉它**。
多個寫作者對同一個檔案時，那道保護是必要的，不是可有可無的謹慎。

## 23. 我的裁定第 10 條與資料脫節，已修根因（不是寫作層的問題）

我在上面第 22 條寫「代理自己修掉了年份、而且改得比我的方案好」——**那個描述
不完整**。真正的原因比「改寫得好」更根本，寫作層在交件回報裡講清楚了：

**該卡的 `facts` 裡根本沒有原盤年份。** 1982（生涯起點）與 1992（重發）都在，
但 **1985 只寫在 `yearVerified` 與 `researchNotes`，沒進 `facts`**。而
`qa-check-research` 只認 facts，所以任何寫到原盤年代的句子都會被判成編造年份。

也就是說：**我的裁定第 10 條要求正文交代「1980 年代中期的地下卡帶」，
但資料層供不出任何一個 1980 年代的年份給它引用。** 裁定與資料脫節，
寫作層照 base 的「每條斷言都要能回溯到 facts」把年代拿掉，是**正確且唯一
正確的處置**，不是聰明的權宜。

**已修根因**：把 1985 連同「1982／1985 兩說未定案」補成一條 fact
（來源就是研究層自己引的英文維基作品表），並在 notes 記下為什麼要補。
研究層其實查證過這個值，**問題是它放在下游讀不到的欄位**。

**這是一個會重複發生的型態，值得寫進規範**：`yearVerified` 與 `researchNotes`
是給主線與本機看的，`facts` 才是餵下游的。**凡是希望正文寫得出來的事實，
必須進 facts**——放在 notes 等於沒給。這批已經出現兩次同型問題
（另一次是第 20 條：《Saigon Supersound》的「經授權」寫在 notes 卻可能被抄進
`exceptionReason`）。**兩次方向相反，但根源同一個：notes 與 facts 的分工
沒有講清楚下游各讀哪一個。**

**`cseac-writer-1.json` 不重跑。** 那是寫作層當時實際看到的內容，
保留原樣才是誠實的紀錄；成品已定稿且 QA 全過。補的那條 fact 進的是研究稿，
下一批（或本機重跑）自然讀得到。
