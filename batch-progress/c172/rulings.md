...
# c-172 鉤子層裁定（a 組 3 張，編號 3681–3710，本節用到 3691）

**append 寫入，未動 3601–3628 任何一行。** 輸出：`desc-tools/batches/hooks/c172-hooks-a.json`（3 筆）。

---

## 第 3681 條（**字元預算：公式、倒量結果、本組分佈**）

**公式逐字照 `hook-base.md` 雲端註記第 2 點（第 1793-B 條），一個字沒改：**

```
預算 = Array.from(hook).length + Array.from(note).length
       − 「主故事：」(4) − 每個「→」(1) − 「正文只寫上列各項。」(9) − 「這條骨架全批只走本張。」(11)
```

**只扣這四樣，其餘一律計入**（年份指定句「發行年寫 1998 年。」、寫法指定句「正文寫成像一場錄音室現場」全部算）。
**逐項 `Array.from().length` 實測，零心算**（量尺腳本 `meas-c172-hook.mjs`，放在 scratchpad，未進 repo）。

**倒回去量 `c171-hooks-a.json`（10 張）：**

```
228 218 229 220 224 224 227 224 217 222
min 217｜max 229｜中位 224｜超過 230 者 0
```

**與派工信寫的「217–229／中位 224」逐格相符** → 同一把尺，確認後才量本組。

**本組實測分佈（3 張）：**

| 卡 | 預算 | hook 原始字元 | hook 加權 | note 原始字元 |
|---|---:|---:|---:|---:|
| `A prima vista` | **223** | 31 | 26 | 218 |
| `Parker's Mood` | **228** | 32 | 26 | 222 |
| `Spirit Talk` | **229** | 24 | 24 | 232 |

**min 223／max 229／中位 228，全部 ≤ 230。** ⚠ **未沿用任何一批的係數**：初稿三張實測 317／366／374，
**等於要砍掉 40%**，與 c-171 的手感完全不同（本批三張的專名密度極高：人名、錄音室名、廠牌名、目錄號同時出現）。

---

## 第 3682 條（**骨架歸屬；照 `hook-base.md` 雲端註記第 3 點**）：**三條骨架、三張各擁有一條，0 張讓出**

**歸屬表由程式掃 `note` 是否含「這條骨架全批只走本張。」產生，不是手打**（腳本 `skel-c172.mjs`）：

| 卡 | 骨架 | 狀態 | 鏈節數 |
|---|---|---|---:|
| `A prima vista` | **跳槽大廠卻回前東家的錄音室錄音**（Label Bleu → Blue Note，錄音地仍是亞眠文化之家內 Label Bleu 自有的 Gil Evans 錄音室） | **擁有** | 3 |
| `Parker's Mood` | **曲目作者歸屬**（十軌無一自作，真正出自 Parker 的只有四首） | **擁有** | 3 |
| `Spirit Talk` | **錄音工法的減法**（直接錄成兩軌母帶、credits 無混音欄、逐軌編制遞減） | **擁有** | 4 |

**三條互不重疊，因此三張都寫了「這條骨架全批只走本張。」，沒有一張需要寫讓出**（讓出的卡什麼都不寫，本批不適用）。
⚠ **`note` 內 0 處點名其他盤名**，`qa-batch` 的 `互指?` 因此 0 行。

---

## 第 3683 條（**同藝人反同構：兩張 Di Battista 的分軸；本批最需要判的一條**）

兩張同為 Di Battista，**「曲目是誰寫的」這一格兩張都寫得出來**（`A prima vista` 十二軌十一軌出自團員、
`Parker's Mood` 十軌無一自作），**互為鏡像，正是最容易同構的形狀**。

**裁定：這條骨架歸《Parker's Mood》**（它只剩這一條可走：本碟沒有商業成績、沒有獎項、發行層不能寫），
**《A prima vista》整格讓出**——連〈Lush Life〉借自 Billy Strayhorn 這個收尾細節也一併不寫，
改以「廠牌與錄音室」為主軸。**判準：可逆（改的是 note 欄位）＋卡住整條線（不決定兩張就撞），當場定。**

---

## 第 3684 條（**跨批反同構：Eubanks 四張連號**）：**本張走錄音工法，不碰前三張用掉的軸**

池中 Eubanks 已有 c-151《Turning Point》(1992)、c-152《Live at Bradley's》(1994)、c-171 a《Spiritalk 2》(1995)。
逐張回查三張的 `note`：

| 卡 | 已用掉的軸 |
|---|---|
| c-151 | **編制缺席**（兩把貝斯、兩套鼓、中音長笛、沒有鍵盤）＋九軌自作＋爵士榜第 8 名 |
| c-152 | **編制缺席**（無鼓三重奏）＋曲目全非自作＋單場現場 |
| c-171 a | Tonight Show 樂團與接任音樂總監的時序＋九首自作＋哥哥 Robin |

**→ c-151 與 c-152 已經把「編制少了什麼」這個 hook 形狀用掉兩次**，本張因此
**把編制放進 `note` 的第二格（逐軌遞減，不是整張缺席），hook 改走錄音工法**（九軌直接錄成兩軌母帶）。
**家族背景（哥哥 Robin）整格不寫**——研究層 `notes` 的反同構備忘已點名它屬於 c-171 a，本層照辦，順帶省 8 字元。

---

## 第 3685 條（**四條「不得寫」的達成方式：全部靠整格捨去，`note` 0 處否定句**）

⚠ **研究層 `notes` 裡那四條「正文不得寫 X」是判斷依據，不是 `note` 的文字**（第 1837-B 條）。
**`note` 一律正面表述，要讓某件事不出現就整格不寫，讓它沒有生成的可能。** 逐條對應：

| # | 不得寫 | 本層的達成方式 |
|---:|---|---|
| 1 | `Parker's Mood` 不得寫成 2004 年美日歐同步、不得把美版寫成 2004 年上市 | **發行格整格不寫**，只留「發行年寫 2004 年。」**連首發地法國 2004-09-17 也不寫**——寫了就給寫作層一個「發行時點」的抽屜，它會去 `facts` 第 11 條把美版那一層一起翻出來 |
| 2 | 不得寫成「收錄 Parker 的經典錄音」 | **主故事鏈第一格就是「2004 年 4 月 10 至 12 日在米蘭的 Officine Meccaniche 錄音室錄成」**——把「這是一次新錄音」放在鏈首，後面「十首沒有一首是他寫的」就只能讀成曲目來源，不會讀成選輯 |
| 3 | `Spirit Talk` 不得寫「1993 年 4 月發行」 | **MB 的 `first-release-date` 整格不寫**；日期只出現兩處，且各自帶標籤——「1993 年 4 月 19 至 23 日**錄於**紐約 Sound On Sound」與「Billboard 爵士榜 **10 月 23 日新進**」，**年份只由「發行年寫 1993 年。」承擔** |
| 4 | `Spirit Talk` 不得寫成 smooth jazz | **曲風格整格不寫**（三張的 `note` 一律 0 處曲風字樣） |
| 5 | `direct to 2-track` 可寫成「像一場錄音室現場」、不得寫成現場盤 | **正面指派句「正文寫成像一場錄音室現場」**，且該格前半已逐字寫明「錄於紐約 Sound On Sound」（錄音室），**不給現場盤的解讀空間** |
| 6 | 榜位只寫觀察，不得寫死「最高第 14 名」 | **只列兩期的期別與名次**（10 月 23 日新進第 24 名、11 月 20 日第 14 名），**`note` 裡 0 處出現「最高」** |

**程式複核**（`skel-c172.mjs` 末段）：`2005`／`美版`／`同步發行`／`經典錄音`／`收錄.{0,4}Parker`／
`4 月.{0,2}發行`／`smooth`／`曲風` **全部 0 命中**。

---

## 第 3686 條（**整格捨去清單；判準第 1830-B 條：名單型＝一格裡兩個以上只起點名作用的專名，密度最低者先砍**）

初稿 317／366／374 → 定稿 223／228／229，逐格捨去如下。**「不是只起點名作用」的人名一律留**
（Barron 帶 Gillespie 樂團這層關係、Ted Jensen ⋯ 見下）：

**`A prima vista`（−94）**
1. **五重奏名單格**（Boltro／Legnini／Bonaccorso／Henocq 四個專名各配一件樂器，約 70 字元）——**標準名單型，第一個砍**。
2. **曲目作者歸屬格**（十二軌十一軌團員自作＋〈Lush Life〉）——**不是砍字，是第 3683 條讓給《Parker's Mood》**。
3. 十二軌總長 58 分 59 秒（載體數字，密度最低）。
4. Label Bleu 沿革（Michel Orier 兩百多張、旗下 Texier／Sclavis／Lourau／Galliano）——**四個專名純點名，名單型**。
5. Di Battista 生平（羅馬、banda、Art Pepper、Massimo Urbani、1992 年 Calvi 遇 Como、Sunset 兩場被 Romano 與 Cugny 看見）。
6. Boltro 與 Legnini 的個人經歷格。7. 1998 年 11 月法國宣傳 CD-ROM 版本格。

**`Parker's Mood`（−138）**
1. **美版 2005-01-25 那一層整格不寫**（第 3685 條第 1 項）。
2. **六首非 Parker 曲目的作曲者名單**（Gillespie／Clarke、Gershwin 兄弟、Paparelli、Raksin／Mercer、Dameron、Monk，**七個專名純點名，最典型的名單型**）——只留「開場〈Salt Peanuts〉出自 Dizzy Gillespie」一個，**因為它與下一句的 Barron 有因果關係，不是點名**。
3. **節奏組名單格**（Bonaccorso 低音提琴、Herlin Riley 鼓、Boltro 只在四軌）——名單型。
4. Herlin Riley 的經歷（紐奧良、Ahmad Jamal、Wynton Marsalis、林肯中心）——四個專名純點名。
5. 〈Donna Lee〉著作權三說（Parker／Miles Davis／Tiny Kahn）——精彩但屬曲目本身的歷史，不是本作的事。
6. 〈Parker's Mood〉1948-09-18 Savoy 那一場（John Lewis／Curley Russell／Max Roach、錄五次、速度 65–87）。
7. Parker 1955-03-12 辭世、距錄音四十九年。8. Officine Meccaniche 由 Mauro Pagani 創辦（Muse／Franz Ferdinand ⋯ 名單型）。
9. 混音與母帶地點（Davout／Marwan Manley、製作 Yves Chamberland ⋯）。10. 十軌／十二軌兩種曲序與各版目錄號。

**`Spirit Talk`（−145）**
1. **曲風格**（第 3685 條第 4 項）。2. **《Turning Point》逐項對照格**（七天錄音混音、Dolby SR、KEF、兩把貝斯兩套鼓）——整段是另一張卡的內容。
3. **逐軌編制普查格**（Holland 八軌／Robin Eubanks 五軌／Kent Jordan 五軌／Mondesir 與 Smith 分打，**五個專名＋一串數字，名單型**）——**只留兩個空格**（第 6 軌〈Inside〉只有長號與低音提琴、第 8 軌〈Journey〉只剩他一人），因為那兩格是「遞減」這條軸的證據，不是點名。
4. **母帶師 Ted Jensen 與 Sterling Sound**——**單一專名但只起點名作用**，砍；「credits 裡沒有混音這一欄」這句已經把同一件事說完。
5. 九軌軌名與軌長清單（52 分 58 秒）。6. Mark Mondesir 生平（倫敦、Courtney Pine、Ian Carr）。
7. 美術群（Cochrane／Larson／Nilsson／Norton／Frazier，名單型）。8. 日版 TOCJ-5868（1994-01-19）與 BMG 郵購卡帶兩個版本格。
9. Cash Box 樂評只留「一種易於進入的空靈感」一句，**指板功夫、與中音長笛的來回、〈Livin'〉的搖擺、Holland 與 Smith 的點名全砍**。

---

## 第 3687 條（**`chk-hook-crossgroup` 看不到的那一類：同句型三次；本層程式自查並改寫兩次**）

⚠ **派工信點名要自己盯。本層不手讀，改寫程式掃跨張重複的 4-gram**（只取中文字、先剝掉固定句式，`skel-c172.mjs`）。

- **第一輪**：掃出 **`年月至日`×3**——三張的錄音日全部寫成「X 年 X 月 X 至 X 日」，**三次同句型成立**。
- **第一次改寫**：把《Parker's Mood》改成「地點在前、日期在後」→ **反而掃出 `錄音室年月至`×2**（它與《A prima vista》撞在一起，**而那兩張是同一位藝人，比撞《Spirit Talk》更糟**），**回退**。
- **第二次改寫（定稿）**：**改《A prima vista》的日期形態**——「1998 年 8 月 27、28、29 三天錄完」（頓號列舉，不用「至」區間）。
  **結果：`年月至日`×2（分屬兩位不同藝人）、`的錄音室`×2，無任何 3 次以上的重複。**

**裁定：跨張句型自查一律用程式掃 n-gram，不用人眼**——本批兩次改寫裡有一次是「越改越糟」，人眼看不出來。

---

## 第 3688 條（**hook 的型別輪換與開頭互異**）

| 卡 | 型 | 開頭前四字 |
|---|---|---|
| `A prima vista` | **人物定調**（他跳進了 Blue Note，錄音卻留在⋯） | `他跳進了` |
| `Parker's Mood` | **斷言**（獻給 Bird 的十首曲子，真正出自 Bird 筆下的只有四首。） | `獻給 B` |
| `Spirit Talk` | **斷言／工法**（九首曲子直接錄成兩軌母帶⋯） | `九首曲子` |

**三張互異，0 處禁語、0 處引號整句包裹、0 處分數星等、句末全形句號齊備**（`chk-hook-crossgroup` 實跑證實）。
⚠ **兩張走斷言型**：三張的批次無法湊滿四型輪換，**判準取「開頭與句式互異」優先於「四型湊齊」**（可逆、有先例）。
⚠ **hook 無懸念句，因此不需要在 `note` 裡交代收尾方式。**

---

## 第 3689 條（**收工兩支腳本的實跑結果，逐字**）

**`node qa-batch.mjs hooks c172`**（cwd `desc-tools`）：

```
（略過 qa-check-hooks.mjs：本 repo 無此檔。字數／禁語／開頭雷同／分數星等
  請改跑 node chk-hook-crossgroup.mjs c172，本階段只做事實對照與字元掃描。）
全部通過 ✓
```

**旗標 0，`互指?` 0 行**（本批三張的 `note` 皆未點名其他盤名，見第 3682 條；**因此沒有需要人工判讀的 `互指?`**）。
⚠ **本批只有一組，`qa-batch` 按檔案存在與否動態掃，未因缺 b 檔報錯**——與派工信所述相符。

**`node chk-hook-crossgroup.mjs c172`**（cwd `desc-tools`，**必跑**）：

```
c172｜1 組｜3 張

hook 加權 24–26｜note 218–232

✓ 全部通過
```

**八道檢查（開頭前四字、同構骨架、校對痕跡、分數星等、禁語、句末標點、否定句、字數）全數 0 旗標。**

---

## 第 3690 條（**研究層 `notes` 與 `facts` 衝突時的取捨；本層未動用的一條**）

研究層 `notes` 逐字要求「**請下游不要寫「九軌全是他自己寫的」**」（該句在四筆 Discogs、MB 兩面、Apple 上都找不到作曲欄）。
**本層的做法不是在 `note` 裡寫禁令，而是《Spirit Talk》整張不碰「誰寫了這些曲子」這一格**——
**`facts` 十二條裡本來就沒有作曲資訊，寫作層沒有來源可寫**，形同雙保險。
（對照：《A prima vista》與《Parker's Mood》的作曲歸屬 `facts` 逐軌俱全，所以那兩張可以寫。）

---

## 第 3691 條（**⚠ 派工信與 `hook-base.md` 原文／既有裁定牴觸之處，以及交件版本認定**）

### （一）牴觸與誤述：**1 處實質、1 處轉述失準**

| # | 派工信原句 | 判讀 |
|---:|---|---|
| 1 | 第三節表格「**`Parker's Mood`⋯十軌全是別人的曲目但是同一次新錄音**」，並在下方警告欄轉述研究層說這句「不夠準確」 | **派工信這句本身成立**（逐軌 `Written-By` 實查：十軌無一出自 Di Battista）。**不成立的是策展層 `why` 那句「十軌全是 Bird 的原創與他留下印記的曲目」**——研究層 `notes` 把兩句併成「兩句都不夠準確」，**這一併是研究層的誤判，不是派工信寫錯**。本層照 `facts` 寫成「十首沒有一首是他寫的，出自 Charlie Parker 的只有四首」，兩邊都不偏。 |
| 2 | 第一節「**`batch-progress/c172/rulings.md` 整檔讀過（策展 3601–3628）**」，但檔案第 1 行標頭逐字仍寫「**a 組 2 張**」 | **標頭是 3601–3617 那一輪留下的，第 3618 條已把總表更新為 3 張**。本層以第 3618 條為準（3 張），**未改標頭**（禁覆寫）。**這是檔案內部的時序落差，派工信沒寫錯。** |

**其餘逐條核對相符**：字元預算公式（雲端註記第 2 點）、骨架歸屬寫法（第 3 點，第 1787-B 條）、
c-171 a 的 217–229／中位 224、`qa-batch` 在 hooks 階段不做字數與禁語（腳本自己印出這句）、
`Spirit Talk` 9 軌 52:58（與第 3624 條相同）、三張皆 `group === "a"`。**0 處與 `hook-base.md` 原文牴觸。**

### （二）交件版本認定

**以工作區當下的 `desc-tools/batches/hooks/c172-hooks-a.json`（3 筆）與本節為交件版**（第 1803-B／3617 條）。
**「筆數對了」不等於「定稿了」**——3 筆在第一次寫回磁碟時就齊了，但那一版的預算是 317／366／374，**全部超標**。
**定稿時點是預算三張全部 ≤ 230、`qa-batch` 旗標 0、`chk-hook-crossgroup` 0 旗標、跨張 n-gram 無 3 次以上重複，四者同時成立之後。**
本層依派工信「每做完 1 張就整份寫回」，整份寫回磁碟共 **8 次**（3 次逐張建檔 ＋ 5 次定點改寫）。
**本節以 append 寫入，未覆寫 3601–3628 任何一行；`seed_cards.json`／`apex_pool.json`／`PROJECT_MEMORY.md`／
`previews.json`／`caa.json`／其他批次檔案一律未碰，未 `git commit`／`git push`／未動 git 索引。**

---

# c-172 寫作層裁定（a 組 3 張，編號 3692–3720，本節用到 3703）

**append 寫入，未動 3601–3691 任何一行。** 輸入：`desc-tools/batches/input/c172-writer-1.json`（3 張）。
輸出：`desc-tools/batches/output/c172-out-1.json`（3 筆，key 與輸入逐字同序同數，欄位只有 `key`／`desc`）。

---

## 第 3692 條（**先對尺，再量自己的稿**）：**倒量 c-171 兩組全部重現，只有「中位數」一項的取法要講清楚**

派工信給的倒量參考是「`c171-out-1.json` 10 張 min 199／max 238／中位 218；`c171-out-2.json` 10 張 min 192／max 233／中位 208」。
**本層用 `Array.from(desc).length` 逐張實量（量尺腳本 `meas-c172-w1.mjs`，在 scratchpad，未進 repo），結果：**

| 檔 | 實測 | 與派工信 |
|---|---|---|
| `c171-out-1.json` | **min 199／max 238／中位 218** | **逐格相符** ✔ |
| `c171-out-2.json` | **min 192／max 233**／中位 **206.5**（下中位 205、上中位 208） | **min／max 逐格相符**；**中位差在取法** |

⚠ **`c171-out-2` 的 10 張是偶數筆，中位數沒有唯一值**：兩個中間值逐字是 **205（`Ruben Hein｜Live`）與 208（`James Francies｜Flight`）**，
**算術平均 206.5、取上中位數 208**。**派工信寫的 208 ＝ 上中位數，不是平均。**
**裁定：同尺確認以 `min`／`max` 為準（兩者逐格相符即同尺），中位數只當參考，並在回報裡標明取法。**
判準第 2 條（可逆）。⚠ **給主線：下一批派工信引中位數時標一下取法**，否則代理會以為自己的量尺不對而回頭重查。

---

## 第 3693 條（**逐張字數與伸縮格；⚠ 本批 `sound` 一格都沒有動用**）

| # | 卡 | **初稿** | **定稿** | 差 | 伸縮格 `sound` |
|---:|---|---:|---:|---:|---|
| 1 | `Stefano Di Battista Quintet《A prima vista》` | **248** | **234** | −14 | **未動用** |
| 2 | `Stefano Di Battista《Parker's Mood》` | **253** | **239** | −14 | **未動用** |
| 3 | `Kevin Eubanks《Spirit Talk》` | **232** | **232** | 0 | **未動用** |

**定稿 min 234／max 239／中位 234，`<180` 0 張、`>240` 0 張。**
⚠ **三張全部貼著上緣（234–239），下限方向本批完全沒有出現。** 初稿是 **248／253／232 ＝ 兩張超標 8 與 13 字元、一張在區間內**。

**⚠ 收斂手段要寫清楚，因為它不是 base 預設的那一種**：
**本批 `note` 鏈上的格全部是主故事所需，沒有一格可以整格捨去；而 `sound` 沒有被寫進正文，因此也沒有「砍 `sound`」這個選項。**
實際砍掉的是**本層自己為了接句而加的連接子句**（第 1 張的「這張碟就是為新東家錄的」13 字元），
以及**同義壓縮**（「錄音在⋯錄音室完成」→「⋯錄於⋯錄音室」、「是 Dizzy Gillespie 的曲子」→「出自 Dizzy Gillespie」）。
**`note` 鏈上的事實格 0 格被砍、`facts` 0 次回撈、鉤子層第 3686 條整格捨去的 25 格 0 格回流。**

**裁定（新增，補在第 1838-B 條「`sound` 是唯一緩衝格」之後）：**
**當 `sound` 未被寫進正文時，它不是緩衝格，只是「未使用的格」——這時超標的收斂順序是
（一）本層自己加的連接子句 →（二）同義壓縮 →（三）才輪到 `note` 鏈上的格。**
**反過來，不足時第一順位仍然是把 `sound` 補進來**（第 1839-B 條的用法不變）。
判準第 1 條（有先例：第 1838-B 條把 `sound` 定為雙向可調）與第 2 條（可逆）。

⚠ **偏差方向不繼承**（`writer-base.md` 第 107–122 行）：本批三張是**單向高估 0、單向低估 2、一張零偏差**——
**寫下來只當紀錄，不得當成下一批的係數。** 本層動筆前依 base 逐張列了預算表，**兩張仍然低估 8–13 字元**，
根因與 base 第 199–210 行記的爵士批一致：**單張要點名 4 個以上 20 字元級的拉丁專名**
（第 1 張：`Blue Note`／`Label Bleu`／`Volare`／`Gil Evans`／`Nicolas Pflug`／`Stefano Di Battista Quintet`＝27 字元；
第 2 張：`Charlie Parker`／`Stefano Di Battista`／`Officine Meccaniche`／`Salt Peanuts`／`Dizzy Gillespie`／`Kenny Barron`）。

---

## 第 3694 條（**首句與第二句對撞：三張逐張的解法**；⚠ 本層自己回頭讀過三次）

`writer-base.md` 第 158 行（首句照抄 hook）有 `qa-check-research` 的硬檢查，第 159 行（不得重述 hook）沒有——
**失效方向固定，只有本層自己會發現**（第 1839-B 條）。**三張逐張列出第二句做了什麼：**

| 卡 | hook 裡的代稱／未具名項 | **第二句的具名動作** | 第二句新增的事實 |
|---|---|---|---|
| **A prima vista** | 「**他**」「**亞眠的那座文化中心**」 | **「他」→ `Stefano Di Battista Quintet`**（卡單藝人欄逐字，見下）；**「那座文化中心」→ 第三句具名為「亞眠文化之家」** | 1997 年《Volare》、法國廠牌 Label Bleu、出道 |
| **Parker's Mood** | 「**Bird**」（全句兩次），**演奏者未具名** | **「Bird」→ `Charlie Parker`**（第二句第一個字串）；**演奏者具名為 `Stefano Di Battista`** | 十軌無一出自 Di Battista、2004 年 4 月 10 至 12 日、米蘭 Officine Meccaniche 錄音室 |
| **Spirit Talk** | **無代稱，但藝人與盤全程未具名** | **具名 `Kevin Eubanks`**，並把「九首曲子」接成「這九軌」 | 1993 年 4 月 19 至 23 日、紐約 Sound On Sound、像一場錄音室現場 |

⚠ **第 3 張是 base 那條解法沒有涵蓋的形狀**：**hook 裡一個代稱都沒有**（「九首曲子直接錄成兩軌母帶，中間沒有混音這道工序。」），
**所以「替代稱具名」無從施力**。**本層的處理：把「具名」的對象從代稱擴大到「hook 通篇未具名的主詞」**——
hook 只講了一件事怎麼做，沒講是誰做的、在哪一張碟上，**第二句就補這兩件**，並繼續往下給錄音日與地點。
**裁定（建議補進 `writer-base.md` 第 158–159 行之間那段）：
第二句的職責是「補上 hook 沒有指明的那個位置」——hook 用代稱就具名，hook 連代稱都沒有就補主詞與作品，
兩種情形都不是把 hook 的句意再講一次。** 判準第 1 條（與第 1839-B 條同向的一般化）與第 2 條（可逆）。

⚠ **藝人名照卡單逐字**（`writer-base.md`「本卡自己的藝人名」那條）：第 1 張正文寫 **`Stefano Di Battista Quintet`**（27 字元，帶 Quintet），
**未因為省字元而改寫成第 2 張的個人形**——第 3604 條把兩張判成 MB 的兩個實體（Group `88131738` vs Person `f6c8ec20`），**正文跟著卡單走。**

---

## 第 3695 條（**兩張同藝人的分軸；`chk-hook-crossgroup` 掃不到，本層程式自查**）

鉤子層第 3683 條已把「曲目是誰寫的」整格判給《Parker's Mood》、《A prima vista》整格讓出。**本層在正文層照辦，並再往下分了三格：**

| 面向 | **A prima vista** | **Parker's Mood** |
|---|---|---|
| **主軸** | **廠牌與錄音室的錯位**（跳進 Blue Note、錄音留在前東家 Label Bleu 自有的錄音室） | **曲目作者歸屬**（十軌無一自作、真正出自 Parker 的只有四首） |
| **第二句的切入** | **藝人＋時序**（1997 出道 → 1998 合約） | **曲庫＋演奏者**（以 `Charlie Parker` 起句，藝人名後置） |
| **日期的句法** | **頓號列舉**「1998 年 8 月 27、28、29 三天錄完」 | **區間**「2004 年 4 月 10 至 12 日」 |
| **收尾** | **載體時差**（日本版 2001 年 3 月才上市） | **人物關係**（Gillespie 的曲子 → Barron 待過 Gillespie 的樂團） |
| **過場連接** | 「；」＋「仍是」 | 「；」＋「，」，無轉折詞 |

⚠ **第二句的開頭刻意做反**：**第 1 張以藝人名起句、第 2 張以 `Charlie Parker` 起句而把藝人名放在句中**。
**本層曾試過第 2 張也用「藝人名＋年份」起句（實測 240 字元、剛好貼上限），當場回退**——
**理由不是字數，是它與第 1 張的第二句骨架逐字同型**（`藝人名 + 年份 + 以／於 + 專名`），
**而這兩張是同一位藝人，正是第 3683 條要擋的形狀。** 判準第 3 條（卡住整條線：兩張不分開就撞）。

---

## 第 3696 條（**跨張 n-gram 自查：逐字結果**）

⚠ **派工信點名要自己掃，本層不用人眼，寫程式掃**（`ngram-c172-w1.mjs`，scratchpad，未進 repo）：
**只取 `desc` 裡的中日韓統一表意文字（剝掉所有拉丁專名、數字、標點），同一張內重複只計一次，掃跨張 4-gram。**

```
2 次: 的錄音室 [0, 1]
2 次: 年月至日 [1, 2]
--- 跨張 4-gram：3 次以上 0 ｜2 次 2
同藝人兩張共用 4-gram: 1 ['的錄音室']
開頭: 他跳進了 Blu ／ 獻給 Bird  ／ 九首曲子直接錄成
```

- **3 次以上的重複：0 個** ✔（派工信的門檻是「3 次以上就改寫」）。
- **兩張同藝人（索引 0、1）共用的 4-gram 只有 1 個：`的錄音室`**——**它是剝掉拉丁專名之後才貼在一起的產物**
  （第 1 張逐字是「亞眠文化之家裡**的**`Gil Evans` **錄音室**」、第 2 張是「米蘭**的** `Officine Meccaniche` **錄音室**」），
  **在原文裡兩者中間各隔著一個拉丁專名，不是同一個句型。判不改寫。**
- **`年月至日` 落在索引 1、2**（`Parker's Mood` 與 `Spirit Talk`），**分屬兩位不同藝人**，且第 1 張已由本層改用頓號列舉避開
  ——**與鉤子層第 3687 條在 `note` 上做的同一個動作，本層在 `desc` 上重做一次。**
- **三張的開頭前四字互異**（`他跳進了`／`獻給 B`／`九首曲子`），**這一項由 hook 決定，本層未動。**

**裁定：沿用第 3687 條——跨張句型自查一律用程式掃 n-gram，不用人眼。** 本層另補一句：
⚠ **剝掉拉丁專名之後的 n-gram 會產生假陽性**（本批的 `的錄音室` 就是），**掃出來要回原文看一眼再決定改不改**。

---

## 第 3697 條（**⚠ `note` 的指派句與 `writer-base.md` 直接牴觸一處；依 base 改寫**）

`c172-hooks-a.json` 第 3 張的 `note` 逐字寫「**樂評說全片有一種易於進入的空靈感**」。
**`writer-base.md`「文字」節逐字：「『全片』『全曲』不可用來指整張專輯。」**
——**這是 `note` 的指派句本身帶了一個 base 明文禁止的詞。**

**本層處置：照 base，正文寫成「樂評當時形容它有一種易於進入的空靈感。」**
**「全片」不進正文，語意零損失。**
⚠ **成因不在鉤子層**：**研究層 `facts` 第 5 條引 Cash Box 原文時逐字就是「有一種易於進入的空靈感貫穿全片」**，
鉤子層壓縮 `note` 時原樣帶了過來。**三層都沒錯，是 base 的禁語表與引文撞在一起。**
**裁定：引文裡的禁語，正文一律改寫成合規的同義說法，不因為「那是引文」就照抄**
（對照：base 只替**專名**開了禁語例外——書名、獎項名、榜單名、專輯名——**引文不在例外之列**）。
判準第 1 條（有先例：第 1838-B 條的 `Five for Fun`，`note` 鏈上的素材違反 base 時以 base 為準）與第 2 條。

---

## 第 3698 條（**六條上游硬限制的達成方式，逐條程式複核**）

鉤子層第 3685 條列了六條「不得寫」，並已在 `note` 層整格捨去。**本層在 `desc` 上逐條複掃，全部 0 命中：**

| # | 限制 | 正文實況 |
|---:|---|---|
| 1 | `Parker's Mood` 不得寫成 2004 年美日歐同步、不得把美版寫成 2004 年上市 | **`2005`／`美版`／`同步` 逐字 0 命中**。**正文只寫錄音日 2004 年 4 月 10 至 12 日，完全沒有發行地與發行日的句子**——`year` 1998／2004／1993 三張都由事件日期承載，**0 張寫「⋯年發行」** |
| 2 | 不得寫成「收錄 Parker 的經典錄音」 | **`經典錄音` 0 命中**；第二句把錄音日與錄音室放在最前，**「十軌無一出自他的筆」只能讀成曲目來源** |
| 3 | `Spirit Talk` 不得寫「1993 年 4 月發行」 | **`4 月發行` 0 命中**；4 月那個日期逐字帶著「錄於」 |
| 4 | `Spirit Talk` 不得寫成 smooth jazz | **`smooth` 0 命中**；**三張正文 0 處曲風字樣** |
| 5 | `direct to 2-track` 寫成「像一場錄音室現場」、不得寫成現場盤 | 正文逐字「整張碟因此像一場錄音室現場」，**`現場盤` 0 命中**，且同句已先寫明「錄於⋯紐約 `Sound On Sound`」 |
| 6 | 榜位只寫觀察，不得寫死「最高第 14 名」 | 正文逐字只列兩期（10 月 23 日新進第 24 名、11 月 20 日第 14 名），**`最高` 0 命中** |

**另掃 base 的禁語與文字規則，三張各 0 命中**：
`傑作`／`必聽`／`里程碑`／`獨樹一格`／`融合多種元素`／`具有代表性`／`層次豐富`／`全片`／`全曲`／`你`／`我`；
**西里爾／希臘／諺文／假名 0 字**；**千分位 0 處**；**半形逗號或分號貼中文字 0 處**；**簡體 0 字**。

---

## 第 3699 條（**樂評與榜單怎麼寫：媒體名 0 處，`Billboard` 1 處且是榜名**）

- **兩張用到樂評**（`Parker's Mood` 的「當年的樂評寫他心繫 Parker 卻不模仿」、`Spirit Talk` 的「樂評當時形容它⋯」），
  **兩張都只寫評語的內容，0 處媒體名、0 處樂評人姓名、0 處星等與分數**（base「來源平台與樂評姓名不進正文」）。
  ⚠ **`facts` 裡的出處分別是 Billboard 2005-01-29 與 Cash Box 1993-10-16，兩個刊名都沒有進正文。**
- ⚠ **`Spirit Talk` 的正文出現 `Billboard`，但那是榜名不是刊名**：逐字「排上 Billboard 爵士榜第 24 名」。
  **判可寫，沿用第 1840-B 條的先例**（逐字「那是官方排行榜的名稱，不是樂評背書；去掉『Billboard』反而變成無出處名次」）。
  **本批與 c-171 a 各自獨立走到同一處，這是第三批同判。**
- **兩張樂評句的句型刻意做不同**（「當年的樂評寫⋯」vs「樂評當時形容⋯」），**避免成為第三個跨張同句型**。

---

## 第 3700 條（**代名詞的所指距離；`Parker's Mood` 回頭改寫一次**）

`writer-base.md` 逐字：「代名詞（他／她／後者）與所指對象不可相隔太遠，寧可具名。」
⚠ **`Parker's Mood` 的初稿在這一條上有實質風險**：該張正文同時出現 `Charlie Parker` 與 `Stefano Di Battista` 兩個男性專名，
**初稿把「十軌無一出自他的筆」放在句末**，「他」與兩個專名的距離相近，**而誤讀成 Parker 會與 hook 直接衝突**（hook 逐字說有四首真的出自 Bird）。

**改寫（定稿）：把「十軌無一出自他的筆」整句前移，緊接在 `Stefano Di Battista` 之後**，並**把樂評句移到 Gillespie／Barron 那句之前**——
**使全張兩處「他」的最近前項都是 `Stefano Di Battista`，`Charlie Parker` 與 `Kenny Barron` 都不會落在更近的位置。**
⚠ **句序因此與 `note` 的 `→` 鏈不同**（鏈是 錄音 → 曲目歸屬 → 樂評，正文是 曲目歸屬＋錄音 → 樂評 → 曲目來源）。
**裁定：`note` 的 `→` 是主故事的內容清單，不是正文的句序規範**；**內容一格不增不減的前提下，句序可為代名詞消歧而調整。**
判準第 2 條（可逆）與第 3 條（不調就會留下一個會誤讀成與 hook 矛盾的句子）。

---

## 第 3701 條（**跨批反同構：三張逐張比對已上架／已寫的同藝人卡**）

| 本張 | 同藝人的既有 `desc` | 已用掉的面向 | **本張的處置** |
|---|---|---|---|
| `A prima vista` | **c-171 b《Round About Roma》** | 1969 年生於羅馬、1992 年由 `Jean-Pierre Como` 帶去巴黎、法國國家爵士樂團、曲目作者比例、交響樂團編制 | **生平 0 字、樂團編制 0 字、曲目歸屬 0 字**（後者另因第 3683 條讓給《Parker's Mood》）。走廠牌與錄音室 |
| `Parker's Mood` | 同上 | 同上 | **生平 0 字**；走曲目作者歸屬＋錄音室 |
| `Spirit Talk` | **c-171 a《Spiritalk 2: Revelations》** | Tonight Show 樂團與音樂總監時序、九首自作、哥哥 `Robin Eubanks`、`Dave Holland` | **四項全部 0 字**（`Robin Eubanks` 與 `Dave Holland` 兩個名字在本張正文逐字 0 命中）。走錄音工法的減法 |

⚠ **`Spirit Talk` 的「九軌全是他自己寫的」本層 0 處**——研究層 `notes` 逐字要求下游不要寫（該句無一手來源），
**鉤子層第 3690 條已把整格拿掉，本層的 `note` 裡本來就沒有作曲資訊，形同第三道保險。**
⚠ **本張也沒有寫成器材說明書**（派工信第四節點名的風險）：正文 4 個句子裡，**只有第二句碰錄音工法**，
其餘三句分別是逐軌編制、樂評的聽感、榜位——**「這是一張唱片」的比重是 3 比 1。**

---

## 第 3702 條（**收工三支腳本的實跑結果，逐字**）

**`node qa-batch.mjs out c172`**（cwd `desc-tools`）：

```
out-1｜3 張｜字數 232–239｜>260: 0
out 合計 3 張，與卡單相符 ✓
（qa-check-research 與 fix-spacing 需另跑，共 1 檔各一次）
全部通過 ✓
```
**exit 0、旗標 0。** ⚠ **它印的「字數 232–239」與本層的 `Array.from` 實量逐格相同**，兩把尺一致。

**`node qa-check-research.mjs batches/input/c172-writer-1.json batches/output/c172-out-1.json`**：

```
QA 完成｜ 3 張｜標記 0
```
**exit 0。** ⚠ **「首句未原封開頭」這一關 3／3 通過**（第 1838-B 條記的那個唯一有效的硬檢查）。

**`node fix-spacing.mjs batches/output/c172-out-1.json --field desc`**（**預設 dry-run，本層未加 `--write`**）：

```
待補 0 張（欄位 desc）
```
**exit 0。⚠ 待補 0，因此 `--write` 不需要跑，本層也沒有跑——輸出檔未被任何腳本改寫過。**

**本層另跑的自檢**（腳本在 scratchpad）：key 與輸入**逐字同序同數 3／3**、
輸出欄位逐字只有 `key` 與 `desc`、首句與 `hook` 逐字相符 3／3、
字數 234／239／232（`<180` 0、`>240` 0）、跨張 4-gram 3 次以上 0 個。

---

## 第 3703 條（**與上游條件牴觸之處，以及交件版本認定**）

### （一）牴觸：**1 處實質（已依 base 處置）、1 處轉述精度**

| # | 上游原句 | 判讀與處置 |
|---:|---|---|
| **1** | **`c172-hooks-a.json` 第 3 張 `note` 逐字「樂評說全片有一種易於進入的空靈感」** | ⚠ **`writer-base.md` 逐字禁止用「全片」指整張專輯。** 這是 `note` 的指派句自身帶了 base 的禁語（源頭是研究層 `facts` 第 5 條的 Cash Box 引文）。**本層以 base 為準改寫成「它」，語意零損失。全文見第 3697 條。** ⚠ **給主線：下一條線若仍要引這段樂評，`note` 那一格建議先把「全片」換掉，免得每一棒都要重判一次。** |
| 2 | **派工信第三節「`c171-out-2.json` 是 10 張 min 192／max 233／中位 208」** | **min 與 max 逐格相符**；**中位 208 是上中位數**（10 張偶數筆，兩個中間值 205 與 208，平均 206.5）。**不是錯，是取法沒寫明**，本層第一次量完差點以為量尺不對。第 3692 條 |

**其餘逐條核對相符**：`desc` 目標 180–240 與硬上限 280／下限 140、`sound` 是唯一伸縮格、
不得撿回鉤子層刪掉的格、只寫 `note` 列出的東西、`note` 的警示字眼不進正文且不寫否定句、
三支腳本的跑法與 `fix-spacing` 預設 dry-run、裁定條號區間 3692–3720、
**只准動 `c172-out-1.json`（新增）與本檔（append）**。**0 處與 `writer-base.md` 原文牴觸**（第 3697 條那一處牴觸的是 `note`，不是 base）。

⚠ **派工信第三節那句「前幾批的實測是分歧的⋯不要憑『這批應該會偏長／偏短』去預留」——本層照辦，動筆前 0 上浮、寫完才量。**
**結果本批是兩張低估 8–13 字元、一張零偏差**，**與 c-171 a（雙峰）、c-171 b（一張撞下限）都不同向**，
**再一次證實方向不可繼承**（第 1838-B 條）。

### （二）交件版本認定

**以工作區當下的 `desc-tools/batches/output/c172-out-1.json`（3 筆）與本節為交件版**（第 1803-B／3617／3691 條）。
**「筆數對了」不等於「定稿了」**——本層依派工信「每寫完 1 張就整份寫回」，**整份寫回磁碟共 4 次**
（第 1 張建檔 1 次、三張合併 1 次、代名詞改寫與同義壓縮 2 次）；**其中第一個三筆齊全的版本有兩張超過 240，不是定稿。**
**定稿時點是：三張字數全部落在 180–240、`qa-batch out c172` 全部通過、`qa-check-research` 標記 0、
`fix-spacing` 待補 0、跨張 4-gram 無 3 次以上重複、首句與 `hook` 逐字相符 3／3，六者同時成立之後。**

⚠ **本節以 append 寫入，未覆寫 3601–3691 任何一行**（檔案第 1 行標頭仍逐字寫「a 組 **2 張**」，
**那是 3601–3617 那一輪留下的，第 3618 條已更新為 3 張**，本層照第 3691 條的先例**不改標頭**）。
**`seed_cards.json`／`apex_pool.json`／`PROJECT_MEMORY.md`／`previews.json`／`caa.json`／
其他批次的檔案／KV／Firestore 一律未碰；未 `git commit`／未 `git push`／未動 git 索引／未 `git add`。**
中間檔（`meas-c172-w1.mjs`／`ngram-c172-w1.mjs`／逐張量尺檔）全在 scratchpad 的 `c172w1/` 子目錄，**未進 repo**。
