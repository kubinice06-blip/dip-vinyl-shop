// c-92 提案檢查。用法：node batch-progress/c92/chk-prop.mjs [組別…]
...
const groups = process.argv.slice(2).length ? process.argv.slice(2) : ['a'];  // c-92 是單組補遺批
const all = [];
let bad = 0;
for (const g of groups) {
  const f = path.join(ROOT, `batch-progress/c92/prop-${g}.json`);
