...
⚠ ⚠ **`note` 沒警語不等於沒問題，四種看不見的形狀**：
**(a) 整個 RG 只建了再發** → 三欄一致、note 空白（c-181 的 `中本マリ` 兩張、`鈴木良雄《Touch of Rain》`，
年份與盤名一起錯，機器完全不報）。
**(b) RG 與 release 兩層同時被污染** → c-175 的 `牡牛座の詩` 實為《Canto of Aries》，**MB 連星座都錯**。
**(c) RG title 取了後來再發的題** → c-183 的 `尺八＆琴 男の世界 <ボサ・ノヴァ編>` 被 MB 取成 2010 再發題。
⚠ **(d) 原壓盤面自己印錯、Discogs 忠實轉錄、MB 與再發都印對**（2026-09-25，c-184 b 組加的）
→ `Something's Coming` 的原壓盤面拼成 `Comming`；
**反過來 `市原宏祐《Departure》` 是 MB 三筆全抄同一個錯字 `DEPARTRUE`、只有 Discogs 對。**
**→ 兩邊不一致時不能預設誰對，要看那個錯字出現在哪一層。**
⚠ ⚠ **(e) 等價形盤名（和文＝外文）在撞池比對上是獨立的坑**（2026-09-25，c-184 b 組抓到）
→ `阿部薫《なしくずしの死》` ＝ 池中的《Mort À Crédit》1976，
**盤面全題是 `Mort À Crédit = なしくずしの死`，兩邊各取了一半**，
**`chk-prop` 的複合鍵與 `dedup` 六道全部抓不到。比對撞池時兩邊都要試。**
**(a)(b)(c) 只能靠 Discogs master 的版本表抓；(d)(e) 要看盤面全題。**

## 一之二、⚠ 2026-09-22 新增的四條判準（主線第 1934-B／1936-B 條，**優先於本檔其他任何一節**）

### (1) 動畫／特攝／遊戲主題曲的**翻奏企劃線**：整線退
`Jam Trip`／`Digital Trip`／`シンセサイザーファンタジー` 這類企劃盤形狀固定：
**現成的動畫主題曲＋器樂翻奏＋爵士或合成器編制**。c-180 兩組各自獨立撞到、合計退 7 張零收；
c-181 又命中 1 張（`うる星やつら Vol.3`）。
**分界線只有一條：曲目來源。** 同一支樂團、同年、同廠、目錄號相差三號，
**`Roman Trip`（原創形象盤）收、`Jam Trip`（動畫翻奏）退**——
⚠ **差別不在演奏水準也不在編制**（班底同一組人），**在第 3753 條收件款的第三肢
「曲目是爵士標準曲或原創曲」：動畫主題曲兩者都不是。**
**→ 這一肢可以獨立退件，不必等 ①②④⑤ 任何一款成立。**
⚠ **`Digital Trip` 這條線做到 80 年代末**，1985 年以後的號段還會再撞。

### (2) 曲風判準 ⑤ 款的第三形：**`genres` 含 Jazz 但 `styles` 零爵士**
`genres` 逐字含 Jazz、**而 `styles` 陣列裡零個爵士成分**
（Fusion／Jazz-Funk／Bop／Modal／Free Jazz／Post Bop／Big Band／Soul-Jazz／Avant-garde Jazz 皆不見）
→ **不成立收件**，改由第 3716 條的三肢逐肢覆核；三肢皆不過才退。
⚠ ⚠ **`styles` 空陣列不在射程內**（第 1936-B 條）——**空陣列是「沒有資訊」，不是「沒有爵士」**，
一律回第 3762 條走人工判。

### (3) 掛名分隔符：**盤面明印才立，資料庫寫法不立**
`〜`(U+301C) 已收為池中第九種分隔符（`富樫雅彦〜高柳昌行` 盤面明印）。

### (4) 東芝EMI 盤的 Apple `releaseDate`：**只能佐證，不能推翻**
第 1907-B 條已裁：**King 盤是真發行日**（每月 21 日）、**コロムビア／NIPPONOPHONE 可信**（每月 25 日）、
**ビクター 常填錄音月或再發日**。**東芝 三個資料點兩正一反**（c-181 兩張逐日相符、c-178《風舞》差一年）
→ **與 Discogs／MB 相符時可寫進 `risk` 當交叉驗證；衝突時一律以 Discogs 原壓為準，Apple 那一邊整格不寫。**

⚠ **第 1924-B 條（四大廠硬門）已三次實地執行**（DOMO ×2、美國 TBA Records ×1，都是曲風那一關過得了的好盤）
——**四大廠指的是原壓廠牌**，「四家只出現在後來的 CD 再發上」不算。
...
5. ⚠ ⚠ **`why` 欄一律只當線索、不當結論**（第 1850-B 條，**這一段原本寫「`rg-tag` 最可信」，c-173 兩組實測推翻**：
   ...
   **理由**：Discogs 對日本盤的 genre 桶很粗，**尺八／琴／和太鼓演奏的爵士盤會被丟進去**。
   ...
   ⚠ ⚠ **在曲風之前先過一關：這張碟的原盤廠牌必須是本線四大廠之一**（第 1924-B 條）。
   **原壓不在 Victor／JVC、東芝 EMI／Express、日本コロムビア／Better Days、King／Paddle Wheel 的，一律退**
   ——**不管曲風多對。** 本線已經照這個理由退過五張
   （`URC`／`Discomate`／`ALM`／`DOMO・トリオ・ケンウッド`，外加一張外國藝人整團的日本企劃盤）。
   ⚠ **「四家只出現在後來的 CD 再發上」也算不在**。

   ⚠ ⚠ **「演奏主體是爵士編制」這一肢必須從盤面本身成立**（第 1925-B 條）——
   **伴奏陣容要算數，得印在盤面（credits／內頁）上；靠第三方商品頁還原的名單不算。**
   實例：`麻生小百合《キャンディ・ジャズ》` 的伴奏是 Mariah 三人＋土岐英史＋伊東たけし，
   **但盤面 credits 一個樂手都沒列 → 退**；
   `アンリ菅野《Just Groovin'》` 的近三十位日本爵士樂手**是逐名印在盤上的 → 收**。
   ...
   **收（爵士）**：Discogs `genres` **含** Jazz（或 Jazz/Rock）且 `styles` **不含** Easy Listening／Kayōkyoku，
   ...
   ⚠ ⚠ **「伴奏名單很強」不是收件款的任何一肢**（第 1923-B 條）——
   **本線已經三次有人拿它當收件理由，三次都不成立。**
   最極端的一次：`Hi‐Fi Set《1&2》` 的 credits 是**佐藤允彦 製作＋作曲＋編曲＋鋼琴一手四角、
   節奏組是日野元彦・岡沢章・高水健司・中牟礼貞則**——**仍然退**，
   因為 **Hi-Fi Set 是三聲部ソフト・コーラス，爵士樂手在這裡是伴奏**。
   **判準看的是「演奏主體是不是爵士編制」**，與《Summer-Time Love Song》（三位主唱）、
   《Sgt. Pepper's》（一個人彈鍵盤）同一條線。

   ⚠ **引用舊裁定時要確認它的前提還成不成立**（第 1923-B 條）——
   c-177 第 4387 條第 9 點寫「某兩個掛名不可合併」，**前提是「池中兩者皆 0 張」，
   而 c-177 自己就把其中一個立進池了。裁定的前提會被後續批次改掉。**

   ⚠ **跑完 `versions` 還要比一次 `catno=` 反查的 id 差集，抓 `master_id` 為 0 的孤兒 release**
   （第 1923-B 條，c-179 a 抓到一筆、少算一版）。
