// c-105 提案檢查。用法：node batch-progress/c105/chk-prop.mjs [組別…]
...
for (const g of groups) {
  const f = path.join(ROOT, `batch-progress/c105/prop-${g}.json`);
  ...
  p.forEach(x => {
    ...
    } else if (['EP', 'Single'].includes(x.releaseType) &&
               (x.genreException === 'asia-mini-album' || x.releaseTypeException === 'asia-mini-album')) {
      // §5.5 asia-mini-album 白名單（2026-09-06 c-105 加，形狀照 c-97 的 electronic 分支）：
      // 韓國的正規 EP／迷你專輯要收就得帶與 §5.6 同等的舉證，逐張說明為什麼這一張是核心經典。
      if (!x.exceptionReason || Array.from(String(x.exceptionReason)).length < 12)
        say(`§5.5 asia-mini-album 缺 exceptionReason（≥12 字）：${x.artist} — ${x.album}`);
      const urls = (x.exceptionEvidenceUrls || []).filter(u => /^https:\/\/\S+$/.test(String(u)));
      if (urls.length < 2) say(`§5.5 asia-mini-album 的證據網址不足兩個：${x.artist} — ${x.album}`);
    } else if (['EP', 'Single', 'DJ-mix'].includes(x.releaseType)) {
      say(`非 Album 未走 §5.5 白名單（缺 genreException=asia-mini-album）：${x.artist} — ${x.album}`);
