// c-53 韓國提案檢查。用法：node batch-progress/c53/chk-prop.mjs [組別…]
...
const groups = process.argv.slice(2).length ? process.argv.slice(2) : ['a', 'b', 'c', 'd'];
const all = [];
let bad = 0;
for (const g of groups) {
  const f = path.join(ROOT, `batch-progress/c53/prop-${g}.json`);
