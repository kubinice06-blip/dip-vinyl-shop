...
# c-167 寫作層 b 組（writer-2）裁定　號段 2621–2640

---

## 第 2621 條：**字數——7 張全數落在 180–240，實測 197–220、中位 210，零超標零破底**

| 卡 | 字元 |
|---|---:|
| `Ron Carter, Ricky Dillard《Sweet, Sweet Spirit》` | 197 |
| `Minyo Crusaders《From Japan With Love》` | 202 |
| `Nduduzo Makhathini《The Myth We Choose》` | 203 |
| `Walter Smith III《Twio Vol. 2》` | 210 |
| `Bill Frisell《In My Dreams》` | 211 |
| `Julian Lage《Scenes From Above》` | 217 |
| `Melissa Aldana《Filin》` | 220 |

⚠ **依派工信 §一第三點（第 1814-B 條）兩端都驗**：上緣離 240 有 20 字元、下緣離 180 有 17 字元。
**未預留上浮，也未假設會超標**——c-166 寫作層「零超標、五張撞破下限」的偏差方向本棒不繼承，
預算表逐張同時對照 240 與 180 兩端（`writer-base.md` 第 120 行）。

**唯一一次回頭補寫是 `Sweet, Sweet Spirit`**：初稿 183，離下限只剩 3 字元，
依 `writer-base.md` 第 82 行的補寫順序（主故事後續的第二個事實優先），
把 `note` 主故事鏈第三項裡本來被我省掉的**合唱團名 New-G** 補回來（`+14`）。
**補的是 `note` 已列的項目，不是從 `facts` 撈新料。**

---

## 第 2622 條：**首句與 `hook` 的對照——7 張全部逐字相符，零改寫**

`qa-check-research.mjs`（第 29 行，硬檢查）`hook 未原封開頭` **0 命中**。
本棒另以 `desc.startsWith(hook)`（**未做任何空白正規化**，比腳本更嚴）獨立驗一次，**7 張全過**。

⚠ **`hook` 已說過的話一律不重述**（`writer-base.md` 第 159 行），三張特別處理：
1. `Nduduzo`：`hook` 已寫「電子聲響全是那個少年的主意」→ 正文**不重講電子**，
   只補「少年是誰」（Thingo Makhathini）與 `facts` 那句話的**另一半**（大家不會聯想到他的節奏也出自兒子）。
2. `Melissa Aldana`：`hook` 已寫「filin ＝古巴人唸 feeling」→ 正文**不重講字源**，直接給 filin 的年代與體裁定義。
3. `Julian Lage`：`hook` 已寫「五軌底下藏著第五位樂手」→ 正文只給名字與樂器，不重述「藏」這件事。

---

## 第 2623 條：**`Julian Lage` 的 Patrick Warren 照 `note` 寫進正文（派工信 §三第 4 點）**

三篇官網稿與策展層都漏了他，研究層從 Discogs 逐軌 credits 補回。**本棒照 `note` 寫，未打折**：
具名 `Patrick Warren`、寫明橫跨五軌、四樣音色（dulcitone／Chamberlin／鐘／弦樂）全列。

⚠ **代價是這張的專名衝到 6 個**（Patrick Warren、dulcitone、Chamberlin、John Medeski、Jorge Roeder、Kenny Wollesen、Blue Note），
遠超 `writer-base.md` 第 289 行給爵士批的「3 個上限」。**本棒判：上限讓位給 `note` 的主故事層。**
理由是那條上限的立法目的是**控字元**（第 271 行逐字「動筆前就把這張要點名的拉丁專名壓到 4 個以內」的下一句是「超過的部分改用中文描述」），
而本張實測 217、離上限 23 字元，**立法目的已達成，上限本身沒有獨立價值**。
**可逆**（要壓就整格砍掉四重奏三人名，改寫成「一個全新四重奏」），**有先例**（c-166 b 組同樣以實測字元數壓過專名計數）。

⚠ **被讓掉的是「四重奏三人各自的樂器」以外的一切**：〈Night Shade〉的管風琴段落、
日版多收的〈Aberdeen〉、Joe Henry 的製作、Sear Sound、SFJAZZ 駐館、Medeski 的那段話——**整格捨去，一項未寫。**

---

## 第 2624 條：**`Sweet, Sweet Spirit` 的「公有領域／傳統聖詩」封口落實方式（派工信 §三第 5 點）**

實掃輸出檔：`公有領域` 0 命中、`傳統聖詩` 0 命中。

正文採用的唯一說法是研究層點名可用的那一句——**「她最愛的十首聖詩」**（`facts` 第 3 項逐字有來源）。
⚠ **本棒另外避開兩個近義陷阱**：沒寫「老詩歌」、沒寫「古老的」、沒寫任何暗示作者不詳或年代久遠的修飾語
——策展層第 2032 條（四）那句查無來源的斷言，**改寫成同義詞一樣是無來源**。

⚠ **逝者克制**（研究層 `researchNotes` 要求）：母親只寫兩件事實——當年搬進全日照護機構、整張紀念她已故；
**未寫辭世時間、病況或任何渲染**。具名 `Mrs. Willie O. Carter` 是 `note` 逐字列的，照寫。

---

## 第 2625 條：**〈Isfahan〉兩張整格不寫，`Ron Carter` 撞陳列兩張分得開（第 2575 條、研究層）**

1. **〈Isfahan〉**：實掃輸出檔 `Isfahan` **0 命中**，`Frisell` 與 `Walter Smith III` 兩張都沒有。
   **未從 `facts` 補回**（`facts` 兩張都有這首、且 Frisell 那張還帶一句他談 Strayhorn 的話，全部整格捨去）。
2. **`Ron Carter` 撞陳列**：他同時是 `Sweet, Sweet Spirit` 的共同掛名與 `Twio Vol. 2` 的客座。
   兩張的素材各自取自自己那張的 `note`，**零共用句構**——
   前者寫他為母親做的紀念盤與「合唱團成為樂器、貝斯成為敘事者」，後者只寫他進了十軌中的五軌與錄音室裡那句話。
   **`qa-batch out c167` 的 `互指?` 0 行。**

---

## 第 2626 條：**`Bill Frisell` 的混合盤紅線——「現場專輯」與「錄音室專輯」兩詞都沒寫**

研究層紅線：MB 的 `secondary-types` 是 `["Live"]`、slice 的 `live` 欄 true，兩者都會誤導。
正文的寫法是**只陳述兩件事實、不下分類標籤**：核心軌錄自 2025 年的三場音樂會；
部分曲目再由 Adam Muñoz 在柏克萊的 Opus Studios 補上整段樂句與音景（**點明補的是樂句與音景，不是修音**）。
碟本身只稱「第五張 Blue Note 專輯」——**中性詞，不含 live／studio 任何一側的判定。**

⚠ **三場音樂會的逐場地點與日期整格捨去**（紐哈芬 Firehouse 12／布魯克林 Roulette Intermedium／丹佛 The Newman Center，
三個場館名合計 50 字元以上）。**〈Isfahan〉之外，夢境那段（Nino Rota 以下八個人名）、《紐約客》的評語、
2LP 的三種版本、Lee Townsend／Sterling Sound 一整排 credits 也全部整格捨去。**

---

## 第 2627 條：**`note` 的「正文只寫上列各項」逐張落實——7 張的整格捨去清單**

⚠ **依派工信 §三第 2 點（第 2606 條）：這是排除條款、不是必寫清單。**
本組 7 張**全部**做了整格捨去，捨後主故事完整、`hook` 懸念皆收尾：

| 卡 | 捨去的整格 |
|---|---|
| `Minyo Crusaders` | 八軌的都道府縣逐軌對照（8 個地名）、朝倉さや、〈花笠音頭〉與〈こきりこ節〉等全部軌名、田中克海／フレディ塚本／Koichiro Osawa 三人、Tiny Desk 與四個音樂節、Iggy Pop |
| `Nduduzo Makhathini` | 常設三重奏兩人＋換鼓手兩軌、Shabaka Hutchings／Black Coffee 等七位客座、四首先行曲與日期、〈Ḽiṅwalo ḽa Mubebi〉三人共同署名、盤名的內襯文原文、Peter Auret 一整排 credits |
| `Ron Carter, Ricky Dillard` | 十軌逐字曲名清單、同名曲先行單曲與 2025-12-05 預告、兩人各一句的表態、「Maestro／Choirmaster」綽號、Carter 的生平（大提琴改貝斯、Eastman、Chico Hamilton 以下一整串） |
| `Bill Frisell` | 見第 2626 條 |
| `Walter Smith III` | 十軌作者逐條清單、〈Casual – Lee〉的和聲來歷、Carla Bley 與 Terri Lyne Carrington 的繞路、Joe Sanders／Kendrick Scott、Village Vanguard 駐場與六位換班客座、三首先行曲、第一輯的編制與客座 |
| `Melissa Aldana` | Peter Washington／Kush Abadey／Cécile McLorin Salvant、〈Little Church〉與 Hermeto Pascoal／《Live-Evil》一整段、《Ballads》參照與七位抄譜對象、她談吹法的那段話、Oktaven Audio 一整排 credits |
| `Julian Lage` | 見第 2623 條 |

**捨去量最大的兩張是 `Minyo Crusaders`（8 個都道府縣＋8 首軌名＋3 位團員）與 `Walter Smith III`（10 軌作者清單）**
——與派工信 §三第 8 點的預告一致。

---

## 第 2628 條：**獎項本組一字不提（派工信 §三第 7 點）——實掃 0 命中**

`葛萊美`／`Grammy`／`Monk`／`入圍`／`得獎` **全部 0 命中**。
`Aldana` 的 Monk 大賽與 2019 年《Visions》葛萊美入圍**整格不寫**，
`Julian Lage` 前作《Speak to Me》的入圍**整格不寫**（`Speak to Me` 字串本身也 0 命中）。
**未補任何一項。**

---

## 第 2629 條：**引語一律轉間接敘述，`Walter Smith III` 那句同時解掉 `qa-check-research` 的禁「你」**

`writer-base.md` 第 163 行要求直接引語轉間接敘述；本組 4 處引語全部轉了
（Ry Cooder、Makhathini 談兒子、Smith 談選曲、Ron Carter 在錄音現場）。

⚠ **`Ron Carter` 那句是雙重必要**：原文逐字「別只彈**你**想彈的。聽**我**在做什麼。」
`qa-check-research.mjs` 第 12 行的 `BANNED` 逐字含 `(?<![迷你])你(?!的)`，**直接引用會硬性標記**；
`writer-base.md` 第 168 行的「禁你我」也擋。**轉成「Carter 在錄音現場要他別只彈自己想彈的，先聽對方在做什麼」，兩條同時解掉。**
記給後批：**爵士批的錄音室軼事極常是第二人稱祈使句，這是固定會撞的一處。**

---

## 第 2630 條：**結尾落點——7 張全部落在事實，`Nduduzo` 那張的 Wayne Shorter 準則判定為事實非格言**

`writer-base.md` 第 162 行禁格言／評語式收語。六張的收尾分別是：編曲歸屬、母親姓名、生日音樂會、
錄音室裡的一句指示、製作人、第五張 Blue Note 專輯——**均為事實。**

⚠ **`Nduduzo Makhathini` 收在「照自己希望世界成為的樣子去演奏與寫作」，形式上像格言，本棒判可留。**
1. **有先例**：該禁令的例句是「換來的是評價，不是銷量」——**寫作者自己下的評語**；
   本句是 **`note` 主故事鏈的第四項、逐字指定的收尾**，且是 Makhathini 轉引 Wayne Shorter 的**創作準則事實**，
   歸屬明確（「他把 Wayne Shorter 的話當創作準則」），不是本棒對這張碟的評價。
2. **可逆**：要換就改收在十六軌／2026 年發行，成本是卡單值一個欄位。
3. **卡住整條線**：不判就沒得收尾——`note` 只給四項，前三項都已用掉。

---

## 第 2631 條：**三支自檢的結果——與本組相關的旗標全數 0**

```
node qa-batch.mjs out c167
  out-2｜7 張｜字數 197–220｜>260: 0
  ⚠ 輸出總張數與卡單不符 7 vs 14
  總標記 1
node qa-check-research.mjs batches/input/c167-writer-2.json batches/output/c167-out-2.json
  QA 完成｜ 7 張｜標記 0
node fix-spacing.mjs batches/output/c167-out-2.json --field desc
  待補 0 張（欄位 desc）
```

⚠ **唯一那個標記是 writer-1 未交件造成的，不是本組的**——派工信 §二已預告
（`out-1` 不存在時 `outTotal 7 !== cardKeys.size 14`）。**本組自己的四道全清**：
`key 不在卡單` 0、`未具名出處?` 0、`desc → 編造` 0、`互指?` **0 行**（無須人工判讀）。
`fix-spacing` 回報 `待補 0`，**因此未加 `--write`**（第 1740-B 條：預設 dry-run，本次無須真改檔）。

---

## 第 2632 條：**落地方式與重啟續跑（派工信 §四第 1 點）**

開工時 `batches/output/c167-out-2.json` **不存在**，無既有內容可續。
草稿存在 `scratchpad/c167b-w2-draft.json`（**加了批次與組別前綴**，第 1758-B 條），
7 張一次寫齊後整檔寫進 repo 輸出檔，其後只做過第 2621 條那一處補寫（草稿與輸出檔同步整檔重寫）。

⚠ **依第 1803-B 條明說：`batches/output/c167-out-2.json` 的工作區當下版本才是交件版**，
上表七個字元數與三支自檢的輸出都量自該版本，**沒有任何更新的草稿留在 scratchpad 未落地。**

---

## 第 2633 條：**派工信與 `writer-base.md` 正本或既有裁定牴觸之處——1 處（第 1725 條要求回報）**

⚠ **派工信 §三第 4 點逐字「`Julian Lage《Scenes From Above》` 的第五位樂手 Patrick Warren ⋯**照 `note` 寫**」，
與 `writer-base.md` 第 280、289 行的「爵士批動筆前先把專名壓到 3 個以內」直接衝突**——
照 `note` 寫這一格，這張的專名最少 6 個，不可能壓到 3。

**本棒判：照派工信與 `note`，專名上限讓位**（理由見第 2623 條：那條上限的立法目的是控字元，本張實測 217 已達成）。
**這不是派工信寫錯，是 `writer-base.md` 的專名上限少寫了一句例外**——建議後批把第 289 行補成
**「3 個上限的例外：`note` 指定為主故事層的專名不計入，但該張實測字元數必須落在區間中段（≤225）」**。

⚠ **派工信其餘各句與 base 檔、既有裁定均無牴觸**；§二關於 `out-1` 缺檔會誤報的預告**實跑完全命中**（見第 2631 條）。

**本節實際用掉 2621–2633，2634–2640 未用。**

---

## 第 2634 條：**補記——writer-1 交件後複跑 `qa-batch out c167`，全批 14 張零標記**

第 2631 條那唯一一個標記（`輸出總張數與卡單不符 7 vs 14`）在 `c167-out-1.json` 落地後**自行消失**，
本棒複跑一次確認：

```
out-1｜7 張｜字數 213–233｜>260: 0
out-2｜7 張｜字數 197–220｜>260: 0
out 合計 14 張，與卡單相符 ✓
全部通過 ✓   (exit 0)
```

**全批 14 張零標記、`互指?` 0 行**，兩組的字數帶都落在 180–240（a 組 213–233、b 組 197–220）。
⚠ **兩組偏差方向一致偏低**（a 組貼中上、b 組貼中下，**無一張觸及任一端**）
——但依 `writer-base.md` 第 117 行，**這個方向不得給後批繼承**。
⚠ **本棒全程未讀寫 `c167-out-1.json`**，上列數字出自 `qa-batch` 的統計輸出。

**本節實際用掉 2621–2634，2635–2640 未用。**

---

# c-167 a 組（7 筆）**寫作層**裁定　號段 2601–2620

（本節一律 **append 於檔末**，未覆寫、未改動既有的 2020–2035／2441–2455／2541–2547／2571–2578 任何一行——第 1806-B 條；
開工前本檔 1280 行、四節。暫存檔一律 `c167w1-` 前綴（`c167w1-draft.json`／`c167w1-spacetest.json`），未使用通用檔名。日期 2026-09-19。）
...
| # | 卡 | `desc` 字元 | 首句＝`hook` |
|---:|---|---:|---|
| 1 | `Charles Pasi《Adamas》` | 233 | ✓ |
| 2 | `Gabrielle Cavassa《Diavola》` | 233 | ✓ |
| 3 | `Immanuel Wilkins《…Vol. 1》` | 232 | ✓ |
| 4 | `Immanuel Wilkins《…Vol. 2》` | 232 | ✓ |
| 5 | `Immanuel Wilkins《…Vol. 3》` | 231 | ✓ |
| 6 | `Kiefer《Memory Bomb》` | 231 | ✓ |
| 7 | `Meshell Ndegeocello《Synonym》` | 213 | ✓ |

**7 張全部落在 180–240，min 213、max 233、中位 232；0 張超標、0 張撞破下限。**
三支自檢全綠（`qa-batch out` 全部通過 ✓、`qa-check-research` 標記 0、`fix-spacing` 待補 0）。

---

## 第 2602 條：**鉤子層 217–227 的預算與寫作層實測 213–233 的落差，以及落差怎麼來的**

鉤子層（第 2542 條）替 a 組 7 張估的是 **217–227、中位 225**；本棒成品是 **213–233、中位 232**。
**逐張比對，落差不是估錯，而是寫作層加了三類鉤子層預算表沒有的字**：

1. **連接詞與指代詞的黏著成本**（每張約 5–12 字元）：預算表數的是「格」的內容，
   實際行文要補「那是」「便由」「也讓」「則挑」這類接縫。
2. **具名取代代名詞**（第 2604 條，3 張各 +5 至 +6）。
3. **未發行卡的時態字**（第 2605 條，2 張各 +2）。

⚠ **裁定：鉤子層的預算表對寫作層是「下界」不是「等值」，後批的鉤子層若要把估值當成 desc 終值，
應在四格之外另留 10–15 字元的接縫額度。** 本棒不回頭改鉤子層（可逆性低於改自己的稿，且成品已在區間內）。

---

## 第 2603 條：**7 張全部走「整格捨去」，0 張靠逐句削字——實際捨去的三格**

第一版 7 張實測 `234,234,242,235,231,229,214`：**只有 1 張（Vol. 1，242）超標，其餘 6 張第一版即在區間內。**
**方向與鉤子層（第 2542 條（三）：第一版 6 張超標、幅度 5%–35%）完全不同**——
再次證實第 1814-B 條：**同一批、同一組卡，鉤子層與寫作層的偏差幅度不可互相繼承。**

壓下來與騰出額度的捨去共三格，全部是整格：

| 卡 | 捨去的整格 | 省下 |
|---|---|---:|
| `…Vol. 1` | **「在 Village Vanguard」這個地點狀語**——盤名《Live at the Village Vanguard, Vol. 1》本身已經逐字帶著場館名，句中再點一次是同一個專名寫兩遍 | 19 |
| `…Vol. 2` | **全長五十七分二十七秒整格**（本卷的辨識點是「唯一收五軌的一卷」，時長是第二層） | 11 |
| `Charles Pasi《Adamas》` | 第一版就沒進草稿的名單型六格，沿用第 2543 條 | — |

---

## 第 2604 條：**三處「他」改成具名——`Alice Coltrane` 那一處是真歧義，不是保守**

`writer-base.md`「代名詞與所指對象不可相隔太遠，寧可具名」的落實，本棒逐張掃過每一個「他／她」：

1. ⚠ **`…Vol. 1` 末句原寫「這是他生涯第一張現場專輯」，而前一句的句尾專名是 `Alice Coltrane`。**
   **這不是距離問題，是指錯人的問題**（Coltrane 是女性、且該句主詞就是她）——改成「這也是 Wilkins 生涯第一張現場專輯」。
2. `…Vol. 2` 的「五軌全是他的原創」離開頭的 `Immanuel Wilkins` 隔了兩個子句與一個盤名，改成具名。
3. `…Vol. 3` 的「出自他以 Bach⋯」**維持代名詞**：該處「他」出現在 `Bach` 之前，中間沒有任何其他人名，
   前指對象唯一。**具名要 +6 而該張已 231，不值得。**

⚠ **裁定：代名詞的判準是「中間有沒有別的人名」，不是「隔了幾個字」。** 三張同族卡因此做出不同處置。

---

## 第 2605 條：**兩張未發行卡的時態——改寫成「定在 X 月 X 日發行」**

`Kiefer《Memory Bomb》`（官方發行日 2026-09-25）與 `Meshell Ndegeocello《Synonym》`（2026-10-02）
在本棒交件日 **2026-09-19 都還沒上市**（研究層兩張的 `researchNotes` 都逐字標了未發行）。

初稿寫成「這張 2026 年 9 月 25 日發行的碟」「2026 年 10 月 2 日發行」，**讀起來是已經發行。**
改成 **「這張碟定在 2026 年 9 月 25 日發行」「定在 2026 年 10 月 2 日發行」**，各 +2 字元。

⚠ **裁定（可逆、且本批有先例可循）：未發行卡的發行日一律寫成「定在⋯發行」。**
理由是卡片會在發行日前後被抽到，**「定在」在兩個時點都讀得通，「發行的碟」只有一個時點讀得通。**
（`note` 只指定「發行日寫 X」，沒有指定時態，所以這不是違反 `note`。）

---

## 第 2606 條：**三張 Wilkins 的「兩晚合錄」上限逐張守住，且三張都沒有把「分三卷發」當主故事**

- **三張的正文對錄音日期只寫到「2025 年 5 月 15 與 16 日兩晚」這一層**；
  實掃三張 `desc`，`第一晚`／`第二晚` **0 命中**，**沒有任何一張出現分卷與夜次的對照**（第 1813-B／2442／2545 條）。
- **`…Vol. 3` 的「第一個錄音夜」承自 `hook` 原文**，指的是 2025-05-15 這個日期本身（發行日撞上它的一週年），
  **不是在指派本卷來自那一晚**——與第 2545 條第 2 點的判讀一致。
- **「同一批錄音分三卷發」在三張裡都只佔一個交代句，主故事鏈各自是**：
  Vol. 1 ＝唯一的實體卷＋那張實體只收本卷四軌＋唯一的外來曲〈CHARANAM〉；
  Vol. 2 ＝連載的中間一棒＋唯一的五軌卷＋Tiny Desk 點名本卷；
  Vol. 3 ＝完結篇撞上錄音一週年＋整套最長的一軌＋Bach 平均律那組無標題作品。
  **與第 2544 條（三）的分工表逐格對上。**
- **`Vol. 1` 只寫「CD 與 2LP 收的都是本卷自己的四軌」**，**未寫另兩卷是數位專屬**（`hook` 的「三卷之中只有這一卷壓成實體」已經涵蓋，
  再寫一次是重述 `hook`）；`Vol. 2`／`Vol. 3` 各自寫「只出數位」。

---

## 第 2607 條：**三筆封口全部生效——`Kiefer` 的葛萊美、`Cavassa` 的並列、`Pasi` 的《Zebra》年份**

1. **`Kiefer《Memory Bomb》`**：成品實掃「葛萊美」「Grammy」「Malibu」「Ventura」**全部 0 命中**。
   **既沒有補寫，也沒有寫成否定句**——第 2546 條第 1 點的「沉默＋封口」在寫作層原樣落地。
   ⚠ **記一筆給後批：封口之所以成功，是因為 `note` 裡一個字都沒有。**
   若 `note` 寫的是「不要寫葛萊美」，寫作層會看到「葛萊美」三個字並被迫處理它——那才是校對痕跡的來源。
2. **`Gabrielle Cavassa《Diavola》`**：成品逐字是「那是該賽九屆以來第一次出現並列冠軍」。
   ⚠ 初稿寫成「拿下⋯冠軍，是該賽九屆以來第一次出現的並列冠軍」，**「拿下冠軍」與「並列」連讀語意打架**，
   改成「在⋯奪冠，那是該賽九屆以來第一次出現並列冠軍」。**兩版都沒有「獨得」，但後者不會被誤讀成獨得。**
   她本人與本張的葛萊美一字不提（第 2546 條第 2 點）。
3. **`Charles Pasi《Adamas》`**：成品實掃 `Zebra` **0 命中**，只寫「在 Blue Note 的第三張碟」——
   **年份疑義（研究層記的 2020 vs 2021）不進正文，留給本機覆核。**
4. **三張 Wilkins 的 `GRAMMY-nominated` 整格不提**（第 2546 條第 4 點），成品實掃 0 命中。

---

## 第 2608 條：**`Meshell Ndegeocello《Synonym》` 的「撞陳列」處置——翻唱身分靠 `hook` 講清楚**

研究層警告十五軌的原唱者（`Chaka Khan`、`Bob Dylan`、`The Human League` 等）多半在池中另有卡，
**下游不可把翻唱寫成她的原創。**

本棒的落實：**`hook` 原文「十五首全是別人的老歌」已經把翻唱身分放在第一句**，
正文只再點一首具體的（末軌翻 `Bob Dylan`），**其餘原唱者一個都不點名**——
**既守住翻唱身分，也不把別張卡的主角拉進本卡**。十九人客座名單維持第 2543 條的整格捨去。

---

## 第 2609 條：**本組的專名密度與 `writer-base.md` 那條「壓到 3–4 個以內」的實際落差**

`writer-base.md` 對爵士批次要求「動筆前把拉丁專名壓到 3–4 個以內」。**本組有 2 張做不到，且本棒判定不該做到**：

- **`Charles Pasi《Adamas》` 有 8 個**（`Charles Pasi`／`Blue Note`／`Nino Pasi`／`Maurice Suissa`／
  `Nina Pasi`／`Lisa Suissa`／`Studio Pigalle`／`Studio Saint Germain`）。
  ⚠ **其中四個人名就是 `hook` 的懸念本身**——「唱的人與他們同姓」這句話，**不點名四個人就收不了尾**。
- **三張 Wilkins 各 3–5 個**，其中盤名《Live at the Village Vanguard, Vol. N》單一專名就 38 字元
  （`note` 逐字要求「提到盤名時帶上掛名與卷號」，見第 2545 條第 3 點）。

⚠ **裁定：專名上限是預算控制手段，不是內容規則。**
判準照 `writer-base.md` 的精神——**壓專名的目的是讓字數估得準，而字數實測已在區間內時，
撐著主故事的專名不砍。** 真正該砍的是名單型專名（第 2543 條已在鉤子層砍完），本棒未再加回任何一個。

---

## 第 2610 條：**`互指?` 判讀：`qa-batch out c167` 全批 0 行**

`out-1` 與 `out-2` 合跑，**`互指?` 一行都沒有、`desc 互指 N 處` 那行未出現、`desc 未比對` 0 張**。
不需要逐筆人工判讀（第 1763-B 條的判讀程序本批用不上）。

⚠ **與第 2544 條（一）預測一致**：鉤子層在 `note` 階段就沒讓任何一張提到同批別張的盤名，
**寫作層只展開 `note`，互指自然不會憑空長出來。**

---

## 第 2611 條：**派工信與 `writer-base.md` 正本或既有裁定牴觸之處——共 2 處，都不影響成品**

1. ⚠ **`writer-base.md` 第 3 行逐字寫「工作目錄一律 `C:\Users\User\dip-vinyl-home\desc-restyle`」，
   本棒實際的工作目錄是雲端 Linux 的 `/home/user/dip-vinyl-shop/desc-tools`。**
   **這不是派工信寫錯，是正本那一行只描述本機產線**；派工信 §二的路徑（repo 根相對）才是雲端的實況。
   **記下來給後批：`writer-base.md` 第 3 行對雲端代理無效，其餘各節全部有效。**
2. ⚠ **派工信 §四第 2 點要求「`node qa-batch.mjs out c167` ⋯writer-2 若還沒交件，那支會報『輸出總張數與卡單不符』」。**
   **本棒開跑時 `c167-out-2.json` 已經存在（writer-2 先交件，7 張、197–220），
   所以那個預告的標記一次都沒出現，實跑逐字是「out 合計 14 張，與卡單相符 ✓」。**
   **不是牴觸，是派工信寫在 writer-2 交件之前**——寫下來免得後批複核時以為漏跑了。

⚠ **不是牴觸、但值得記的兩處**：
- 派工信 §三第 2 點提醒「『正文只寫上列各項』是排除條款，不是必寫清單」——**本棒據此在 `…Vol. 2` 捨去時長一格**（第 2603 條），
  **捨後主故事完整、`hook` 懸念收尾**，與第 2606 條的分工表一致。
- 派工信 §三第 1 點警告「不要預留上浮，也不要假設會超標」——**實測第一版 7 張只有 1 張超標**（第 2603 條），
  **警告方向正確**：若照 c-166 鉤子層的經驗預留上浮，本組會有數張掉到下限。

---

## 第 2612 條：**落地方式與重啟續跑（派工信 §四第 1 點）**

開工時 `desc-tools/batches/output/c167-out-1.json` **不存在**，無既有內容可續。
草稿全程存在 `scratchpad/c167w1-draft.json`（每一輪修改整檔寫回），**量到 7 張全部落在區間後一次寫進 repo 輸出檔**；
其後的兩處定點修改（第 2605 條的時態）**直接改 repo 檔並重跑三支腳本**，並把 repo 檔回拷成草稿，讓兩份保持同步。

⚠ **交件版＝工作區當下的 `desc-tools/batches/output/c167-out-1.json`**（第 1803-B 條：「筆數對了」不等於「定稿了」）。
**本節實際用掉 2601–2612，2613–2620 未用。**
