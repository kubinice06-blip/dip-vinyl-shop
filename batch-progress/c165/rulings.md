...
## c-165 writer-2（b 組 22 張）裁定 2091–2114

- **2091**　派工信第一節要求對照 `writer-base.md` 第 158 行（首句＝hook 原文一字不改）。逐字比對後確認**本派工信沒有把它講反**，全信亦無與 base 檔或既有裁定牴觸之處；22 張一律以 hook 原文開頭，逐字相符 22／22。
- **2092**　`note` 尾端的「這條骨架全批只走本張。」與「正文只寫上列各項。」判為指派／排除句，不入正文（b 組十張帶骨架歸屬句者全數照此處理）。
- **2093**　研究稿 `facts` 與 `hook`／`note` 不一致時以後者為準（派工信第三節第 5 點）。b 組實際只有一處需要判：《Bells on Sand》note 寫「十軌裡有五軌不是他寫的」，`facts` 的作曲欄逐項相加（Mompou 兩軌、〈My Ideal〉兩軌、末軌 Jeff Clayton）正好是五軌，兩者其實一致，採 note 表述。其餘鉤子層更正過的六處（軌數、時長下界、樂器、鍵盤 vs 鋼琴等）在正文採用的欄位上與 `facts` 無衝突。
- **2094**　《Squint》鼓手兩形（Discogs 逐字 David King、官網逐字 Dave King），note 用 Dave King，正文採 note 形。
- **2095**　《Grateful Deadication》Don Was 在〈Loser〉彈貝斯只見於 `facts`、不在 note 的主故事鏈，依「正文只寫上列各項」不寫；只寫 note 給的「1981 年同團錄過音」。
- **2096**　《In the Spirit of Ntu》Ubuntu 那句英諺的中譯含第二人稱單字，會撞 `qa-check-research.mjs` 的禁語正則（`「」` 不在剝除範圍內，引號包起來也擋不掉）。改成不直引、轉述成「人因著彼此而存在」，note 的「集體」語意完整保留。
- **2097**　《Reboot》的廠牌線素材已由鉤子層整格捨去，**不補回**；正文只走 Mystic Brew 出版公司那一條主故事。
- **2098**　字數補足一律用 `sound` 欄的聲音描述，**不加第二個故事**（一張卡只講一個主故事）。實際動用五張：《Homeward Bound》《View With a Room》《The 7th Hand》《12 Stars》《Live: Cookin' with Blue Note at Montreux》各補一句聲音格。
- **2099**　《Reboot》初稿 241 撞上限，依 base 檔「整格捨去」砍掉整句聲音格（不逐字削），落 213。全批唯一一次超標。
- **2100**　三張《Trios》嚴守研究稿的分離要求：場地、錄音年、同台樂手三組事實各自獨立，無一張互串；三張的主故事分別是「場地容不下鼓」「空場直播」「父子身分的巧合」。
- **2101**　《Succession》依 note 指示把〈Watarase〉寫明為板橋文夫原曲的翻奏；東亞音樂人照原文漢字（森山威男、沖野修也、板橋文夫）。
- **2102**　《Today Is Another Day》掛名照卡單藝人欄逐字寫 山中千尋，不改 Chihiro Yamanaka（base 檔「本卡自己的藝人名照卡單」那條）。
- **2103**　《Purest Form》依 note 指示把〈My Favorite Things〉寫明為翻奏，避開與 seed 同名碟的折鍵同鍵。
- **2104**　榜單與獎項整條不寫：本組多張的葛萊美入圍、Thelonious Monk 大賽名次等都不在 note 的主故事鏈上，依「拿不到可具名出處就整條不寫」與「正文只寫上列各項」雙重理由排除。
- **2105**　樂評媒體名與樂評人姓名不進正文：《The 7th Hand》前作的年度榜評語、《Revival》內頁文字作者、《Trios: Sacred Thread》facts 裡當年的樂評引語，三處全數不寫；本組每張都另有足夠的主故事，不觸發 2026-08-09 的例外二。
- **2106**　時長沿用 note 的中文數字寫法（一分五十六秒、九分五十秒、二十六分鐘、七分半），年月日、軌序與軌數用阿拉伯數字或中文數詞照 note 原形。
- **2107**　《Where Are We》盤名三形（MB／Discogs 大寫、Apple 全小寫），卡單取 MB 形；正文未提盤名，無實際影響。
- **2108**　《Four》盤名撞四張 seed 卡，研究稿指定的辨識點是「沒有貝斯的四重奏」與 2022 年 Blue Note。此屬下游辨識的硬需求，**破例把 note 未列的「沒有貝斯手」一句寫進正文**（來源在 `facts`，非外部知識），並保留 2022 年 11 月 11 日的發行日。
- **2109**　《Revival (Live at Pookie's Pub)》依 note 指示寫成「1967 年的現場在 2022 年首度發行」。
- **2110**　《Live: Cookin' with Blue Note at Montreux》依 note 指示寫成「1973 年的現場在 2022 年發行」，年份不取 Apple 的 2013 佔位日。
- **2111**　開頭一律照 hook 原文，前四字互異與跨組反同構已在鉤子層結清，本層**不改動任何一句首句**。
- **2112**　逝者相關依克制原則：《Passage》《Homeward Bound》《Lifted》《Reboot》四張只寫題獻與作曲歸屬，不延伸事件細節。
- **2113**　交件版本認定：以**工作區當下的 `batches/output/c165-out-2.json`** 為交件版（第 1803-B 條：本線曾三次被中途檢查點撈走未定稿的版本）。
- **2114**　`互指?` 逐筆判讀：`node qa-batch.mjs out c165` 對 out-2 **沒有印出任何 `互指?` 行**（xref 0 處），因此無逐筆判讀項目；三支自檢旗標全 0。
