# c-153 a 組裁定（編號 1200–1229）

本檔為 **c-153 a 組**（23 筆）策展層的裁定，編號 **1200–1212**（派工信給的區間是 1200–1229，**1213–1229 未用，留給 b 組之外的追記**）。
本段直接接續 **c-152 的第 1140–1187 條**（1994–95 那一批），**判準沿用、只在第 1206 條把第 1173 條的界線再切細一刀**。

---

## 第 1200 條（c-153 a 交件）：**23 筆 → 收 19、退 4；四張退件全部是「1957／1969／1970／1981-85 年母體的 CD 化或合輯化」，`chk-prop` 一個燈都沒亮**

- **收 19 張、18 位**（`Denise Jannah` 吃兩張，其餘一位一張）：
  `Lonnie Smith —《Live at Club Mozambique》(1995, live, 庫存首發)`、`Bill Stewart —《Snide Remarks》(1995)`、
  `Kevin Hays —《Go Round》(1995)`、`John Scofield —《Groove Elation!》(1995)`、
  `Denise Jannah —《I Was Born in Love With You》(1995)`、`Ron Carter —《Mr. Bow-Tie》(1995)`、
  `Jacky Terrasson —《Reach》(**1996**)`、`日野皓正 & 菊地雅章 —《Acoustic Boogie》(1995)`、
  `Eliane Elias —《Solos and Duets》(1995)`、`Cæcilie Norby —《Cæcilie Norby》(1995, 同名)`、
  `Denise Jannah —《Different Colours》(1996)`、`Dianne Reeves —《The Grand Encounter》(1996)`、
  `Michel Petrucciani —《Live at the Village Vanguard》(**1985**, live)`、`US3 —《Broadway & 52nd》(1996)`、
  `Renee Rosnes —《Ancestors》(1996)`、`Big John Patton —《Memphis To New York Spirit》(1996, 庫存首發)`、
  `Cassandra Wilson —《New Moon Daughter》(1996)`、`Richard Elliot —《City Speak》(1996)`、
  `Don Pullen —《Sacred Common Ground》(**1995**)`。
- **退 4**（第 1201 條）：Duke Ellington《70th Birthday Concert》、Dianne Reeves《The Palo Alto Sessions 1981-1985》、
  Hank Mobley《Curtain Call》、Lou Donaldson Quintet《Wailing With Lou》。
  **第 315 條：prop 19 ＋ 退表 4 ＝ 23 ✓。**
- **退貨率 17%**（1985 後各批：c-148 a 9%／c-148 b 18%／c-149 a 4%／c-149 b 32%／c-150 a 13%／c-150 b 18%／
  c-151 a 13%／c-151 b 18%／c-152 a 17%／c-152 b 9%／**本組 17%**）——**連續七批落在 9–18%，本組回到區間上緣**，
  原因是**本組的「1985 前錄音」有五張，兩張是庫存首發（收）、三張是母體的 CD 化或合輯化（退）**。
- **年份改判 3 張**（第 1203 條）：`Reach` 1995→**1996**、`Sacred Common Ground` 1996→**1995**、
  `Live at the Village Vanguard` 1996→**1985**。**覆核成立、不改的 16 張**也在第 1203 條列出。
- **庫存首發 2 張**（Live at Club Mozambique 1995／Memphis To New York Spirit 1996，**兩張都在 `Blue Note Rare Groove Series`**）。
- **現場盤 2 張**（Live at Club Mozambique／Live at the Village Vanguard），**MB `secondary-types: ['Live']` 兩張都標對 ✓**
  （退件的《70th Birthday Concert》MB 也標對了）——**第 1178 條「MB 在老現場盤上標得比 1990 年代新現場盤準」在本批續成立。**
- **`releaseType`：19 張全是 Album、合輯 0、走 §5.6 的 0、`exceptionReason` 0。**
- **`genres`：18 張 `['jazz']`、1 張 `['jazz','hiphop']`（US3《Broadway & 52nd》，第 1207 條）。**
- **CAA**：RG 層**有圖 14 張**、**真 404 5 張**（Snide Remarks／Go Round／Acoustic Boogie／Different Colours，
  ＋退件的 Wailing With Lou，**皆 `redirect: follow` 重試三次確認，三次都是 404 不是 5xx**，第 589a 條）。
  **圖數最多的是《Broadway & 52nd》11 圖**，最少的有五張各 1 圖。
  ⚠ **有圖的 14 張裡，來源不是原盤的有 3 張**（Petrucciani 用 1996 德國再發、New Moon Daughter 用日版、Broadway & 52nd 用 NL 版）。
- **Apple**：`search` us 一種查法命中 **13 張**、**第三種查法（換市場）命中 3 張**（I Was Born in Love With You→fr、
  Different Colours→fr/nl、Cæcilie Norby→dk）、**三種查法全空 3 張**（Mr. Bow-Tie／Acoustic Boogie，
  ＋Go Round 雖命中但同頁混入三張完全無關的碟）。**第二種查法（換盤名字串）本組一張都沒用到。**
- **本層沒有新抓任何 OCR**：本批用的五份紙本（`billboard-bn-1994-1995q1`／`billboard-bn-1995q2-q4`／`billboard-bn-1996`
  ＋`cashbox-bn-1995`／`cashbox-bn-1996`）**全部是 c-152 已入庫的**，**因此沒有動 `SOURCES-billboard-cashbox.md`**。
- **`node batch-progress/c153/chk-prop.mjs a` 標記 0**（跨批 116 批、4,706 卡、撞卡 0、同 rgMbid 不同掛名 0、同掛名盤名詞元包含 0）。

---

## 第 1201 條（同批）：**退表 4 筆，逐筆與理由分類**

| # | 盤 | rgMbid | 目錄號／年 | 退的分類 | 逐張文案（第 782／842 條要求的那一行） |
|---|---|---|---|---|---|
| 1 | **Duke Ellington —《70th Birthday Concert》** | `8f6835fc-ec14-306e-8486-bdeb9f417c92` | **CDP 7243 8 32746 2 8**／1995-08-15 | **CD 化再發（母體是 1970 年的 Solid State 雙 LP）** | **錄音 1969-11-25 Colston Hall, Bristol（〈Satin Doll〉）與 1969-11-26 Free Trade Hall, Manchester（其餘全部），英國巡演現場**。**母體是 Solid State `SS 19000` 雙 LP（1970）**——Discogs 上美 1817622／7847286／8638383／10989212／13059929、德 3025534（`United Artists`／`Solid State Series`／`SS 19000 X/1-2`）、英 UAD.60001／4420548／14240417、瑞典 4971602、加 8283388、澳 SUAL 933853/4、西 HUS 061-59、1977 日本 LAX 3158 59，**一整群 1970 年原壓**。**1995 年 Blue Note 那張 2CD（Discogs 12613854）的 `format` 欄逐字 `CD, Album, Reissue`**，notes 逐字寫出兩場的日期與軌號歸屬，並註明「The original 2 LP set also edited the medley, but with different omissions」——**它自己就把 1970 年的雙 LP 稱為 original**。⚠ ⚠ **MB 把 1970 年的德國 UA／Solid State 原盤（32ea95af，9＋8 軌）與 1995 年的 Blue Note CD 掛在同一個 RG 裡**——**原壓就在 RG 內，這是最乾淨的再發形狀**；**而且 9939d4c1 那一筆的 credit 是 `Duke Ellington & His Orchestra`，與其餘兩筆不同**（第 1183 條(6) 那種「同一個 RG 混不同廠牌／不同 credit」在本批再中一次）。⚠ **維基逐字「originally released on the Solid State label in 1970 … later reissued on CD on the Blue Note label in 1995」。**⚠ **照 c-152 a 第 1141 條第 4 筆（同一位藝人的《Togo Brava Suite》，1971 UA 雙 LP → 1994 Blue Note CD）的先例退**，同形同判。**Apple 716591871 的 releaseDate 直接寫 `1970-01-01`（17 軌）——店面自己把它標回 1970。CAA RG 層 3 圖，來源是 1970 德版 32ea95af。** |
| 2 | **Dianne Reeves —《The Palo Alto Sessions 1981-1985》** | `ecc57932-8e8b-3bc1-9a81-4c7d1b4398ce` | **CDP 7243 8 36545 2 9**／1996-04-02 | **合輯（母體是 1982／1984 兩張 Palo Alto 原盤 ＋ 一軌 Caldera 時期舊錄音）** | **十三軌全部是 1981–85 年間已經發行過的錄音**：整張 1982 年的 Palo Alto LP《Welcome to My Love》＋1984／85 年《For Every Heart》的三軌＋她在樂團 Caldera 時期的一軌。**Discogs 六筆條目的 `format` 欄全部帶 `Compilation`**（3905107 歐版 `CD, Compilation`、8661311 美版 `CD, Compilation, Reissue`、8661336 俱樂部版、12936501 宣傳版、10511861 宣傳卡帶、日本 TOCJ-6056／26139914），**jazzmusicarchives 的條目標題後面直接掛 `(compilation)`**。⚠ ⚠ **MB 的 `secondary-types` 是空的**——**第 397／613 條「該標沒標」那個方向，本批中一次**；**列舉檔也因此沒把它濾掉**。⚠ **第 874 條過半門檻：13/13 已發行 → 純合輯，不是新錄音，也不夠格走 §5.6**（母體兩張 Palo Alto 原盤都在他廠、也都不是公認的歷史文獻盤）。⚠ **第 1175 條的名單裡就有它**（Billboard 1995-12-16 p114 逐字「and Reeves' 'Palo Alto Sessions.'」）——**那則名單證明的是發行時點，不是它是不是新錄音**，**廠牌預告不能把合輯變成專輯**。⚠ **與本組收下的《The Grand Encounter》（1996-10-29，38268）是同一位藝人、同一年、相鄰號段的兩張完全不同的碟**，主線上架時要能分辨。**Apple 723694684（13 軌，releaseDate 1996-01-01）。CAA RG 層 6 圖。** |
| 3 | **Hank Mobley —《Curtain Call》** | `9ffd3b09-ba64-4ccf-a402-d533e303cc3f` | **TOCJ-1611**／1996-11-27 JP | **庫存盤的 CD 化（東芝 EMI「The BN Works 1500 Unissued」復刻；母體是 1984 年日本首發的 `BNJ 61006`）** | **錄音 1957-08-18 Van Gelder Studio, Hackensack**，班底 Kenny Dorham tp、Sonny Clark p、Jimmy Rowser b（首發時誤記為 George Joyner）、Art Taylor d。**原本排在 1500 系列卻沒發，1984 年才以 Blue Note Japan `BNJ 61006` 在「The Other Side Of Blue Note 1500 Series」首度上市，題名是《Hank Mobley Quintet Featuring Sonny Clark》**（Discogs 4903613／宣傳 29482873，`Vinyl, LP, Album, Limited Edition, Stereo`）。**1996 年的 TOCJ-1611（Discogs 5521365）`format` 欄逐字 `CD, Album, Reissue, Remastered, Stereo`、廠牌欄第二格印系列名 `The BN Works 1500 Unissued`、notes 逐字「Recorded on August 18, 1957. **A reissue of [m681662] with a new title and cover art.**」**——**Discogs 自己講明它是 1984 年那張的再發、只是換了題名與封面**。⚠ **這就是第 1146 條右半邊（「庫存盤的 CD 化」）＋第 1142／1172 條結論 1（東芝 EMI 的 `The BN Works …` 系列看到就當再發）的合體**，與 c-152 a 退掉的《Step Lightly》《Shoutin'》逐字同一種形狀，只是系列名從 4100 換成 1500。⚠ **`chk-prop` 不會亮燈**：池中的 `Hank Mobley —《Hank Mobley Quintet》`（seed ＋ c-136 b）是 **BLP 1550**、是完全不同的一張碟——**但 1984 年那張庫存首發的題名剛好就是《Hank Mobley Quintet Featuring Sonny Clark》**，**兩者在字串比對上會互撞**，主線補卡時務必帶目錄號區分。**Apple 1436770387 的 releaseDate 直接寫 `1984-01-01`——店面自己把它標回 1984 年的首發。CAA RG 層 4 圖，來源是 1996 復刻 CD 5cb5d2b1。** |
| 4 | **Lou Donaldson Quintet —《Wailing With Lou》** | `e14c3acd-0024-4991-a900-2b5fddd595d8` | **TOCJ-1545（＋BLP-1545）**／1996-01-24 JP | **CD 化再發 ＋ 撞池 ＋ MB 同碟兩個 RG** | **原盤是 1957 年的 Blue Note `BLP 1545`**（Discogs 3957365／17184625／28703713／17453311／17404360 一整群美國原壓）。**1996 年的 TOCJ-1545（Discogs 7184733）`format` 欄逐字 `CD, Album, Reissue, Remastered, Mono`、廠牌欄第二格印系列名 `The BN Works 1500 Series`**；同一張碟另有 1976 LNJ-70078、1978 Masterpiece Selection 150、1984 BLP 1500 Series、1990 LP Last Reissue、1999 TOCJ-9088、2005 TOCJ-6491、2009 TOCJ-8614、2021 UCCU-8090、2026 Tone Poet 一長串復刻。⚠ ⚠ **池中已經有 `Lou Donaldson —《Wailing With Lou》`（seed ＋ c-136 b，`year` 1957）**，**而且列舉檔裡本來就有原盤的另一個 RG `ecfbd30e-5984-4938-ad7b-a5e0827c31c2`（掛名 `Lou Donaldson Quintet with Donald Byrd, Herman Foster, John "Peck" Morrison & Art Taylor`，period 1956-66）**——**這是第 936 條「MB 把同一張碟建成兩個 RG」在 1985 後這一段的第一次實例，而且兩個 RG 的掛名長度差了三十個字元**。⚠ **`chk-prop` 一個燈都沒亮**：折鍵是「掛名｜盤名」，**池中是 `Lou Donaldson`、本筆是 `Lou Donaldson Quintet`**——**第 1181 條那個盲區（掛名不同所以第一道不報）在本批再中一次**。**Apple 1444084267 的 releaseDate 直接寫 `1957-01-01`。CAA RG 層真 404（三試）。** |

**⚠ 四張沒有一張被 `chk-prop` 標記**（第 611／1089／1157／1170 條，**1985 後第六批連續成立**）。**抓出來的方法**：
(a) **Discogs 的 `format` 欄**（四張全中：`Reissue` 三張、`Compilation` 一張）；
(b) **Discogs 的廠牌欄／`notes` 寫出母體系列名或母體目錄號**（`The BN Works 1500 Unissued`／`The BN Works 1500 Series`／`SS 19000`／`A reissue of [m681662]`）；
(c) **Apple 的 `releaseDate` 標回母體年**（四張全中：1970／1996／1984／1957——**本批是第 1141 條第四種抓法命中率最高的一批**）；
(d) **MB 把原壓與再發掛在同一個 RG 裡**（《70th Birthday Concert》）。

**交主線的資料庫缺口**（第 1114 條(甲)／第 1141／1171 條的續筆）：
1. **Solid State `SS 19000` — Duke Ellington《Duke Ellington's 70th Birthday Concert》(1970，雙 LP，1969-11 英國巡演)**：
   **池中 `Duke Ellington` 已有 22 列**（含 c-148《The African Flower》的 James Newton 致敬盤與 c-150 b《1969 All-Star White House Tribute》），
   **1969 年英國巡演這一段仍然沒有**（與 c-152 a 第 1141 條第 4 筆《Togo Brava Suite》＝1971 年英國巡演是同一趟脈絡的前後段）。
   **若要補，`year` 取 1970、`label` 寫 Solid State SS 19000，不要用 1995 年的 Blue Note CD。**
2. **Blue Note Japan `BNJ 61006` — Hank Mobley《Hank Mobley Quintet Featuring Sonny Clark》／後改題《Curtain Call》(1984 日本庫存首發，1957-08-18 錄音)**：
   **池中 `Hank Mobley` 已有 55 列、這一張沒有**。**若要補，`year` 取 1984、`label` 寫 BNJ 61006**，
   **並且務必與池中 BLP 1550 的《Hank Mobley Quintet》區分**（見上表第 3 筆）。
3. **Blue Note `CDP 7243 8 36545 2 9` — Dianne Reeves《The Palo Alto Sessions 1981-1985》(1996)**：**不建議補**（純合輯）；
   **要補的是它的兩個母體**——`Welcome to My Love`（Palo Alto，1982）與 `For Every Heart`（Palo Alto／TBA，1984），**兩張池中皆 0**，**但都是他廠、不在本線範圍**。
4. **Somethin' Else `TOCJ-5574` — 日野皓正＝菊地雅章《Acoustic Boogie》的日本原盤（1995）**：**MB 沒建**，只把美國目錄號掛在一筆 country=JP 的 release 上（見第 1209 條）。
5. **Concord Jazz／The George Wein Collection `GW-3006` — The Michel Petrucciani Trio《Live at the Village Vanguard》(1985 雙 LP)**：
   **MB 整個沒建原盤**（只有 1996 德國再發與 2002 Blue Note 版）——**本層已用第 1206 條把卡收進來、`year` 取 1985**，主線上架時要知道封面與廠牌有兩版。

---

## 第 1202 條（同批，**重要；新立，把第 1146／1173 條的界線操作化**）：**「先前發行過」之後的那一問，要問的是「那次發行的目錄號在不在本線掃的 Blue Note 目錄裡」**

第 1173 條把判準寫成兩句：
> **(甲) 沒發行過 → 收，`year` ＝ 首次商業發行年。**
> **(乙) 有 → 再問一句：「先前那次發行在本線的任何清單／池中另有身分嗎？」有 → 退（重複）；沒有 → 收、`year` 取那次的年。**

**本批一次撞到 (乙) 的兩邊，而且同一批裡三比一，剛好可以把「另有身分」這五個字定義死：**

| | **退（母體在 Blue Note／Liberty-UA 家族的號段裡）** | **收（母體在真正的他廠號段裡）** |
|---|---|---|
| 例 | **《Curtain Call》**母體 `BNJ 61006`（Blue Note Japan, 1984）<br>**《70th Birthday Concert》**母體 `SS 19000`（Solid State／United Artists, 1970）<br>**《Wailing With Lou》**母體 `BLP 1545`（Blue Note, 1957） | **《Live at the Village Vanguard》**母體 `GW-3006`（**Concord Jazz／The George Wein Collection**, 1985） |
| 母體在不在列舉檔掃的目錄裡 | **在**（Blue Note／Liberty／UA／Solid State 都屬本線期一至期二的範圍） | **不在**（Concord 的自家號段，列舉檔只掃 Blue Note 目錄） |
| 退掉會怎樣 | **不會消失**：母體本來就該以原盤年建卡，寫進資料庫缺口即可 | **整張碟從管線消失** |
| 判定 | **退**，母體進缺口表 | **收**，`year` 取母廠首發年、`label` 寫母廠 |

**→ 判準改寫成三句（覆蓋第 1173 條的兩句版）**：
> **(甲) 這批錄音先前沒有以商品形式發行過 → 收，`year` ＝ 首次商業發行年。**
> **(乙) 發行過，而且那次發行的目錄號屬於 Blue Note／Liberty／United Artists／Solid State 家族 → 退；母體以原盤年寫進資料庫缺口。**
> **(丙) 發行過，但那次發行在真正的他廠（Concord、Palo Alto、Artalent……）→ 收，`year` 取他廠首發年、`label` 寫原廠、`risk` 註明 Blue Note 版年份**（簡報 §三-3 那條）。

⚠ **(乙) 裡特別要記 Solid State**：它是 United Artists 的爵士子廠、1966–70 年與 Blue Note 同一個母公司，
**c-152 a 第 1141 條退《Togo Brava Suite》時用的就是同一個理由（母體 1971 UA 雙 LP）**——**本條只是把它從個案變成規則。**
⚠ **(乙) 與第 1173 條的 `Rachelle Ferrell —《First Instrument》` 不衝突**：那張的母體是 **Somethin' Else 的自家號 `TOCJ-5520`**，
**Somethin' Else 雖然算 Blue Note 的子廠牌（第 906／1118／1155 條），但它的自家號段不在列舉檔掃的範圍內**——
**「算不算子廠牌」與「號段在不在列舉範圍內」是兩件事，收退看後者。**

---

## 第 1203 條（同批）：**年份——改判 3 張、維持但留反向證據 2 張、覆核成立 14 張**

| 盤 | 列舉檔／MB 的說法 | 本層取 | 依據 |
|---|---|---|---|
| **`Jacky Terrasson —《Reach》`** | MB frd 1995；三筆 release 全 1995 | **1996** | **Billboard 1995-12-16 p114（EMI 的 Gary Gersh）逐字「…acclaimed pianist Jacky Terrasson's second release, 'Reach'」列在 1996 第一季名單**（第 1175 條）＋ **Apple releaseDate `1996-02-20`** ＋ **1996-03-02 p63 評介** ＋ **1996-03-09 p41 TOP JAZZ ALBUMS 以 NEW 進榜、03-23 第 24** ＋ **1995 年兩刊查 `35739` 零命中** |
| **`Don Pullen —《Sacred Common Ground》`** | MB frd **1996** | **1995** | **Discogs 美國原壓 4825343／歐版 5351188／美國宣傳版 33628557 三筆 `year` 欄全 1995** ＋ **盤面 ℗© Capitol Records, Inc. 1995** ＋ **1995 年就有宣傳版**；反方只有 **Billboard 1996-01-20 p77 與 Cash Box 1996-02-03 p14 兩則評介、零榜位** → **第 1053／1176 條：評介不足以推翻盤面 ℗ 年**，**階序上 Discogs 原壓群 ＞ MB frd** |
| **`Michel Petrucciani —《Live at the Village Vanguard》`** | MB frd **1996**（抄的是 1996 德國 Bellaphon 再發 CD） | **1985** | **Concord Jazz／The George Wein Collection `GW-3006` 雙 LP**：Discogs 美 3839547／德 1491837／加 7123502 三筆原壓 `year` 欄全 1985、format 欄乾淨的 `Vinyl, LP, Album`、德版 notes 逐字「Recorded live March 16, 1984」、唱片面 A/B/C/D＝雙 LP；同年另有 Concord CD `CCD 43006` 與日本 King `K32Y 6045`；**維基 infobox Released 1985／Label Concord Jazz**（第 1206 條） |
| ⚠ **`Ron Carter —《Mr. Bow-Tie》`** | MB frd 1995；Discogs 美日四筆 1995 | **1995（不改）** | **反向證據是 Cash Box 1996-02-03 p14 的完整評介**；但**盤面 ℗© 逐字 1995 Somethin' Else**、**Billboard 兩年查 `35407` 零命中、連榜位都沒有** → **第 1053／1176 條：只有評介，不足**（與《Bang! Zoom》同形） |
| ⚠ **`Cæcilie Norby —《Cæcilie Norby》`** | MB frd 1995-11-01；Discogs 歐日美四筆 1995 | **1995（不改）** | **反向證據是 Billboard 1996-01-20 p47 Global Music Pulse（丹麥）整段報導 ＋ Apple dk 市場 releaseDate `1994-01-01`**；但**那篇自己說「the current album」、是國際推廣專題不是新片欄**，Apple 的 1994 是年頭回填（**方向還相反**）→ 不動 |

**覆核成立、不改的 14 張**（列出來省得下一批重查）：
《Live at Club Mozambique》1995（**Billboard 1995-04-08 p67 庫存首發廣告＋Discogs `released` 1995-03-00**，第 1171 條已預判）、
《Snide Remarks》1995（**Billboard 1995-07-01 p47 COMING SOON 廣告＋1995-10-14 p70 評介**）、
《Go Round》1995（**同一則 1995-07-01 廣告**；無評介無榜位）、
《Groove Elation!》1995（**MB frd 1995-10-24＋Discogs 美歐加日原壓群＋1995-11-11 p97 評介**）、
《I Was Born in Love With You》1995（NL 首發；Discogs 三筆；**兩刊查 33390 零命中**，1996-07-27 p34 只是回顧行文）、
《Acoustic Boogie》1995（日美同年；**兩刊零命中**）、
《Solos and Duets》1995（**同一則 1995-07-01 廣告逐字「ELAINE ELIAS 32073」**＋Discogs 七筆）、
《Different Colours》1996（**Discogs 2150285 荷蘭原壓 `CD, Album`**；兩刊零命中）、
《The Grand Encounter》1996（**1996-07-27 p37「Coming this fall」預告＋11-02 p1 全版廣告＋p33「streets on Tuesday (29)」＋11-16 p48 榜 NEW**，本組證據最厚）、
《Broadway & 52nd》1996（**日版 TOCP-50040 `released` 1996-11-27＋℗&© 1996**；美歐 1997，第 1205 條）、
《Ancestors》1996（**MB frd 1996-02-20＋Discogs 四筆＋1996-05-04 p33「fairly new in the racks」**）、
《Memphis To New York Spirit》1996（**1996-03-30 p117 評介逐字「never seen complete release until now」**）、
《New Moon Daughter》1996（**MB frd 1996-01-24 JP／歐 03-04／美 03-05＋Discogs 十餘筆＋1996-11-30 p33 在榜第 37 週**）、
《City Speak》1996（**1996-04-20 p57 榜 NEW＋04-27 p73 評介＋連續八期在榜**）。

⚠ **紙本層完全空白的有 3 張**（Acoustic Boogie／Different Colours／Go Round 的評介與榜位部分）——
**兩張是 EMI 日本／荷蘭在地發行的碟，一張是只有廣告沒有評介的新人盤**，**紙本空白是預期的，不是證據不足。**
⚠ **本組出現兩次「Apple 的 releaseDate 整整標歪一年」**（`Ancestors` 標 1995、`New Moon Daughter` 標 1995，兩張實際都是 1996）
——**第 1141 條那張「Apple 年份怎麼歪」的表要補第五種：不是回填到錄音年、也不是年頭回填到正確的年，而是整整早一年。**

---

## 第 1204 條（同批）：**《The Palo Alto Sessions 1981-1985》是本段第一張「MB `secondary-types` 空、但它確實是合輯」的碟**

第 397／613／782 條說 MB 的 `secondary-types` 兩個方向都會漏。**c-152 a 撞到的是「該標 Live 沒標」，本批撞到的是「該標 Compilation 沒標」**：

| 來源 | 說法 |
|---|---|
| **MB RG `ecc57932`** | primary-type `Album`、**`secondary-types` 空** |
| **列舉檔** | 因此**沒有濾掉**（列舉檔已濾 Compilation，但它信 MB 這一欄） |
| **Discogs 六筆條目的 `format` 欄** | **全部帶 `Compilation`**（歐版 `CD, Compilation`、美版 `CD, Compilation, Reissue`、俱樂部版／宣傳版／宣傳卡帶／日版 TOCJ-6056 同） |
| **內容** | **整張 1982 年 Palo Alto LP《Welcome to My Love》＋1984/85《For Every Heart》三軌＋Caldera 時期一軌 ＝ 13/13 全部已發行** |
| **盤名** | **`Sessions` ＋ 年份區間 `1981-1985`**——**派工信點名要細看的形狀** |

→ **給 c-153 b 之後**：**盤名帶 `Sessions`／年份區間的，`secondary-types` 空不能當「不是合輯」的證據**，
**一律去讀 Discogs 的 `format` 欄與逐軌來源**（第 782 條說 Discogs 的 format 欄也會錯，**但它錯的方向是把合輯標成專輯，本批是相反方向、它標對了**）。

---

## 第 1205 條（同批）：**Us3《Broadway & 52nd》——日版比美版早四個月，而且 Discogs 的「原壓群」整群站在美版那一邊**

| 來源 | 年份 |
|---|---|
| **日本 Blue Note `TOCP-50040`（Discogs 1103781）** | **`released` 1996-11-27**、notes 逐字 **`℗&© 1996 Capitol Records, Inc.`** |
| **MB frd** | **1996-11-27**（JP release 3de1e7a1，但 MB 把目錄號登成 `TOCP-50042`） |
| **MB NL release 231119ad** | 1996，目錄號 `7243 8 30027 2 6` |
| **Discogs 上所有美／英／歐／加／法／義／澳／波／巴條目** | **1997**（同一個目錄號 `7243 8 30027 2 6`） |
| **MB US release 1833f73e** | **1997-04-08** |
| **Billboard 1996-12-07 p20** | 逐字「Blue Note act US3 makes its return to the music scene **March 11** with 'Broadway & 52nd.'」（＝1997-03-11 美國街頭日） |

**取 1996（日本首發）。** 這條要記，因為：
1. **第 531 條的「Discogs 原壓群」在這裡是多數但錯**——**十幾筆歐美條目全 1997，唯一的 1996 是日版一筆**；
2. **推翻多數的是「日版有完整日期 ＋ 盤面 ℗&© 1996 ＋ MB frd 一致」三件**，**不是任何一件單獨**；
3. ⚠ **MB 的 NL release 標 1996 但目錄號與 Discogs 標 1997 的那群完全相同**——**這一筆的年份可疑，不要拿它當 1996 的證據**；
4. ⚠ **MB 與 Discogs 的日版目錄號不一致**（`TOCP-50042` vs `TOCP-50040`）——**兩形都要帶**。
→ **第 1063 條（日本盤常比美版早半年到一年）在本批得到第二次驗證，這次早了四個多月。**

---

## 第 1206 條（同批，**重要；第 1202 條的實例**）：**Michel Petrucciani《Live at the Village Vanguard》——MB frd 抄了再發那一層，母體在 Concord，所以收、`year` 取 1985**

- **MB frd 1996** ＝ **1996 年德國 Bellaphon `660.50.013` 那張再發 CD**（MB 把廠牌登成 `Jazzmen`；Discogs 7118081 的 format 欄是 `CD, Album, Reissue`）。
- **原盤是 1985 年 Concord Jazz／The George Wein Collection `GW-3006` 雙 LP**，**MB 整個沒建**。
- **Blue Note 版是 2002 年的 `7243 5 40382 2 8`**（Discogs 11367326 美版 `CD, Album, Reissue, Remastered`）。
- **依第 1202 條 (丙)：母體在 Concord 的號段、不在本線任何清單上、池中也沒有 → 收，`year` 取 1985、`label` 寫原廠。**
- **逐軌比對（第 738／859／1049 條）已做完**：
  **本張八軌**（Nardis／Oleo／Le Bricoleur de Big Sur／To Erlinda／Say It Again and Again／Trouble／Three Forgotten Magic Words／'round About Midnight，1984-03-16 Village Vanguard，Palle Danielsson b／Eliot Zigmund d）
  vs **池中 c-151 b 的 `Michel Petrucciani —《Michel Petrucciani (Live)》`（1994 發行）八軌**
  （Black Magic／Miles Davis Licks／Contradictions／Bite／Rachid／Looking Up／Thank You Note／Estate，**Discogs 2426976 notes 逐字「Recorded in November of 1991 at The Arsenal in Metz, France」**）
  ——**0 重疊、不同年、不同班底、不同場地**，**兩張現場盤並存，risk 互指**（第 1049 條）。
- ⚠ **掛名收攏**：MB RG credit 是 **`The Michel Petrucciani Trio`（740b1ced Group）**、Discogs 與 Apple 也用這個字串，
  **但池中 16 列全部是 `Michel Petrucciani`（Person）、Trio 字串 0 列** → **取池中多數寫法**（第 307 條），Trio 版進 queryAlias，**不新造分裂**。
- ⚠ **盤名撞擊**：池中另有**三張盤名一字不差的 `Live at the Village Vanguard`**（seed 的 Elvin Jones、Thad Jones & Mel Lewis，c-152 a 的 大西順子）
  **＋五張含該詞組的碟**（Sonny Rollins／Dexter Gordon／Bill Evans Trio／John Coltrane／The Great Jazz Trio／Joe Henderson／The Benny Green Trio／Joe Lovano）——
  **`chk-prop` 折鍵是「掛名｜盤名」所以八張全部不報**，**上架比對必須帶掛名**。**這是本段「同盤名不同藝人」密度最高的一個盤名，記給後批。**

---

## 第 1207 條（同批）：**曲風——18 張 `['jazz']`、1 張 `['jazz','hiphop']`；同一組藝人與 seed 的舊卡刻意不一致**

- **`US3 —《Broadway & 52nd》` 判 `['jazz','hiphop']`**：**Billboard 1996-12-07 p20 的欄名逐字就是「HIP-HOP JAZZ」**，
  十四軌全部是 MC 主唱的嘻哈曲式配 Blue Note 母帶採樣——**與 c-152 a 的 `Greg Osby —《Black Book》`（第 1140 條末段）同一個處理。**
- ⚠ **池中 seed 的 `US3 —《Hand on the Torch》`（1993）用的是 `['jazz','soul']`**——**同一組藝人兩張卡曲風不一致，本層知道並刻意如此**：
  **seed 期的舊判不回頭改別人的卡**（第 858 條可逆；要改是主線的事，不是策展層的事）。
- **`Richard Elliot —《City Speak》` 判 `['jazz']` 不加 `soul`**：**照 c-152 a《After Dark》的先例**（第 1170 條末段同樣的處理）。
- **簡報 §一-4 的「撞到疑似非爵士的退」在本批沒有觸發**：US3 與 Richard Elliot 兩張看似邊緣，
  **但前者是 Blue Note 自家的爵士採樣企劃、池中首作已在，後者由 Blue Note Contemporary 發行並上 Billboard 的 TOP CONTEMPORARY JAZZ ALBUMS**，兩張都收。

---

## 第 1208 條（同批）：**掛名——沿用 10、收攏 4、新字串 3、聯名新造 1、新造分裂 0**

### （一）沿用池中既有字串 10（共 10 張卡）

`Kevin Hays`（c-152 a 1 張）、`John Scofield`（seed＋c-150～c-152 共 7 張，另有 `John Scofield Quartet` 2 張並存）、
`Ron Carter`（seed 2＋c-150／c-151 共 4 列）、`Jacky Terrasson`（c-152 b 1 張）、`Eliane Elias`（c-148～c-150 共 4 張）、
`Dianne Reeves`（c-148／c-149／c-152 a 共 3 張）、`Renee Rosnes`（c-150／c-151 共 3 張）、
`Cassandra Wilson`（seed 1 張）、`Richard Elliot`（c-152 a 1 張）、`Don Pullen`（seed 3＋c-149／c-150 共 5 列裸名，
另有 `Don Pullen & The African-Brazilian Connection` 與 `The Don Pullen-George Adams Quartet` 兩個既有分裂並存，**本層不動**）。

### （二）收攏 4——**四次都是把 MB／店面的變體收回池中的既有寫法**

| MB artist-credit | 卡上掛名 | 收攏的理由 |
|---|---|---|
| **`Dr. Lonnie Smith`**（RG 層） | **`Lonnie Smith`** | **同一個 RG 底下 1995 年原盤 99b644a7 的 release credit 就是裸名**，Discogs 全部條目也印裸名；池中 8 列全是 `Lonnie Smith`／`Lonnie Smith Trio`。⚠ **Apple 的 artistName 是 `Dr. Lonnie Smith`**，上架要兩形都帶。 |
| **`John Patton`**（裸名） | **`Big John Patton`** | **既有分裂取多數**（第 583／1116 條）：池中 `Big John Patton` **10 列** ＞ `John Patton` 1 列（seed《Along Came John》）；**Apple 的 artistName 逐字也是 `Big John Patton`**。**這是這組分裂吃到的第十張卡**，c-139／c-140／c-141／c-142／c-152 b 都用同一條理由。 |
| **`The Michel Petrucciani Trio`**（Group 740b1ced） | **`Michel Petrucciani`** | 第 307 條：池中 16 列全是 Person 裸名、Trio 字串 0 列（第 1206 條）。 |
| **`Us3`**（Group b370defd；Discogs 部分條目印 `Us 3`） | **`US3`** | 第 307 條：**池中 seed 的既有字串是全大寫 `US3`**。⚠ `chk-prop` 的折鍵會把三形折成同一個鍵，**但上架字串比對不會**。 |

### （三）新字串 3（池中皆 0 列，漢字／羅馬字／片假名三形都掃過）

| 掛名 | MB 實體 | 第 307 條反查 |
|---|---|---|
| **`Bill Stewart`** | 47ca3bd6 Person／US／**「US jazz drummer」**／1966-10-18 | ⚠ ⚠ **MB 同字串有四個 Person**：47ca3bd6（本卡）、**536339f3「US jazz saxophonist」（1924-05-13）**、c8b660a0（南方搖滾鼓手／製作人）、963cc91c（加拿大吉他手），另有群組 `Bill Stewart Trio` 5eadff8e。**同一個曲風裡同字串的兩個人（爵士鼓手 vs 爵士薩克斯手）＝第 935 條 `Bill Evans (saxophonist)` 那個形狀**——**日後任何 `Bill Stewart` 進池一定要先帶消歧義。** |
| **`Denise Jannah`**（吃兩張卡） | 523b9c07 Person／NL／1956-11-07 | **同字串只有這一個實體、零衝突**（本組最乾淨的新掛名）。 |
| **`Cæcilie Norby`** | 75b54a14 Person／DK／「Danish jazz and rock singer」／1964-09-09 | 同字串只有這一個實體。⚠ **`æ` 是本卡最大的上架風險**：盤名與掛名同字串（同名專輯），**兩個欄位都帶 `æ`**，**Discogs 的搜尋索引與 Apple 部分市場用 `Caecilie`**——**`æ`／`ae` 兩形都要掃**。**`æ` 是合法拉丁字母、不是第 50 條那種非 ASCII 連字號，`chk-prop` 不擋。** |

### （四）聯名新造 1

**`日野皓正 & 菊地雅章`**（60d3fa1e Person ＋ 5216d6d4 Person）——**不是新造分裂**：
**兩個字串都是池中既有的漢字多數寫法**（`日野皓正` 25 列、`菊地雅章` 14 列），本層只是照 MB 的 RG credit 用 `&` 接起來，
**先例是 seed 的 `Duke Ellington & John Coltrane`、`Jim Hall & Ron Carter`**。
⚠ **Discogs 與 Apple 都用羅馬字 `Terumasa Hino-Masabumi Kikuchi Quintet`**（連字號形、且多了 Quintet），進 queryAlias。
⚠ **池中的羅馬字／漢字兩形本來就已經分裂**（`Terumasa Hino` 0 列、`Masabumi Kikuchi Quintet` 1 列 seed），**本層不去動它**（第 307 條：不自行合併）。

### （五）新造分裂 0。

---

## 第 1209 條（同批）：**MB 這一批的六種毛病**（第 1183 條那張表的續筆）

| # | 毛病 | 本批實例 |
|---|---|---|
| 1 | **`first-release-date` 抄了再發那一層** | **《Live at the Village Vanguard》**：frd 1996 ＝ 1996 德國 Bellaphon 再發 CD，**原盤 1985 Concord 整個沒建**（第 1206 條）。 |
| 2 | **label-info 只有俱樂部號、沒有正規目錄號** | **《Solos and Duets》a8c177cc 的 catno 是 `D 104094`（BMG／哥倫比亞唱片俱樂部序號）**，正規號 `CDP 7243 8 32073 2 9` 只在 Discogs 上。**本段第一次抓到這種形狀。** |
| 3 | **label-info 整個空，導致列舉檔誤判廠牌** | **《Different Colours》e2ab1d48（1996 NL）label-info 空**，列舉檔因此在 note 寫「BN 首發 2002，原盤 1996 可能他廠（再發）」——**Discogs 2150285 的廠牌欄逐字就是 Blue Note，列舉檔這一欄錯**（第 1210 條）。 |
| 4 | **label-info 掛錯廠牌** | **《City Speak》2b7b4a73 掛 `Blue Note`，實際是 `Blue Note Contemporary`**（Billboard 1996-04-27 p73 評介署名＋Discogs 6177225／15331721）——**與 c-152 a《After Dark》同形，第 1154 條在本批再中一次。** |
| 5 | **release 的欄位互相矛盾／把資訊塞進 disambiguation** | **《Acoustic Boogie》246a894b：country=JP 卻掛美國目錄號 `CDP 7243 8 36259 2 5`，日本原盤 `TOCJ-5574` 整個沒建**；**《Memphis To New York Spirit》6389dcba 的 disambiguation 逐字塞了「UCCQ-5010 Japan 1996; … Europe 1996 …」，而 UCCQ-5010 其實是 2014 年的再壓**。 |
| 6 | **殘件 release（無日期／無國別／無廠牌／媒體欄是 `?`），而且曲序與正式盤不同** | **《The Grand Encounter》05b8de90**（`?`×10 軌，曲序 Let Me Love You 排第 1）。 |

⚠ **本批沒有出現的**：第 817 條那種「MB 建出不存在的版本」、第 1116 條那種「credit 帶雙空格 join phrase」。
⚠ **本批出現的第 936 條實例在退件上**：**《Wailing With Lou》同一張碟有兩個 RG**（`ecfbd30e` 原盤 1957／`e14c3acd` 1996 TOCJ），
**而且兩個 RG 的掛名長度差了三十個字元**（見第 1201 條第 4 筆）。

---

## 第 1210 條（同批）：**列舉檔這一批有兩欄是錯的，兩欄都是「因為 MB 缺欄位就推論成另一件事」**

| 筆 | 列舉檔的 `note` | 實際 |
|---|---|---|
| **《Different Colours》** | **「BN 首發 2002，原盤 1996 可能他廠（再發）」** | **1996 年就是 Blue Note 荷蘭首發**（Discogs 2150285：`Blue Note 7243 8 54729 2 3`、NL、1996、format 欄乾淨的 `CD, Album`）。**MB 的 1996 那筆 label-info 是空的，不是「不是 Blue Note」。** |
| **《Acoustic Boogie》** | **「僅 JP 盤」** | **有美國版**（Discogs 12097026，`Blue Note CDP 7243 8 36259 2 5`，廠牌欄併印 `Somethin' Else Records`）。 |

⚠ **另有一筆 `note` 是對的但會誤導**：**《Curtain Call》的 note 寫「再發系列: Blue Note Tone Poet Series」**
——**那指的是 2022 年的 Tone Poet 黑膠（B0033314-01），不是 1996 年的 TOCJ-1611**；
**真正讓它被退的是 1996 那一筆的系列名 `The BN Works 1500 Unissued`，列舉檔沒有抓到這個系列**（第 1201 條第 3 筆）。
→ **給 c-153 b 之後**：**列舉檔的 `reissueSeries` 欄只收了十個折疊過的系列，東芝 EMI 的 `The BN Works …` 家族不在內**——
**看到 `TOCJ-` 開頭又是 1990 年代的碟，一律去 Discogs 讀廠牌欄第二／三格的系列名。**

---

## 第 1211 條（同批）：**第 1172 條「措辭像再發、實際是首發」的反向證據，本批又中兩次**

| 盤 | 措辭像再發的地方 | 判首發的依據 |
|---|---|---|
| **`Lonnie Smith —《Live at Club Mozambique》`** | **系列名 `Blue Note Rare Groove Series`**（第 1172 條已證是混裝系列）；**Blue Note 當年甚至已經給過目錄號 `4371`** | **Billboard 1995-04-08 p67 廣告上欄「unreleased sessions from the late 60s」** ＋ **Discogs 367861 notes 引內頁逐字「it remained on the shelf for 25 years」** ＋ **format 欄乾淨的 `CD, Album`** |
| **`Big John Patton —《Memphis To New York Spirit》`** | **Billboard 1996-03-30 p117 評介的製作人欄逐字「REISSUE PRODUCER: Bob Belden」** | **同一段正文逐字「they have never seen complete release until now」** ＋ **Discogs 兩筆 format 欄全是乾淨的 `Vinyl, LP, Album`／`CD, Album`、一個 `Reissue` 都沒有** |

→ **這是第 1146 條「Billboard 把 Chet Baker 排在 VITAL REISSUES 欄、正文卻寫 previously unreleased」的第三、四次同形**。
**判準仍然是第 1202 條 (甲)：看「這批錄音先前有沒有以商品形式發行過」，不看製作人頭銜、不看系列名、不看目錄號有沒有被預先指派。**

⚠ **《Memphis To New York Spirit》另有一個軌數陷阱**：**CD 8 軌、同期黑膠只有 5 軌**
——**第 738／859 條要求「看到 CD 那一層」的理由在本張最具體**：只看黑膠會漏掉三軌（Man From Tanganyika／Cissy Strut／Dragon Slayer）。

---

## 第 1212 條（同批，**收尾**）：**交件數字、中間檔、本棒改動的檔案**

- **`batch-progress/c153/prop-a.json`：19 張、18 位**（`Denise Jannah` 吃兩張）。**第 315 條：19 ＋ 退表 4 ＝ 23 ✓。**
- **`node batch-progress/c153/chk-prop.mjs a` → 標記 0**（116 批、4,706 卡、撞卡 0、同 rgMbid 不同掛名 0、同掛名盤名詞元包含 0）。
- **中間檔**（scratchpad `c153a/`，第 533 條，**不在 repo 內**）：
  `mb/rg-*.json`＋`mb/rel-*.json`（23 筆 × 2 端點，UA `dip-vinyl-shop/1.0 (kubinice06@gmail.com)`、1 req/s、零 503）、
  `mb/a-*.json`（四個新掛名的 `artist?query` 反查）、`dg/s*.json`＋`dg/r*.json`（Discogs 搜尋與 release 明細）、
  `store.json`（Apple 三市場 × 23 ＋ CAA RG 層探測）、`pool.mjs`／`sum.mjs`／`tl.mjs`／`ocr.py`（掃池、MB 摘要、軌目、OCR 取脈絡）。
- **本棒改動的檔案**：`batch-progress/c153/prop-a.json`（新建，19 張）、**`batch-progress/c153/rulings.md`（新建，第 1200–1212 條）**。
  **沒有動** `seed_cards.json`／`apex_pool.json`／`PROJECT_MEMORY.md`／KV／Firestore／`batch-progress/enum/*`／`SOURCES-billboard-cashbox.md`／git。
- ⚠ **號段**：本批 a 組用 **1200–1212**，**1213–1229 留白**；**b 組的編號由派工信另給，不要佔用本段。**
- **給 c-153 b 的三句話**：
