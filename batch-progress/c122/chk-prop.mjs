// c-122 提案檢查。用法：node batch-progress/c122/chk-prop.mjs [組別…]
...
const WHITELIST = new Set(['electronic', 'hardcore-7inch', 'asia-mini-album']);
...
for (const g of groups) {
  const f = path.join(ROOT, `batch-progress/c122/prop-${g}.json`);
  ...
  p.forEach(x => {
    ...
    } else if (['EP', 'Single', 'DJ-mix', 'Other'].includes(x.releaseType) &&
               WHITELIST.has(x.genreException || x.releaseTypeException)) {
      // §5.5 白名單。2026-09-08 店主核定增列 `hardcore-7inch`——1980–90 年代 hardcore／punk
      // 的 7 吋、split 與 demo。c-120 實測不開的代價是 99 個 release-group、33 支團收不進來。
      // 舉證要求與 electronic 完全相同：逐張說明為什麼這一張是該樂種的核心經典，不做廠牌全收。
      const ex = x.genreException || x.releaseTypeException;
      if (!x.exceptionReason || Array.from(String(x.exceptionReason)).length < 12)
        say(`§5.5 ${ex} 缺 exceptionReason（≥12 字）：${x.artist} — ${x.album}`);
      const urls = (x.exceptionEvidenceUrls || []).filter(u => /^https:\/\/\S+$/.test(String(u)));
      if (urls.length < 2) say(`§5.5 ${ex} 的證據網址不足兩個：${x.artist} — ${x.album}`);
    } else if (['EP', 'Single', 'DJ-mix', 'Other'].includes(x.releaseType)) {
      say(`非 Album 未走 §5.5 白名單（genreException 要是 ${[...WHITELIST].join(' / ')}）：${x.artist} — ${x.album}`);
