...
### 第 2571 條：c-166 寫作層 a 組（writer-1，23 張）預算實測——**本批鉤子層的超標預期只兌現了一次**

派工信第三節第 1 點說「本批專名密度高，鉤子層第一版 23 張全部落在 245–313、是靠整格捨去壓下來的」。
**寫作層實測的方向與之相反：23 張初稿無一張超過 240，反而有一張掉到下限以下。**

- 初稿實測 `Array.from(desc).length`：**22 張落在 189–230，1 張 175（`Joe Chambers《Dance Kobina》`）**。
- 定稿分佈 **189–230、中位 209**，`>260` 0 張，`<180` 0 張。
- 依 `writer-base.md` 第 107–124 行（w2-118 定案）**不從卡的組成預測偏差方向**，
  本條只記錄「鉤子層的字元壓力不等於寫作層的字元壓力」這個事實，**不留任何修正係數給後批**。
  成因在本批很清楚：note 的專名要進 desc 得帶中文述語（「波多黎各作曲家 Pedro Flores」），
  而 hook 已經吃掉 16–32 字元且一字不能改，**可支配的格數比鉤子層少，於是自然往下限靠**。

### 第 2572 條：`Joe Chambers《Dance Kobina》`——**捨去的格子改回補回，因為撞破的是下限不是上限**

動筆前依 `writer-base.md` 第 121 行「專名先壓下來」把 note 裡的 **`Francis Wolff` 整格捨去**
（理由：hook 已寫「Alfred Lion 請他錄一張自己的碟」，再點第二位創辦人接近重述 hook）。
**初稿實測 175，低於 180 下限。**

**裁定：把 `Francis Wolff` 補回，寫成「出面邀約的其實還有另一位創辦人 Francis Wolff」。**
判準是 `writer-base.md` 第 80–82 行——**補的順序是「主故事後續的第二個事實」優先**，
而這一格正是主故事（拒絕 Blue Note 邀約）本身的第二個事實，比補發行資料或聲音描述更貼題；
而且改寫成「出面邀約的其實還有」是**增訊**不是重述 hook（hook 只點了 Alfred Lion 一人）。
定稿 213。**可逆（改的是一個子句，不是骨架），依三條判準第 2 條自行定案。**

### 第 2573 條：`Norah Jones《Little Broken Hearts: Live at Allaire Studios》`——**本碟盤名整格不引用**

note 的指示是「引用盤名時帶完整副標題」。**本張逐字照辦的代價是 41 字元**
（`《Little Broken Hearts: Live at Allaire Studios》`），而該卡的預算已被
hook（23）＋母體盤名＋錄音日期地點＋三位樂手姓名＋限量規格佔滿，**加進去實測會落在 244 左右、撞破上限**。

**裁定：本碟盤名整格不引用，只引用 2012 年的母體《Little Broken Hearts》，本碟一律以「這份錄音」指稱。**
note 那句是**條件句**（「引用盤名**時**」），不引用即不觸發，**不構成違反 note**。
依 `writer-base.md` 第 96 行「修剪一律以整格捨去」處理，定稿 209。
⚠ **給人工審稿的提醒**：若店主要求卡面務必出現完整副標題，**要砍的是三位樂手那一格**（約 −50），不是逐句削字。

### 第 2574 條：`Chris Botti《Vol. 1》`——**排除性更正確認未回補，且未寫成否定句**

派工信第三節第 5 點說本批有一張的素材是「排除性更正」、鉤子層已整格不寫。
**本棒實掃指認出該張為 `Chris Botti《Vol. 1》`**：其 `researchNotes` 逐字寫
「策展層 why 列的合作名單多了 Steven Tyler⋯本稿不引用該名單」。

**裁定：正文零提及該合作名單、零提及 `Steven Tyler`、也不寫「名單裡其實沒有某人」這類否定句。**
該卡 note 本來就沒有「名單」二字，**照 note 寫即自動滿足**；本條只是把「確實沒有自行回補」留成可稽核的紀錄。
同一卡的另外兩處研究層更正**已落地**：軌數照策展層釘的 10 軌（未寫日版 11 軌與〈Milestones〉）、
〈Paris〉照研究層補正寫成「整張碟唯一的人聲曲」而非單曲。定稿 200。

### 第 2575 條：限量張數的中文／阿拉伯數字**在本批並存，不強求一致**

`writer-base.md` 第 176 行：年月日與榜單名次一律阿拉伯數字，**概略數量維持中文數字**。
本批兩張限量盤的 note 給的寫法不同——`Norah Jones《Playing Along》` 是「限量三千張」、
`《Little Broken Hearts: Live at Allaire Studios》` 是「限量 2500 張」。

**裁定：兩張各照自己的 note 寫，不統一。** 三千張是整數概數、2500 張不是，
base 第 176 行本來就以「概略／精確」而非「欄位種類」分流，**兩種寫法各自合規**。
機器檢查亦無異議（千分位掃描 0 命中）。**可逆且有先例，自行定案。**

### 第 2576 條：三張獎項卡與 `Norah Jones` 兩張——**一律照 note 落字，未重算屆數或得主**

依派工信第三節第 4 點與研究層第 2248 條，本棒對下列五張**只做搬運、零推算**：

- `Gregory Porter《Christmas Wish》`→ 第 67 屆葛萊美最佳傳統流行人聲專輯，**寫成入圍**。
- `Meshell Ndegeocello《The Omnichord Real Book》`→ 第 66 屆葛萊美最佳另類爵士專輯，**寫成得獎**，並照 note 寫明是該獎項首屆得主。
- `Mark Knopfler《One Deep River》`→ 第 67 屆葛萊美最佳美式根源歌曲，**入圍單位寫成〈Ahead of the Game〉這首歌，不是專輯**。
- `Julian Lage《Speak to Me》`→ 第 67 屆葛萊美最佳當代器樂專輯，**寫成入圍**。
- `Norah Jones` 兩張（`Playing Along`／`Little Broken Hearts: Live⋯`）**無獎項欄，一字未寫獎項**。

⚠ **`Arturo O'Farrill《Legacies》` 的八座葛萊美與 2015 年《The Offense of the Drum》整格不寫**：
研究稿 `researchNotes` 逐字標明那是累計得獎數、**與本碟無關**，note 也未指派，**照 note 的排除條款辦**。

### 第 2577 條：骨架歸屬句的處理——**11 張帶句，一字不進正文**

派工信第三節第 2 點指名 note 裡的 `這條骨架全批只走本張。` 與 `正文只寫上列各項。` 是指派／排除句。
**本棒實測 a 組 23 張全部帶 `正文只寫上列各項。`，其中 11 張另帶 `這條骨架全批只走本張。`**
（`Corridors`、`return to casual`、`Rollin'`、`Christmas Wish`、`Playing Along`、`Dance Kobina`、
`Little Broken Hearts: Live⋯`、`KARPEH`、`Dolce Vita`、`uNomkhubulwane`、`Technically Acceptable`），
**與派工信說的 11 張逐張相符**。**兩句皆零進正文。**

### 第 2578 條：`互指?` 逐筆判讀結果——**0 行，無須人工判讀**

`node qa-batch.mjs out c166` 對 out-1 的 23 張 `desc` 跑完 `factCheck`，**`互指?` 前綴 0 行、硬標記 0 筆**。
依第 1763-B 條本應逐筆人工判讀，**本批無可判讀之項**。
兩張 `Erik Truffaz` 是同批同藝人的姊妹作，是最可能觸發互指的一組，
**但兩張的 note 各自自足**（`Clap!` 只以「與同年四月那張同一套企畫的第二集」指稱姊妹作、未引用其盤名），
所以連互指都沒有形成。**這也順帶確認本棒沒有把 a 組的事實寫到 b 組的卡上。**

### 第 2579 條：本派工信與 `writer-base.md`／既有裁定牴觸之處——**硬牴觸 0 處，需要記錄 2 處**

1. ✅ **派工信第一節的兩條預先警示都寫對了，且與 base 一致**：
   「首句與 `hook` 的關係照 base 第 158 行」（＝hook 原文一字不改，`qa-check-research.mjs` 確實硬檢查）、
   「`desc` 字數以 base 第 205 行為準（180–240），鉤子層的 230 是 `note` 的預算」。
   **第 1806-B 條那個把兩者混談的錯誤，本信沒有再犯。**
2. ⚠ **派工信第三節第 1 點的推論方向與實測相反**（見第 2571 條）。**不是指示錯誤，是預期與實測不同**，
   而且 base 第 107–124 行本來就禁止繼承偏差方向，**本棒未依該推論預留上浮，結果正確**。
   **記此一筆，免得後批的派工信把「鉤子層超標 → 寫作層也會超標」寫成通則。**
3. ⚠ **派工信第四節第 2 點列的三支自檢命令完整且正確**，`fix-spacing.mjs` 預設 dry-run（第 1740-B 條）
   的提醒亦屬實——**本批 `待補 0 張`，因此未加 `--write`、未改動檔案**。
   **但 `node qa-batch.mjs out c166` 會同時驗 out-1 與 out-2 並比對卡單總數**，
   writer-2 若尚未交件，該指令必報「輸出總張數與卡單不符」。**本棒執行時 out-2 已存在（22 張），總數 45 與卡單相符 ✓**，
   純屬時序運氣。**建議後批的派工信註明這一點**，免得寫作代理把並行組未交件誤判成自己的標記。

### 第 2580 條（⚠ **交件狀態：工作區當下版本才是交件版**，第 1803-B 條，本線第五次）

本節所有數字都是對**磁碟上當下那一份** `desc-tools/batches/output/c166-out-1.json` 實跑出來的，不是中途快照。

⚠ **`main` 上已經有一個 5 筆的中途檔**：提交 `7f5123e`
「checkpoint: 藝人軸稽核／c166 寫作 1／c168 串流探測 中途存檔」
是主線在本棒寫完第 5 張時把工作區檔案撈去做的檢查點，
`git show HEAD:desc-tools/batches/output/c166-out-1.json` 逐字是 **5 筆**。**那不是交件版。**

實測比對：**那 5 筆裡有 4 筆與工作區當下版本逐字相同，
`desc2:Dave McMurray|Grateful Deadication 2` 1 筆不同**（收工前依 note 校正過措辭，字數同為 201），
工作區另多出第 6–23 張共 18 筆。**交件版＝工作區的 23 筆**，請主線以工作區為準再提交一次。

本棒依邊界規定**未 `git commit`、未 `git push`、未動 git 索引**；
`git status --short` 當下只有 ` M desc-tools/batches/output/c166-out-1.json`
與 `?? desc-tools/batches/output/c166-out-2.json`（後者是 writer-2 的檔，本棒未碰）。

**未碰**：`seed_cards.json`／`apex_pool.json`／`PROJECT_MEMORY.md`／KV／Firestore／
`c166-out-2.json`／`c166-hooks-*.json`／其他批次的檔案。

### 第 2581 條：號段結餘

**本棒用掉 2571–2581，2582–2600 未用。**

---

## c-166 b 組（22 張）寫作層裁定　號段 2601–2630

（writer-1 用 2571–2600。本節一律 append，未覆寫任何既有內容——第 1743-B／1806-B 條。）

### 第 2601 條：骨架歸屬句與指派／排除句的處理

b 組 22 張的 `note` 全部帶骨架歸屬句（`這條骨架全批只走本張。`）與排除句（`正文只寫上列各項。`）。
**兩句均為指派語句，22 張一律不進正文**，只當成寫作邊界執行。**22 條骨架兩兩互異，本棒逐張確認無同構撐文。**

### 第 2602 條：五張獎項卡照 `note` 寫，未重算

`Meshell Ndegeocello`《No More Water》**寫得獎**（第 67 屆葛萊美最佳另類爵士專輯）；
`Bill Frisell`《Orchestras》（第 67 屆最佳當代器樂）、`Immanuel Wilkins`《Blues Blood》（第 68 屆最佳另類爵士）、
`Gerald Clayton`《Ones & Twos》（第 68 屆最佳當代器樂）、`Branford Marsalis Quartet`《Belonging》（第 68 屆最佳爵士器樂）
**五張全寫「入圍」且得主名字整格不寫**。依鉤子層第 2298 條定案，本棒未回頭重查。
葛萊美屬 writer-base「可具名的機構獎項」，具名合法。

### 第 2603 條：`Brandon Woody` 的 `SCORE` 素材整格不寫

`DownBeat` 四星（研究稿 facts 逐字帶「四星」）**未寫、未改寫成「樂評正面」**，沿用第 2451 條同一判準。
該張的預算由走調收音機那條骨架與露台、Upendo、口頭禪三格填滿，**不需要靠樂評收尾**
（writer-base 樂評例外二的判準是「拿掉樂評還剩不剩得下故事」，本張剩得下）。

### 第 2604 條：人名字形照 `note`，未自行轉換

`Noriko Ueda` 維持羅馬字（`ARTEMIS`《Arboresque》）；`山中千尋`（卡單藝人欄）與內文的 `筒美京平`
照漢字。**未套用「東亞音樂人一律漢字」去改 `Noriko Ueda`**，理由是 writer-base 該條的適用對象是內文提到的
其他音樂人、而研究層已就該名字形做過判定，派工信第七點也明令照 `note`。

### 第 2605 條：`note` 未列的軌名一律不從 `facts` 補回（實測 6 張）

派工信第三節第 4 點屬實，本棒逐張核對後確認 6 張的 `facts` 帶有 `note` 未列的軌名，**全部未補**：
`Melissa Aldana`（只寫「同名開場曲」，未寫出曲名）、`Bill Frisell`、`Wayne Shorter`、
`Bill Charlap Trio`、`Brandon Woody`（`keyTracks` 兩首皆未進正文）、`Johnathan Blake`（只寫「開場曲」「第二支單曲」）。
**`keyTracks` 欄位在本組不具引用授權**——它是研究層欄位，不是 `note` 的指派。

### 第 2606 條：因字元預算整格捨去的 `note` 項目（7 張）

`正文只寫上列各項。` 是排除條款、不是「每一項都必須寫」。預算不足時依 writer-base
「修剪一律以整格捨去」處理，本棒捨去下列各格，**每張捨去後主故事仍完整、懸念仍收尾**：

1. `Wayne Shorter`：英文原名 `Unidentified Flying Objects`（31 字元）捨去，`hook` 已給中文《不明飛行物》。
2. `McCoy Tyner & Joe Henderson`：〈Taking Off〉曲名捨去，改寫成「另一首由四人共同掛名的曲子」。
3. `Bill Charlap Trio`：作曲者清單（`Kenny Barron`／`Monk`／`Gershwin`）整格捨去，只留「八首全是別人的作品」
   ——writer-base「清單是佐料不是故事，一張最多留一組」，該張的一組已給偷聽對象。
4. `山中千尋`：假名原題〈スニーカーぶる〜す〉捨去，只留碟上題名〈Sneaker Blues〉與 `筒美京平`。
5. `Harold López-Nussa`：祖父全名 `Leonel López-Nussa`（19 字元）與〈Gitanerías〉曲名捨去
   ——`hook` 已交代「封面是祖父畫的」，弟弟的二重奏改用「另一首曲子」承接。
6. `Charles Lloyd, Jason Moran, Marvin Sewell`：〈Hymn to the Mother, for Zakir〉（29 字元）捨去，
   `hook` 的「吹一首給剛走的老搭檔」已指認該曲。
7. `Blue Lab Beats`：三首旅行路線曲名**全留**（`note` 的主故事收尾靠這條線），改為壓縮客座描述那一格。

### 第 2607 條：`Blue Lab Beats` 的廠牌不具名

`note` 全篇以「這個廠牌」指稱、未給名字，`facts` 則有。依「正文只寫上列各項」，
**正文沿用不具名的「這家廠牌」**，未從 `facts` 補廠牌名。同一處理套用於 `Dave McMurray`（「廠牌總裁」不具名）
與 `Joshua Redman`（「這個廠牌的第二張」一句因預算捨去）。

### 第 2608 條：`Branford Marsalis Quartet` 的「引用盤名時帶掛名」落實方式

`note` 要求引用盤名時帶掛名，本棒寫成「`Keith Jarrett` 1974 年的《Belonging》」，
與同段前面的「`Keith Jarrett` 1979 年的《My Song》」對齊。**兩處都具名**，避免代名詞與所指相隔過遠
（writer-base 代名詞條）。該張 214 字元為 b 組最長，`Kenny Kirkland`／`Keith Jarrett`×2／兩個盤名共吃 52 字元。

### 第 2609 條：`Immanuel Wilkins` 的代名詞具名

`note` 原句作「讓一些瘀血流出來給對方看」，正文改寫成「給醫生看」。
理由是該句前面剛出現「一群黑人少年」與「其中一人」兩個指涉，「對方」的所指會斷；
「醫生」由 `note` 的「求醫」直接推出，**不是外部知識**。

### 第 2610 條：本組的預算偏差方向——手算偏高、實測偏低（不可繼承）

依 writer-base 逐張列預算表後，**實測普遍比估值低 10–20 字元**，
初稿 22 張有 **4 張掉到 180 以下**（176／172／162／173），**零張超過 240**——
與派工信第三節「鉤子層已把超標格子整格捨去，沒有留給你砍」一致，但方向與「爵士批次單向超標」
（writer-base 第 284–289 行 w3-02 實測）**相反**。補的順序照 writer-base 第 82 行：
主故事後續的第二個事實 → 一筆成績或聲音。四張補回的都是 `note` 已列、預算表階段先捨去的格
（`Wilkins` 的舞台做菜、`Aaron Parks` 的盤名由來、`Nels Cline` 的 `Wilco`、`Brandon Woody` 的軌名）。
**再次證實第 1806 條那條：任何偏差係數都不要繼承，只繼承「兩端都要驗」。**

### 第 2611 條：三支自檢輸出（對工作區當下版本實跑）

```
node qa-batch.mjs out c166
out-1｜23 張｜字數 189–230｜>260: 0
out-2｜22 張｜字數 184–214｜>260: 0
out 合計 45 張，與卡單相符 ✓
全部通過 ✓

node qa-check-research.mjs batches/input/c166-writer-2.json batches/output/c166-out-2.json
QA 完成｜ 22 張｜標記 0

node fix-spacing.mjs batches/output/c166-out-2.json --field desc
待補 0 張（欄位 desc）
```

**旗標 0**。`fix-spacing` 因待補 0 張而**未加 `--write`**（第 1740-B 條的反面：沒有要改的東西就不必寫檔）。
`out-1` 那一行是 writer-1 的檔案，本棒未碰。

### 第 2612 條：`互指?` 逐筆判讀——0 行，且已人工覆核不是漏報

`node qa-batch.mjs out c166 | grep -c 互指` 逐字回 `0`。**但 0 行有兩種可能**（真的沒有互指／比對層沒掃到），
本棒另寫比對確認是前者：b 組有 8 處正文寫到「同批別張卡的掛名或盤名」——
`Wayne Shorter`（出現在 `Melissa Aldana` 那張）、`Meshell Ndegeocello`（在 `Immanuel Wilkins` 那張）、
`Melissa Aldana`（在 `Joshua Redman` 那張）、`Joe Henderson`（在 `Bill Charlap Trio` 與 `Horace Silver` 兩張）、
`Keith Jarrett`／`Sam Rivers`／`Antônio Carlos Jobim`。
**八處逐一回查各自本卡的研究稿，全部命中本卡 `facts`**（不是靠同批別張豁免），
所以判定為「過」而非「互指」。**零筆需要人工放行，零筆是掛錯卡。**

### 第 2613 條：派工信與 `writer-base.md`／既有裁定的牴觸盤點——硬牴觸 0，需記錄 3 處

1. ⚠ **派工信第三節第 1 點說「超標的格子已整格捨去，沒有留給你砍」屬實，但沒有提醒下限風險。**
   實況是 b 組偏低（第 2610 條），**4 張初稿掉到 180 以下要回頭補**。這不是錯，是 `writer-base` 第 78–85 行
   早就寫過的「偏差是雙向的」——**派工信只轉述了上限那一半**。建議後批在同一句補「兩端都要驗」。
2. ⚠ **派工信第三節第 4 點「`note` 沒列的軌名就是刻意不寫」與第二節「正文只寫上列各項」合起來，
   字面上會讀成「`note` 列的每一項都必須寫」。** 與 `writer-base` 第 96／124 行的「整格捨去」在預算吃緊時直接打架。
   本棒依第 2606 條判準處理（排除條款，不是必寫清單）——**7 張各捨一格**。**這一點請主線在後批的派工信寫清楚。**
3. ⚠ **派工信第四節第 2 點要求 `fix-spacing.mjs` 必須加 `--write` 才真的改檔（第 1740-B 條）——正確，
   但本組待補 0 張，加了也是空轉。** 記一筆免得複核者以為漏跑。

**派工信寫對、本棒實測確認的**：第一節指的 `writer-base` 第 158 行（首句＝`hook` 原文一字不改）
確實被 `qa-check-research.mjs` 硬檢查（`hook 未原封開頭`），22 張全過；
第一節「`desc` 以第 205 行的區間為準、鉤子層的 230 是 `note` 預算」（第 1806-B 條）屬實——
本組 `note` 實測 203–237，若誤當成 `desc` 區間會整批寫錯。

### 第 2614 條（⚠ **交件狀態：工作區當下版本才是交件版**，第 1803-B 條）

本節所有數字都是對**磁碟上當下那一份** `desc-tools/batches/output/c166-out-2.json` 實跑出來的，不是中途快照。
該檔 **22 筆、`key` 逐字複製自輸入檔且順序相同**（`qa-batch` 的「與卡單相符 ✓」已覆核）。
**「筆數對了」不等於「定稿了」**：本棒在 22 筆到齊之後又改過 6 處（第 2606、2609 條），
**請主線以工作區當下版本為準提交，不要用中途存檔。**

本棒依邊界規定**未 `git commit`、未 `git push`、未動 git 索引**。
`git status --short` 當下兩行：` M desc-tools/batches/output/c166-out-1.json`（writer-1 的檔，本棒未碰）、
`?? desc-tools/batches/output/c166-out-2.json`（本棒產出）。

**未碰**：`seed_cards.json`／`apex_pool.json`／`PROJECT_MEMORY.md`／KV／Firestore／
`c166-out-1.json`／`c166-hooks-a.json`／`c166-hooks-b.json`／其他批次的檔案。

### 第 2615 條：號段結餘

**本節用掉 2601–2615，2616–2630 未用。**
