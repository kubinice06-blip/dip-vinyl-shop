...
## 第 394 條（主線 2026-09-15，研究層 b 組交件後）：**年份再改三張——策展層的改判方向對，但兩張改得不夠深**

研究層 b 組 22/22 全 `full`，逐張跑了 Discogs 盤面與日文百科的領銜作目錄。**三張再改**：

| 盤 | 策展層／卡單 | 改 | 依據 |
|---|---:|---:|---|
| 松本英彦《Four Wings》 | 1994 | **1980** | Discogs 兩筆同版本條目的盤面是 Trio Records／Full House **PAP-9197、1980、Japan**；策展層拿的店面 1994-12-21 是 CD 再發 |
| 宮沢昭《Love For Sale》 | 1978 | **1967** | Discogs 盤面註記「Recorded 1966-10-07 與 1967-01-13，**Original LP on Takt**」；RCA RVL-5523（1978）是再發 |
| 宮沢昭カルテット《Sea Horse》 | 1989 | **2002** | MB 轄下唯一那筆 1989 release **country 未填、label-info 是空陣列**——沒有實體對應的登錄；實體盤是 2002。**1989 移進錄音年** |

**策展層改對的四張，研究層逐一背書**：白木秀雄《白木秀雄》1959（MB 的 1958 是錄音年）、
《プレイズ・ボッサ・ノバ》1963、《祭りの幻想》1961、《ファンキー!登場》1960（**廠牌更正成ビクター也對**）。

**通則**：`first-release-date` 失真的第四種形狀——**MB 有一筆「沒有國別、沒有廠牌」的 release**。
那種列是登錄者從某處抄來的年份，**不是一張碟**。判年份時，
**先看那筆 release 有沒有 country 與 label-info；兩欄都空的不能當首發依據。**

## 第 395 條（同日）：**`qa-batch` 的簡體字檢查對日文內容會誤報——日文專有名詞不得「改正」**

研究 b 被標「簡體字：会国双」，逐處查看全部是**日文漢字的正常用法**：
`トリオ株式会社`（公司名）、**`稲葉国光`（貝斯手本名）**、`双方ともに同一録音`（日文百科引文）。
**這些不是簡體字，改了就是把人名寫錯。**
**通則**：日本爵士這條線的研究稿會大量引日文原文，`qa-batch` 的簡體檢查在這條線上**只能當提示**，
要逐處看過；**日文專有名詞（人名、公司名、盤名）一律保留原字**，正文的中文敘述才要繁體。

## 第 396 條（同日）：**研究 b 組也在收工回報時被內容過濾砍掉，檔案 22/22 完整**

第 303 條同形第四次（c-127 研究 a、c-129 研究 a、c-129 寫作 a、本次）。
**最後一句是「Research complete. Now writing the delivery file in chunks.」，看起來像沒寫完，實際上是完整的。**
**代理被砍時，一律先看輸出檔再決定要不要重派。**

## 第 397 條（主線 2026-09-15，研究層 a 組交件後）：**MB 的 `secondary-types` 漏標 Live 的三張，`releaseType` 維持 Album、正文寫明是現場錄音**

《Dedicated to Charlie Parker》《Open Road》《Hino at Berlin Jazz Festival '71》**三張都是現場盤**，
但 MB 的 `secondary-types` 是空的，卡單照記 Album。
**依第 262 條（欄位要與那一筆登記相符），`releaseType` 不改**——它反映的是資料庫端的登記；
**現場錄音這個事實寫進正文**（hook 層要在 note 指定）。
與第 424 條（大野雄二有 16 張原聲帶 MB 沒標）同族：**`secondary-types` 是提示不是真相，兩個方向都會漏。**

## 第 398 條（同日）：**《East Wind》的「廠牌第一號盤」保留——研究層找到可具名出處**

第 253 條（無來源的「第一／唯一」句一律擋）在這張**不適用**：
Universal Music Japan 官方頁逐字寫「イースト・ウィンド・レーベル第一回作品」，
英文百科的廠牌條目與 1974 年成立年份對得上。**這是這條線上第一句通過查證的「第一」。**
**通則：擋的是沒有出處的「第一／唯一」，不是這個句型本身。**

## 第 399 條（同日）：**店面缺席的切法再推一格——不是廠牌大小，是「權利人」與「時期」**

策展層第 378 條說「缺的是 CBS/Sony 1969–74」。研究層兩個反例：
(a) Victor 把日野皓正 1977–90 整批數位化，**1971 年那張 Victor World Group 的柏林現場卻不在**——同一家廠牌內部按時期切；
(b) 佐藤允彦的 日本コロムビア／DENON 目錄幾乎全有、**Sony 系一張都沒有**——同一位藝人身上按權利人切。
另：Sony 把《Pastoral》的一首單曲授權給 2025 年的拼盤，**所以缺的不是授權，是整張盤沒有數位條目**。
**通則：「某某廠牌在店面上有沒有」這種判斷，粒度是「廠牌×時期×權利人」，不是廠牌。**

## 第 400a 條（同日）：**研究層漏查試聽的三種原因（找回 9 張的實證）**

盤名多後綴（`(Live, 1969)`）、**盤名是片假名或羅馬字轉寫**（イースト・ウィンド／アウト・オブ・ケイオス／サンシャワー／`Magnolia No Ki`）、
**掛在別的 artistId**（《Wishes》掛「東風(こち)」）。
⚠ **同一個 MB 實體在店面兩個端點回不同顯示名**：`Masahiko Sato Trio` 在 `lookup` 是「佐藤允彦トリオ」、在 `search` 是「Satoh Masahiko Trio」。
**三種查法的第二步（藝人目錄）要把每個 artistId 的目錄整份 dump 下來比對，不能只看搜尋結果。**
