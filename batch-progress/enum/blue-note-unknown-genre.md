...
## 四、`jazz-missing`／2000 年以後（58 張）

> **2026-09-18 更新（主線第 1557 條）：這一節原本寫「不建 slice、交給主線日後併進 c-156 以後的批次」，
> 已作廢。** 併進既有批次做不到——c-156～c-167 的 slice 全部已經進管線或已經發過卡。
> 58 張裡 **#96 Wilkins《Vol. 1》已補進 `c167/slice.json`**（避免出現只有 Vol. 2、3 的斷號），
> **其餘 57 張已另切成 `batch-progress/c169/slice.json`（41 張）與 `c170/slice.json`（16 張）**，
> 走與其他批完全相同的五層管線。切法見下方表格後的說明。
...
### 這 57 張怎麼切（2026-09-18 主線第 1557 條）

- **界線取 2020 年**：2000–2019 歸 `c169`（41 張，a 21／b 20），2020 年以後歸 `c170`。
- **例外一條：Kandace Springs 的三張（2016／2018／2020）整組移進 `c170`**，
  讓「池中一張卡都沒有的藝人」整條線落在同一批——**反同構條款與掛名判定都只需做一次**，
  也不會出現「同一位藝人被兩批各判一次掛名」的第 1418 條風險。
  Joel Ross 的四張（2020／2022／2024／2026）本來就全在 2020 年以後，天然同批。
- `c170` 因此是 16 張（a 8／b 8），是本線最小的一批；**不再往下拆**。
- 兩批的 `note` 欄一律寫明**「列舉層曲風判錯層級（讀藝人端而非 RG 端）而漏切」**＋人工判定的曲風，
  策展層看得到自己為什麼晚了十批才收到這些盤。
- 兩批的 `genre` 欄統一寫 `jazz`（人工判定值另存 `triageGenre`，`triageN` 保留本檔編號以便回查）。
