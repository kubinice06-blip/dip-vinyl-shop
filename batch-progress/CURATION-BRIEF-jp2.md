...
《Prism Live》頁把「サンバ・カリオカ(後のカリオカ)」寫死——**那是主線第 1971-B 條那條裁定的第三個來源、而且是原廠**）。
⚠ **`sonymusic.co.jp` 替 `T-SQUARE` 與 `ザ・スクェア` 各留一個並存的藝人頁（兩個都 200）**
——**那是字串分裂的原廠側依據。**
...
31. ⚠ ⚠ ⚠ **固定動作：同批兩組的曲題表互掃一次**（主線第 1987-B 條）：
   **c-187 的《Eyes of the Mind》漏算一軌，因為那一軌的原盤是同批另一組的收件卡**
   ——**策展層以「池中已有」為基準，看不到同批另一組，而六道 dedup 也抓不到。**
   **→ 交件前把兩組的曲題表對一次；同批跨組撞曲目是新的一族。**
32. ⚠ ⚠ **「整筆 credits 空白」與「逐軌 credits 空白」是兩層，不要混**（主線第 1987-B 條）：
   **c-187 a 的一張卡，策展層寫「原壓查不到編制」，而原壓的逐軌 credits 本來就列完十四位**
   ——**空的只是「整筆」那一層。** **`releases/<id>` 的 `extraartists` 與逐軌 `tracklist[].extraartists` 都要看。**
33. ⚠ **`Apple` 命中要先比對 MB 有沒有對應的 `Digital Media` 實體**（主線第 1987-B 條）
   ——**否則同一個數位版會被算成兩版**（c-187 a 實測三筆會各多算一版）。
   ⚠ **反過來，MB 那筆「無廠牌無日期的殘缺 release」可以用逐軌毫秒對上 Apple 的秒數釘成數位版**（零成本的新動作）。
