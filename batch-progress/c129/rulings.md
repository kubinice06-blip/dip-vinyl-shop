...
## 第 346 條（寫作層 2026-09-15 自判，主線追認）：**印尼／泰國那批的「樂評媒體榜單」一律不寫**

`note` 指定 Iwan Fals《Opini》收尾寫「開場曲在某雜誌印尼版 150 首名單裡排第 65」，
但 `writer-base.md` 的〈榜單、獎項、名人堂〉2026-08-08 定案是**樂評媒體的榜單／票選整條不寫**，
2026-08-09 的具名例外只在「樂評是這張卡最重要的內容」時成立。**這張的主故事不是樂評，拿掉仍寫滿 222 字。**
寫作層依派工詞「以 writer-base 為準」判不寫，**正確**。同類事實在 Chrisye《Puspa Indah》與
Koes Plus《Volume 8》的 facts 裡也有，一律未寫。
**通則**：hook 層把 facts 裡的榜單搬進 `note` 時，**要先過 writer-base 的榜單條款**；
`note` 與 writer-base 打架時以 writer-base 為準。這是第 305 條（禁令在 hook 層漏出去）的鏡像——
**這次漏出去的不是人名而是禁寫的榜單**。

## 第 347 條（同日）：**hook 說「三種曲序」但 facts 只有兩種——寫作層照 facts 走**

คาราวาน《คนตีเหล็ก》的 hook 寫「卡帶與資料庫端三種曲序」，
但 facts 明寫「串流側曲序與 1983 卡帶 A／B 面相同」——**實際只有兩種**。
寫作層只寫卡帶與資料庫端的差異、沒有去湊第三種，**正確**。
**hook 的數字與 facts 打架時，以 facts 為準、hook 照抄但正文不擴寫那個數字。**

## 第 348 條（同日）：**78 轉蟲膠盤不能寫成「母帶」**

《Luk Thung: Classic & Obscure 78s》的來源是收藏家的 78 轉蟲膠盤收藏，不是磁帶。
**載體名詞不得互換**（與第 262 條同族：載體要與那一筆登記相符）。
