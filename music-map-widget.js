(() => {
  ...
  function addStyle() {
    ...
    style.textContent = `
      ...
      .music-map-layout{display:grid;grid-template-columns:minmax(0,1.25fr) minmax(180px,.75fr);gap:16px;align-items:center}.music-map-svg{width:100%;max-width:500px;margin:0 auto;display:block}.music-map-svg .grid{fill:none;stroke:var(--line,#ddd);stroke-width:1}.music-map-svg .axis{stroke:var(--line,#ddd);stroke-width:1}.music-map-svg .axis.selected{stroke:var(--black,#111);stroke-width:2}.music-map-svg .axis-hit{stroke:transparent;stroke-width:24;pointer-events:stroke;cursor:pointer}.music-map-svg .rail{stroke:var(--line,#ddd);stroke-width:2}.music-map-svg .shape{fill:rgba(17,17,17,.12);stroke:var(--black,#111);stroke-width:2}.music-map-svg .dot{fill:var(--black,#111)}.music-map-svg .node{fill:#fff;stroke:var(--line,#ddd);stroke-width:1.5}.music-map-svg .node.on{fill:var(--black,#111);stroke:var(--black,#111)}.music-map-svg text{fill:var(--black,#111);font-family:var(--mono,monospace);font-size:18px;letter-spacing:.02em}.music-map-svg .val{fill:var(--gray,#888);font-size:14px}.music-map-svg .mobile-label{font-size:24px}.music-map-svg .mobile-value{font-size:18px}
      .music-map-paths{display:grid;gap:7px}.music-map-path{display:grid;grid-template-columns:1fr auto;gap:10px;font-size:10px;border-bottom:1px solid var(--line,#ddd);padding-bottom:6px}.music-map-path-button{width:100%;font:inherit;color:inherit;background:none;border:0;border-bottom:1px solid var(--line,#ddd);text-align:left;cursor:pointer}.music-map-path-button.selected{background:var(--black,#111);color:#fff;padding:6px}.music-map-path span:last-child{color:var(--gray,#888);text-align:right}.music-map-path-button.selected span:last-child{color:#ddd}.music-map-path-button:focus-visible,.music-map-svg .axis-hit:focus-visible{outline:2px solid var(--black,#111);outline-offset:2px}.music-map-untagged{font-size:10px;color:var(--gray,#888);padding-top:2px}.music-map-note{margin-top:10px;font-size:10px;line-height:1.7;color:var(--gray,#888)}
  ...
  function render(target, source, compact, message = '', interactive = false, selected = '') {
    ...
    const paths = (compact ? ranked.slice(0, 4) : GENRES).map(([id, name]) => interactive
      ? `<button type="button" class="music-map-path music-map-path-button ${selected === id ? 'selected' : ''}" data-map-genre="${id}"><span>${name}</span><span>${state.credits[id]} 點 · ${Math.round(state.credits[id] / (all || 1) * 100)}% · ${milestone(state.credits[id])}</span></button>`
      : `<div class="music-map-path"><span>${name}</span><span>${state.credits[id]} 點 · ${Math.round(state.credits[id] / (all || 1) * 100)}% · ${milestone(state.credits[id])}</span></div>`).join('');
  ...
  window.DipMusicMap = { empty: () => normalise(), normalise, mount(target, opts = {}) { addStyle(); let data = normalise(opts.data), message = opts.message || '', selected = ''; const interactive = typeof opts.onGenreSelect === 'function'; const select = id => { if (!ids.includes(id)) return; selected = id; render(target, data, !!opts.compact, message, interactive, selected); opts.onGenreSelect?.(id); }; if (interactive) { target.addEventListener('click', event => { const hit = event.target.closest?.('[data-map-genre]'); if (hit) select(hit.dataset.mapGenre); }); target.addEventListener('keydown', event => { if (event.key !== 'Enter' && event.key !== ' ') return; const hit = event.target.closest?.('.axis-hit'); if (!hit) return; event.preventDefault(); select(hit.dataset.mapGenre); }); } render(target, data, !!opts.compact, message, interactive, selected); return { update(next, nextMessage = '') { data = normalise(next); message = nextMessage; render(target, data, !!opts.compact, message, interactive, selected); }, select }; } };
})();
