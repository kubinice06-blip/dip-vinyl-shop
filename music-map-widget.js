(() => {
  ...
  window.DipMusicMap = { empty: () => normalise(), normalise, titleFor, mount(target, opts = {}) { addStyle(); let data = normalise(opts.data), message = opts.message || '', selected = ''; const interactive = typeof opts.onGenreSelect === 'function'; const select = id => { if (!ids.includes(id)) return; selected = id; render(target, data, !!opts.compact, message, interactive, selected); opts.onGenreSelect?.(id); }; if (interactive) { target.addEventListener('click', event => { const hit = event.target.closest?.('[data-map-genre]'); if (hit) select(hit.dataset.mapGenre); }); target.addEventListener('keydown', event => { if (event.key !== 'Enter' && event.key !== ' ') return; const hit = event.target.closest?.('.axis-hit'); if (!hit) return; event.preventDefault(); select(hit.dataset.mapGenre); }); } render(target, data, !!opts.compact, message, interactive, selected); return { update(next, nextMessage = '') { data = normalise(next); message = nextMessage; render(target, data, !!opts.compact, message, interactive, selected); }, select }; } };
})();
