(() => {
  ...
  function addStyle() {
    ...
    style.textContent = `
      ...
      @media(max-width:520px){.music-map-layout,.music-map.compact .music-map-layout{grid-template-columns:1fr}.music-map.compact .music-map-svg{max-width:310px}.music-map.compact .music-map-paths{grid-template-columns:1fr 1fr;gap:5px}.music-map.compact .music-map-path{font-size:8px}.music-map-head{align-items:flex-start;flex-direction:column;gap:3px}}
    `;
  ...
  function chart(data, compact) {
    ...
    return `<svg class="music-map-svg" viewBox="0 0 500 480" role="img" aria-label="聆聽品味分布圖">${grid}${axes}${rails}<path class="shape" d="${path}"/>${points}${mapNodes}${labels}</svg>`;
  }
  ...
  function render(target, source, compact, message = '') {
    ...
    target.innerHTML = `<div class="music-map-head"><div><div class="music-map-kicker">MY COLLECTION · PERMANENT DATA</div><div class="music-map-title">我的聆聽品味地圖</div></div><div class="music-map-total">收藏 ${state.albums} 張<br>曲風點數 ${all}</div></div><div class="music-map-layout"><div>${chart(state, compact)}</div><div class="music-map-paths">${paths}</div></div><div class="music-map-note">中央八角圖是各曲風在你的永久收藏中所占比例；外側節點是固定的收藏里程碑（1／3／7／15／30／60），不會因為總收藏增加而倒退。跨界專輯會同時點亮兩條路徑。</div>${message ? `<div class="music-map-note">${message}</div>` : ''}`;
  }
