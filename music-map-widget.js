(() => {
  ...
  function chart(data, compact, interactive, selected) {
    ...
    const mapNodes = GENRES.map(([id], i) => MARKS.map((m, j) => { const p = at(i, nodes[j], cx, cy); return `<circle class="node ${data.credits[id] >= m ? 'on' : ''}" data-map-node="${id}-${m}" cx="${p.x}" cy="${p.y}" r="4.5"/>`; }).join('')).join('');
