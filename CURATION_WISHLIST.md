...
## 2026-09-02（c-61 策展時實掃卡池發現）：廣度線該補的北歐正典

深掘線收的是「非正典那一端」（裁定第 47 條），所以下面這些**不能靠深掘批補進來**，
硬塞會讓 2026-09-02 建立的 `lineType` 分類失去意義。留給往後的**廣度**批。

| 藝人 | 碟 | 池中 | 為什麼該收 |
|---|---|---|---|
| **Bo Hansson** | 《Sagan om ringen》(1970) | **零張** | 瑞典 Silence 廠牌的旗艦盤（SRS 4600），進過英國 Charisma 全球發行、Discogs 有 90 筆版本。資料最齊，c-61 的 b 組刻意沒收只因為它屬正典 |

同一份名單該一併查的：**Silence 廠牌其他進過國際發行的旗艦盤**
（c-61 的 b 組只收了 Silence 的非正典那一端 4 張）。

北歐這一區的廣度缺口比預期大——c-61 的 b 組實掃回報，
它提的 **24 位藝人在池中一張都沒有，連藝人字串都不存在**
（瑞典 progg 5、丹麥 2、挪威 2、芬蘭 2、冰島 2、荷蘭 3、比利時 2、澳洲 4、紐西蘭 2）。
深掘批補掉的是其中的非正典那一端，**正典那一端仍是空的**。
