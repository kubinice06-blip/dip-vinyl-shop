...
### 2026-09-01｜蘇聯爵士與南斯拉夫地下搖滾　**店主點名，待做**

店主 2026-09-01 指示「蘇聯爵士也是空白的 那裏面也有很豐富的片藏／
南斯拉夫地下搖滾也是豐富的片藏」。兩邊都先實測過卡池，店主的判斷正確：

- **蘇聯／俄語圈爵士：取樣 40 位藝人，命中 0。** Ganelin Trio、Сергей Курёхин、
  Арсенал（Алексей Козлов）、Олег Лундстрем、Адди Рознер、Вагиф Мустафазаде
  全部不在池裡。**併入進行中的 c-53 當第三組**（同一個國家、同一套 Мелодия
  發行基建、掛名規則已在 c-53 定案，不需要另開批）。
- **南斯拉夫：只有 2 位藝人、9 張卡**，而且都是對外輸出的那張臉——
  Laibach×5（Mute 旗下的藝術／工業線）與 Goran Bregović×4（電影配樂）。
  真正的南斯拉夫搖滾正典 Bijelo Dugme、Idoli、Šarlo Akrobata、Ekatarina Velika、
  Azra、Haustor、Buldožer、Riblja Čorba、YU Grupa、Smak 一張都沒有。
  另有巴爾幹銅管與羅姆音樂數張（Boban Marković、Esma Redžepova、Kočani Orkestar、
  Šaban Bajramović），那是不同的場景，不算重疊。**排為 c-54。**

**執行備註**：
- 蘇聯爵士的資料基建可能與搖滾線不同。Ganelin Trio 的正典多半由英國 **Leo Records**
  在西方發行，MB／CAA 覆蓋預期良好；但 Мелодия 自己的爵士系列與各共和國的錄音
  （Вагиф Мустафазаде 的亞塞拜然 jazz-mugham、Tõnu Naissoo 的愛沙尼亞線）
  預期較差，要有走人工身分路線的心理準備。
- 南斯拉夫的掛名要**先定字母**：塞爾維亞-克羅埃西亞語同時有拉丁與西里爾兩套正字法
  （Ekatarina Velika／Екатарина Велика），且拉丁那套帶擴充字母 č ć š ž đ。
  池中現有 19 張卡帶這些字母（多為捷克與波蘭古典），所以字母本身不是問題，
  但**同一個團有兩種文字寫法**是 c-53 已經踩過的坑，開批第一件事就要定。
- 南斯拉夫還有一個 c-53 沒有的問題：**國家已經不存在**。1991 年後的塞爾維亞、
  克羅埃西亞、斯洛維尼亞、波士尼亞各自算數，策展層要先講清楚收錄範圍
  是「南斯拉夫時期（至 1991）」還是「含後續各國」。


### 2026-08-31｜東南亞搖滾與印尼爵士（c-SEA）　**店主點名，待做**

店主 2026-08-31 指示「東南亞搖滾 印尼爵士等等 很值得做 排入」。

這是卡池目前**最大的一塊空白**：`world` 只有 864 張，而且多半不是這一區。
方向（待策展層展開）：

- **印尼**：1970 年代的 psych／funk 搖滾（Koes Plus、Panbers、AKA、Dara Puspita），
  Guruh Gipsy 那條前衛線，以及 **印尼爵士**（Bubi Chen、Jack Lesmana、
  Indra Lesmana、Ireng Maulana）。
- **泰國**：molam 與 luk thung 的樂隊化世代、1970s 泰式 funk。
- **菲律賓**：Manila sound、Pinoy rock（Juan de la Cruz Band）。
- **馬來西亞／新加坡**：pop yeh-yeh 世代（與既有的星馬第二圈不重複，那批是華語線）。
- **越南**：1960–70 年代西貢的搖滾與 Bolero。

**執行備註**：MusicBrainz 建檔率預期中等偏低，但比台灣獨立盤好——
Sublime Frequencies、Strut、Now-Again、Soundway 這些再發廠牌把不少原盤重新建了檔，
封面也因此進得了 CAA。**但要小心 §5.6**：這一區大量正典是廠牌考古合輯
（《Thai Funk》《Sumatran Folk Cinema》這類），依合輯規則要帶
`exceptionReason` 與兩個 HTTPS 證據網址，不能整批當正規專輯收。

### 台灣獨立樂團（c-TW3）：封面在 2026-08-31 重測後**部分可解**

原本 26 張卡在封面。2026-08-31 抽驗 14 張，**Apple 台灣的目錄與 8/23 那次不同了**：

| 卡 | 結果 |
|---|---|
| 骨肉皮《壹玖玖伍台灣地下音樂檔案〈肆〉》 | ✅ Apple 台灣有 1995 原盤 |
| 刺客《I Don't Care》 | ✅ 有 1994 原盤——**掛名是「刺客樂團」**，8/23 用「刺客」查才落空 |
| 其餘 12 張 | ✗ 仍查無（濁水溪公社、賽璐璐、盪在空中、農村武裝青年、好客樂隊等） |

1976 與何欣穗有命中但**是別張專輯**（《不合時宜》《She & Me》），不能用。

**待做**：整批 26 張重測，掛名要同時試「團名」與「團名＋樂團」兩種寫法。
研究層的工作 8/23 就完成了，補到封面即可上架，不需重跑。
