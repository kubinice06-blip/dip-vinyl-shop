...
// ── 3-B. 一般否定句（只報不擋）───────────────────────────
// 2026-09-22（c-178／c-180／c-182 三批鉤子層各自人工掃到一次，主線第 1949-B 條）：
// `TRACE` 收的全是「一看就是校對痕跡」的字串（查無／不得寫／禁補⋯），
// **但真正漏進正文的是最普通的那兩個：`不是` 與 `沒有`。**
// 「他不是科班出身」這種是內容、合法；「那個人不是高羽元彦」是校對痕跡、要整格捨去。
// **機器分不出來**，所以這一道只列出來給人看，不進總標記。
const NEG = /不是|沒有|並不|未曾|從未/g;
{
  const rows = [];
  for (const r of all) {
    const m = String(r.note || '').match(NEG);
    if (m) rows.push(`[${r.g}] ${r.k}  →  ${[...new Set(m)].join('／')}`);
  }
  if (rows.length) console.log(`\n（只報不擋）note 含一般否定句 ${rows.length} 筆——逐筆看是內容還是校對痕跡：\n  ${rows.join('\n  ')}`);
}
