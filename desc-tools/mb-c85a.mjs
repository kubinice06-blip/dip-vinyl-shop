...
for(const id of ids){
  ...
  for(const r of (rs.releases||[])){
    console.log(JSON.stringify({id:r.id,title:r.title,date:r.date,country:r.country,status:r.status,barcode:r.barcode,
      ...
      tracks:(r.media||[]).flatMap(m=>(m.tracks||[]).map(t=>`${t.position}. ${t.title} ${t.length?Math.floor(t.length/60000)+':'+String(Math.round(t.length%60000/1000)).padStart(2,'0'):''}`))},null,1));
  }
}
